param(
    [string]$Manifest = "docs/function-lane/W22_CRITERIA_SHAPE_SCENARIO_MANIFEST_SEED.csv",
    [string]$Out = ".tmp/w22-criteria-shape-results.csv"
)
...
try {
    ...
    foreach ($row in $rows) {
        ...
        foreach ($assignment in (Parse-Assignments -Raw ([string]$row.setup_values))) {
            ...
            $scalar = Convert-ManifestScalar -Expr ([string]$assignment.Expr)
            if ($scalar -is [string]) {
                $target.Value = $scalar
            } else {
