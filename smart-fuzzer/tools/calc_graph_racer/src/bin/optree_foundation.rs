...
impl Vec80 {
    ...
    fn map1(&self, f: impl Fn(&Ext80) -> Ext80) -> Vec80 { Vec80(self.0.iter().map(f).collect()) }
    // spill to double (RN53) and reload as Ext80 — represents an FSTP qword / reload.
    fn spill_rn(&self) -> Vec80 { self.map1(|x| ext_from_f64(ext_to_f64(x, RN53))) }
    fn to_f64_bits(&self) -> Vec<u64> { self.0.iter().map(|x| ext_to_f64(x, RN53).to_bits()).collect() }
}

// x87 ops: PC64 (RN64), operands loaded to 80-bit, result 64-bit-mantissa (extended).
fn vadd(a: &Vec80, b: &Vec80) -> Vec80 { a.map2(b, |x, y| ext_add(x, y, CW)) }
fn vsub(a: &Vec80, b: &Vec80) -> Vec80 { a.map2(b, |x, y| ext_sub(x, y, CW)) }
fn vmul(a: &Vec80, b: &Vec80) -> Vec80 { a.map2(b, |x, y| ext_mul(x, y, CW)) }
fn vdiv(a: &Vec80, b: &Vec80) -> Vec80 { a.map2(b, |x, y| ext_div(x, y, CW)) }
...
fn sse_add(a: &Vec80, b: &Vec80) -> Vec80 { a.map2(b, |x, y| ext_from_f64(d(x) + d(y))) }
fn sse_sub(a: &Vec80, b: &Vec80) -> Vec80 { a.map2(b, |x, y| ext_from_f64(d(x) - d(y))) }
fn sse_mul(a: &Vec80, b: &Vec80) -> Vec80 { a.map2(b, |x, y| ext_from_f64(d(x) * d(y))) }
fn sse_div(a: &Vec80, b: &Vec80) -> Vec80 { a.map2(b, |x, y| ext_from_f64(d(x) / d(y))) }
...
fn main() {
    ...
    for line in csv.lines().skip(1) {
        ...
        if f.len() < 6 { continue; }
        rows.push((f[0].parse().unwrap(), f[1].parse().unwrap(), b(f[2]), b(f[3]), b(f[4]), b(f[5]).to_bits()));
    }
    let m = rows.len();
    ...
    let ef = ext_from_f64;
    let mk = |f: &dyn Fn(&(i32, u32, f64, f64, f64, u64)) -> Ext80| Vec80(rows.iter().map(f).collect());
    ...
    let leaf_tau_e = mk(&|r| ext_mul(&ef(-(r.1 as f64)), &ext_fyl2xp1(&ln2, &ef(2f64.powi(r.0)), CW), CW));
    let leaf_u_e = mk(&|r| {
        let tau = ext_mul(&ef(-(r.1 as f64)), &ext_fyl2xp1(&ln2, &ef(2f64.powi(r.0)), CW), CW);
        exp_ext(&tau, &l2e).0
    });
    let leaf_w_e = mk(&|r| {
        let tau = ext_mul(&ef(-(r.1 as f64)), &ext_fyl2xp1(&ln2, &ef(2f64.powi(r.0)), CW), CW);
    ...
    let leaves: Vec<(&str, &Vec80)> = vec![
        ("one", &leaf_one), ("two", &leaf_two), ("half", &leaf_half), ("ln2", &leaf_ln2),
        ("l2e", &leaf_l2e), ("r", &leaf_r), ("n", &leaf_n), ("negn", &leaf_negn),
        ("tau_d", &leaf_tau_d), ("u_d", &leaf_u_d), ("lnu_d", &leaf_lnu_d), ("a_d", &leaf_a_d),
        ("l1p_e", &leaf_l1p_e), ("tau_e", &leaf_tau_e), ("u_e", &leaf_u_e), ("w_e", &leaf_w_e),
        ("a_e", &leaf_a_e), ("lnu_e", &leaf_lnu_e),
    ];

    let score = |v: &Vec80| -> usize { v.to_f64_bits().iter().zip(&em_target).filter(|(a, b)| a == b).count() };

    // sanity: ext leaves spill == captured doubles
    println!("=== leaf sanity (80-bit resident spilled == captured double) ===");
    println!("tau_e==tau_d: {}/{}", leaf_tau_e.spill_rn().to_f64_bits().iter().zip(leaf_tau_d.to_f64_bits()).filter(|(a,b)|**a==*b).count(), m);
    println!("u_e==u_d:     {}/{}", leaf_u_e.spill_rn().to_f64_bits().iter().zip(leaf_u_d.to_f64_bits()).filter(|(a,b)|**a==*b).count(), m);
    println!("lnu_e==lnu_d: {}/{}", leaf_lnu_e.spill_rn().to_f64_bits().iter().zip(leaf_lnu_d.to_f64_bits()).filter(|(a,b)|**a==*b).count(), m);
    ...
    fn hash80(v: &Vec80) -> u128 {
        ...
        for e in &v.0 { for &byte in &e.0 { h = h.wrapping_mul(0x100000001b3).wrapping_add(byte as u128); } }
        h
    }
    #[derive(Clone)]
    struct Node { v: Vec80, dbl: bool }
    // leaf kinds: constants/inputs/_d are double; ln2,l2e,_e are extended
    let dbl_leaf = |name: &str| matches!(name, "one"|"two"|"half"|"r"|"n"|"negn"|"tau_d"|"u_d"|"lnu_d"|"a_d");
    let mut nodes: Vec<Node> = leaves.iter().map(|(nm, v)| Node { v: (*v).clone(), dbl: dbl_leaf(nm) }).collect();
    ...
    for level in 1..=3 {
        ...
        let mut push = |v: Vec80, dbl: bool, seen: &mut std::collections::HashSet<u128>, fresh: &mut Vec<Node>, found: &mut bool| {
            let h = hash80(&v);
            if seen.insert(h) {
                if v.to_f64_bits() == target_double { *found = true; }
        ...
        for i in 0..cur.len() {
            for j in 0..cur.len() {
                ...
                if i <= j { push(vadd(&a.v, &bb.v), false, &mut seen, &mut fresh, &mut found); }
                push(vsub(&a.v, &bb.v), false, &mut seen, &mut fresh, &mut found);
                if i <= j { push(vmul(&a.v, &bb.v), false, &mut seen, &mut fresh, &mut found); }
                push(vdiv(&a.v, &bb.v), false, &mut seen, &mut fresh, &mut found);
                // SSE2 arith (both double) -> double
                if a.dbl && bb.dbl {
                    if i <= j { push(sse_add(&a.v, &bb.v), true, &mut seen, &mut fresh, &mut found); }
                    push(sse_sub(&a.v, &bb.v), true, &mut seen, &mut fresh, &mut found);
                    if i <= j { push(sse_mul(&a.v, &bb.v), true, &mut seen, &mut fresh, &mut found); }
                    push(sse_div(&a.v, &bb.v), true, &mut seen, &mut fresh, &mut found);
                }
            }
        }
        // unary: spill (->double)
        for a in &cur {
            if !a.dbl { push(a.v.spill_rn(), true, &mut seen, &mut fresh, &mut found); }
        }
        let n_new = fresh.len();
        nodes.extend(fresh);
        println!("level {level}: +{n_new} distinct  (bank total {})  found_em={}", nodes.len(), found);
        if found { println!(">>> em vector REACHED at level {level}"); break; }
    }
}
