//! PQR R[1]+1 vs 0x5005 DIRECT overlap and 6 leftover-low. Not an identity.
...
fn main() {
    ...
    let mut only_r_u1c = 0usize;
    let mut gain = Vec::new();
    let mut lose = Vec::new();
    for r in &tail {
        let t = f64::from_bits(r.qbits);
        let dc = ulp_distance(qw(r.z, cephes(r.z)), t).unwrap_or(99);
    ...
    // fix double-count: I incremented only_r_u1c twice. recompute cleanly below.
    println!(
        "DIRECT z>=4 both={both} only_5005={only_c} only_R1={only_r} (5005 ulp1={only_r_u1c}) union={} n={}",
        both + only_c + only_r,
        tail.len()
    );
    print!("GAIN (R1 not 5005) first");
    for z in &gain {
        print!(" {z:.6}");
    }
    println!();
    print!("LOSE (5005 not R1) first");
    for z in &lose {
        print!(" {z:.6}");
    }
    println!();
    ...
    for &lz in &SIX {
        ...
        println!("  z={lz:.16} 5005={dc}{sc} R1={dr}{sr}");
    }
    let mut qe_c = 0usize;
    let mut qe_r = 0usize;
    for row in rows.iter().filter(|rr| rr.z >= 4.0) {
        let t = f64::from_bits(row.qbits);
        if ulp_distance(qw(row.z, cephes(row.z)), t).unwrap_or(99) == 0 {
            qe_c += 1;
        }
        if ulp_distance(qw(row.z, pqr_r1(row.z)), t).unwrap_or(99) == 0 {
            qe_r += 1;
        }
    }
    println!("Q z>=4 5005={qe_c} R1={qe_r}");
}
