//! Exhaustive 7-bit Cephes Horner stores DIRECT z>=4. Not an identity.
...
fn main() {
    ...
    println!(
        "DIRECT z>=4 n={n} unmask={} 0x4e05={} 0x24a5={}",
        score(0),
        score(0x4e05),
        score(0x24a5)
    );
    println!("0x4e05 ∪ extra / 0x24a5 ∪ extra:");
    for &(base, bname) in &[(0x4e05u32, "0x4e05"), (0x24a5u32, "0x24a5")] {
    ...
    println!("top 12 7-bit:");
    for (s, mask) in ranked.iter().take(12) {
        println!("  fused={s} mask={mask:#x}");
    }
    let best = ranked[0].0;
    let nbest = ranked.iter().filter(|t| t.0 == best).count();
    let nge60 = ranked.iter().filter(|t| t.0 >= 60).count();
    let ngt60 = ranked.iter().filter(|t| t.0 > 60).count();
    println!(
        "best 7-bit={best} mask={:#x} unique={} nbest={nbest} ge60={nge60} gt60={ngt60} n7={}",
        ranked[0].1,
        nbest == 1,
        ranked.len()
    );
    println!("all fused>60:");
    for (s, mask) in ranked.iter().filter(|t| t.0 > 60) {
        println!("  fused={s} mask={mask:#x}");
    }
    println!("all fused==60 7-bit:");
    for (s, mask) in ranked.iter().filter(|t| t.0 == 60) {
        println!("  fused={s} mask={mask:#x}");
    }
}
