...
fn tf(x:&Ext80)->f64{ ext_to_f64(x,CW_PC64_RN) }
...
fn ln1p_ext(r:f64, var:u8, cw:u16)->Ext80{
    match var{
        0 => if r.abs()<0.292893218813452 { ext_fyl2xp1(&ext_ln2(),&ef(r),cw) }
             else { ext_fyl2x(&ext_ln2(),&ext_add(&ext_one(),&ef(r),cw),cw) },
...
fn log2_1p_ext(r:f64, var:u8, cw:u16)->Ext80{
    match var{
        0 => if r.abs()<0.292893218813452 { ext_fyl2xp1(&ext_one(),&ef(r),cw) }
             else { ext_fyl2x(&ext_one(),&ext_add(&ext_one(),&ef(r),cw),cw) },
...
#[derive(Clone,Copy)]
struct Cfg{ lnvar:u8, cw_ln:u16, cw_mul:u16, cw_exp:u16, cw_corr:u16, form:u8, lndel:u8, assoc:u8, fin:u16 }
...
fn em(r:f64,n:f64,c:Cfg)->f64{
    ...
    let em = match c.form{
        0 => { let u=fexp(&tau,c.cw_exp); ext_sub(&u,&ext_one(),c.cw_exp) }
        1 => { // 2^t - 1, t = tau*log2e
            let t=ext_mul(&tau,&ext_l2e(),c.cw_exp);
            if tf(&t).abs()<=1.0 { ext_f2xm1(&t,c.cw_exp) }
            else { let k=ext_rndint(&t,c.cw_exp); let f=ext_sub(&t,&k,c.cw_exp);
                   let w=ext_f2xm1(&f,c.cw_exp);
                   ext_sub(&ext_scale(&ext_add(&w,&ext_one(),c.cw_exp),&k,c.cw_exp),&ext_one(),c.cw_exp) }
        }
        2 => { // base-2 native: y=-n*log2(1+r)
            let y=ext_chs(&ext_mul(&ef(n),&log2_1p_ext(r,c.lnvar,c.cw_ln),c.cw_mul),c.cw_mul);
            if tf(&y).abs()<=1.0 { ext_f2xm1(&y,c.cw_exp) }
            else { let k=ext_rndint(&y,c.cw_exp); let f=ext_sub(&y,&k,c.cw_exp);
                   let w=ext_f2xm1(&f,c.cw_exp);
                   ext_sub(&ext_scale(&ext_add(&w,&ext_one(),c.cw_exp),&k,c.cw_exp),&ext_one(),c.cw_exp) }
        }
        _ => { let u=fexp(&tau,c.cw_exp); let b=ext_sub(&u,&ext_one(),c.cw_corr);
               let l=if c.lndel==0 { ext_fyl2x(&ext_ln2(),&u,c.cw_corr) }
                     else { ext_fyl2xp1(&ext_ln2(),&ext_sub(&u,&ext_one(),c.cw_corr),c.cw_corr) };
               match c.assoc{ 0=>ext_div(&ext_mul(&b,&tau,c.cw_corr),&l,c.cw_corr),
...
fn pin_gen(rows:&[(f64,u64)], center:f64)->Option<f64>{
    ...
    for d in -20..=20i64{ let e=f64::from_bits((cb+d) as u64); if e>=0.0{continue}
        if rows.iter().all(|(pv,want)|((pv/e)*(rows[0].0*0.0+1.0)).to_bits()==*want || true){ } }
    None
}
...
fn main(){
    ...
    for ci in 0..ncfg{
        ...
        for j in 0..128{ let w=&wl[ci*128+j];
        ...
        let cb=center.to_bits() as i64; let mut found=None;
        for d in -20..=20i64{ let e=f64::from_bits((cb+d) as u64); if e>=0.0{continue}
            if rows.iter().all(|(pv,want)|((pv/e)*r).to_bits()==*want){ found=Some(e); break; } }
        if let Some(e)=found{ pins.push((r,n,e,tau0.to_bits())); }
    }
    let _=pin_gen;
    let ntot=pins.len();
    let kbase=pins.iter().filter(|(r,n,ep,_)| rx::excel_expm1_internal(-(n*rx::excel_log1p(*r))).to_bits()==ep.to_bits()).count();
    println!("collision pinned: {}   double-Kahan baseline: {}/{}", ntot, kbase, ntot);
    ...
    for lnvar in 0u8..3{
      for (_lnn,cw_ln) in pcs{ for (_mn,cw_mul) in pcs{ for (_en,cw_exp) in pcs{
        for form in 0u8..4{
          let corrspace:&[u16]= if form==3 {&[CW_PC53_RN,CW_PC64_RN]} else {&[CW_PC64_RN]};
          for &cw_corr in corrspace{
            let (lnspace,assocspace):(&[u8],&[u8]) = if form==3 {(&[0,1],&[0,1,2])} else {(&[0],&[0])};
            for &lndel in lnspace{ for &assoc in assocspace{
              for (_fnn,fin) in fins{
                let c=Cfg{lnvar,cw_ln,cw_mul,cw_exp,cw_corr,form,lndel,assoc,fin};
                let ok=pins.iter().filter(|(r,n,ep,_)| em(*r,*n,c).to_bits()==ep.to_bits()).count() as u32;
                let desc=format!("lnvar={} cwln={} cwmul={} cwexp={} form={} corr={:04x} ln={} assoc={} fin={}",
                    lnvar, if cw_ln==CW_PC53_RN{"53"}else{"64"}, if cw_mul==CW_PC53_RN{"53"}else{"64"},
                    if cw_exp==CW_PC53_RN{"53"}else{"64"}, form, cw_corr, lndel, assoc,
    ...
    for (ok,d,_) in best.iter().take(12){ println!("  {:>3}/{}  {}", ok, ntot, d); }
    ...
    let ext_best=best.iter().filter(|(_,_,c)|
        c.lnvar<2 && c.cw_ln==CW_PC64_RN && c.cw_mul==CW_PC64_RN && c.cw_exp==CW_PC64_RN
        && (c.form!=3 || c.cw_corr==CW_PC64_RN)).max_by_key(|x|x.0);
    if let Some((ok,d,_))=ext_best{ println!("BEST purely-EXTENDED (all PC64) config: {}/{}  {}", ok, ntot, d); }

    // ---- per-group residual dump for the single best config ----
    let (bok,bd,bc)=best[0].clone();
    println!("\n=== per-group residual (em_cand - em_pinned, ulps) for BEST: {}/{}  [{}] ===", bok,ntot,bd);
    use std::collections::BTreeMap;
    let mut groups:BTreeMap<u64,Vec<(f64,f64,f64)>>=BTreeMap::new();
    for (r,n,ep,tb) in &pins{ groups.entry(*tb).or_default().push((*r,*n,*ep)); }
    fn sord(u:u64)->i64{ if u<1<<63{u as i64}else{-((u^(1u64<<63))as i64)} }
    let mut shown=0;
    for (tb,lst) in &groups{ if shown>=6{break} shown+=1;
        let mut hit=0;
        let mut line=format!("group tau0={:016x} ({}): ", tb, lst.len());
        for (r,n,ep) in lst{ let g=em(*r,*n,bc); let d=sord(g.to_bits())-sord(ep.to_bits());
            if d==0{hit+=1;} line.push_str(&format!("{:+} ",d)); }
        println!("  {}  [{}/{}]", line, hit, lst.len());
    }
    // Does the BEST purely-extended config reproduce the DISTINCT-em structure of the collision groups?
    if let Some((_,exd,exc))=best.iter().filter(|(_,_,c)|
        c.lnvar<2 && c.cw_ln==CW_PC64_RN && c.cw_mul==CW_PC64_RN && c.cw_exp==CW_PC64_RN
        && (c.form!=3 || c.cw_corr==CW_PC64_RN)).max_by_key(|x|x.0){
        println!("\n=== distinct-em reproduction by BEST-EXTENDED [{}] ===", exd);
        println!("  (Excel distinct em / config-count  vs  candidate distinct em)");
        let mut shown=0;
        for (tb,lst) in &groups{ if shown>=8{break} shown+=1;
            use std::collections::BTreeSet;
            let exset:BTreeSet<u64>=lst.iter().map(|(_,_,ep)|ep.to_bits()).collect();
            let cnset:BTreeSet<u64>=lst.iter().map(|(r,n,_)|em(*r,*n,*exc).to_bits()).collect();
            let hit=lst.iter().filter(|(r,n,ep)|em(*r,*n,*exc).to_bits()==ep.to_bits()).count();
            println!("  group {:016x} ({:2}): Excel {} distinct, cand {} distinct, exact {}/{}",
                tb, lst.len(), exset.len(), cnset.len(), hit, lst.len());
        }
    }
}
