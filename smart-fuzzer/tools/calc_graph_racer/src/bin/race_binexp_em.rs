...
use rx::{
    CW_PC53_RN, CW_PC64_RN, Ext80, ext_div, ext_from_f64, ext_mul, ext_one, ext_sub, ext_to_f64,
};
...
fn a_recip_of_pow(opr: f64, n: u32) -> f64 {
    let p = binexp_d(opr, n);
    (1.0 / p) - 1.0
}
// B: b=1/(1+r) double; P=b^n double binexp; em=P-1 double
fn b_pow_of_recip(opr: f64, n: u32) -> f64 {
    let b = 1.0 / opr;
    binexp_d(b, n) - 1.0
}
...
fn f_pow_d_recip_ext(opr: f64, n: u32) -> f64 {
    let p = binexp_d(opr, n);
    let v = ext_to_f64(
        &ext_div(&ext_one(), &ext_from_f64(p), CW_PC64_RN),
        CW_PC53_RN,
    );
    v - 1.0
}
...
fn main() {
    ...
    println!(
        "=== em = (1+r)^-n - 1 candidate race, N={} (model-free pinned oracle) ===",
        tot
    );
    ...
    println!(
        "\n=== signed ULP error histogram for [{}] ===",
        cands[best].0
    );
    ...
    for (k, n, pin, got) in &rows {
        if got[best] != *pin {
            ...
            if nm <= 30 {
                println!(
                    "  k={:3} n={:3}  err={:+} pin={:016x} got={:016x}",
                    k,
                    n,
                    ulp_err(got[best], *pin),
                    pin,
                    got[best]
                );
            }
