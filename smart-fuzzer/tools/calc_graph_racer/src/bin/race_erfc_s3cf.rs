//! 3 stubborn leftover vs NSWC/CF named F. Not an identity.
...
fn main() {
    ...
    let graphs: [(&str, Box<dyn Fn(f64) -> f64>); 8] = [
        ...
        ("as714", Box::new(|z| qw(z, f::cf_as714_f(z)))),
        ("gaut", Box::new(|z| qw(z, f::cf_gautschi_f(z)))),
        ("lentz12", Box::new(|z| qw(z, f::cf_lentz_as714_n(z, 12)))),
        ("lentz24", Box::new(|z| qw(z, f::cf_lentz_as714_n(z, 24)))),
    ];
    println!("3 stubborn NSWC/CF:");
    for &lz in &STUB {
    ...
    for (name, ev) in &graphs {
        ...
        for &lz in &STUB {
            ...
            let g = ev(lz);
            if g.is_finite() && ulp_distance(g, f64::from_bits(r.qbits)).unwrap_or(99) == 0 {
                hit += 1;
            }
        }
