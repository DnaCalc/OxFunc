...
use rx::{
    ...
    ext_ln2, ext_mul, ext_one, ext_to_f64,
};
...
fn l1p_fyl_dbl(r: f64) -> f64 {
    if r.abs() < 0.292893218813452 {
        ext_to_f64(&ext_fyl2xp1(&ext_ln2(), &ef(r), CW_PC64_RN), CW_PC64_RN)
    } else {
        ext_to_f64(
            &ext_fyl2x(
                &ext_ln2(),
                &ext_add(&ext_one(), &ef(r), CW_PC64_RN),
                CW_PC64_RN,
            ),
            CW_PC64_RN,
        )
...
fn ln1p_ext(r: f64) -> Ext80 {
    if r.abs() < 0.292893218813452 {
        ext_fyl2xp1(&ext_ln2(), &ef(r), CW_PC64_RN)
    } else {
        ext_fyl2x(
            &ext_ln2(),
            &ext_add(&ext_one(), &ef(r), CW_PC64_RN),
...
fn tau_sse2(r: f64, n: f64, l1p: fn(f64) -> f64) -> f64 {
    -(n * l1p(r))
}
fn t_sse2_cr(r: f64, n: f64) -> f64 {
    tau_sse2(r, n, l1p_cr)
}
fn t_sse2_ucrt(r: f64, n: f64) -> f64 {
    tau_sse2(r, n, l1p_std)
}
fn t_sse2_fyl(r: f64, n: f64) -> f64 {
    tau_sse2(r, n, l1p_fyl_dbl)
}
...
fn t_x87dr_cr(r: f64, n: f64) -> f64 {
    tau_x87dr(r, n, l1p_cr)
}
fn t_x87dr_ucrt(r: f64, n: f64) -> f64 {
    tau_x87dr(r, n, l1p_std)
}
fn t_x87dr_fyldbl(r: f64, n: f64) -> f64 {
    tau_x87dr(r, n, l1p_fyl_dbl)
}
...
fn t_x87pc53_cr(r: f64, n: f64) -> f64 {
    tau_x87_pc53(r, n, l1p_cr)
}
fn t_x87pc53_ucrt(r: f64, n: f64) -> f64 {
    tau_x87_pc53(r, n, l1p_std)
}
...
fn main() {
    let cands: [(&str, TauFn); 10] = [
        ("sse2_cr", t_sse2_cr),
        ("sse2_ucrt", t_sse2_ucrt),
        ("sse2_fyl", t_sse2_fyl),
        ("x87dr_cr", t_x87dr_cr),
        ("x87dr_ucrt", t_x87dr_ucrt),
        ("x87dr_fyldbl", t_x87dr_fyldbl),
        ("x87pc53_cr", t_x87pc53_cr),
        ("x87pc53_ucrt", t_x87pc53_ucrt),
        ("x87extfyl", t_x87_extfyl),
        ("x87extfyl_dr", t_x87_extfyl_dr),
    ];
    // pin em on collide
    ...
    for ((rb, nb), rows) in &data {
        ...
        if let Some(e) = pin_gen(rows, r, km) {
            pins.push((r, n, e.to_bits()));
        }
    }
    println!("collide pinned: {}", pins.len());
    println!(
        "{:<14} {:>7} {:>8} {:>9} {:>9}",
        "tau-cand", "groups", "conflG", "conflCfg", "emRepro"
    );
    for (nm, tf) in cands {
        ...
        for (r, n, ep) in &pins {
            ...
            if rx::excel_expm1_internal(tau).to_bits() == *ep {
                repro += 1;
            }
        }
        let mut cg = 0;
