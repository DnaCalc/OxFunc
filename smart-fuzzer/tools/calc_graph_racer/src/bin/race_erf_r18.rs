//! 18 joint ulp≥2 that miss signed 1+w last-store vs named F. Not an identity.
...
fn main() {
    ...
    println!("remain ulp>=2 miss signed 1+w last-store:");
    let mut n = 0usize;
    let mut nlo = 0usize;
    for &(z, bits) in &rows {
        ...
        let s = lead(z, &joint);
        let signed_hit = if g < t {
            ulp_distance(z * s.next_up(), t).unwrap_or(99) == 0
        } else {
            ulp_distance(z * s.next_down(), t).unwrap_or(99) == 0
        };
        if signed_hit {
            continue;
        }
        n += 1;
        let dir = if g < t { "LOW" } else { "HIGH" };
        if g < t {
            nlo += 1;
        }
        let dl = ulp_distance(libm::erf(z), t).unwrap_or(99);
        let dab = ulp_distance(cody_ab(z), t).unwrap_or(99);
        let d1 = ulp_distance(1.0 - libm::erfc(z), t).unwrap_or(99);
        println!(
            "  z={z:.16} kind={} j={d}{dir} libm={dl} AB={dab} 1-erfc={d1}",
            kind(z)
        );
    }
    println!("n={n} LOW={nlo} HIGH={}", n - nlo);
}
