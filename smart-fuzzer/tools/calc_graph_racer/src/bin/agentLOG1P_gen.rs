...
fn h(x:f64)->String{ format!("0x{:016x}", x.to_bits()) }

fn main(){
    let mut probes=String::new();
    let mut idx=0u32;
    let mut push=|probes:&mut String, id:String, r:f64, n:f64, pv:f64|{
        if !probes.is_empty(){ probes.push(','); }
    ...
    let mut disc:Vec<f64>=Vec::new(); let mut ctl:Vec<f64>=Vec::new();
    ...
    while disc.len()<44 || ctl.len()<6 || fyl_diff.len()<4 {
        ...
        let cr=l1p_cr(r); let sd=l1p_std(r); let fy=l1p_fyl(r);
        if fy.to_bits()!=cr.to_bits() && fyl_diff.len()<4 { fyl_diff.push(r); continue; }
        if sd.to_bits()!=cr.to_bits() { if disc.len()<44 { disc.push(r);} }
        else if ctl.len()<6 { ctl.push(r); }
        if seed==0 { seed=1; }
    }
    let mut all:Vec<(f64,&str)>=Vec::new();
    for r in &disc { all.push((*r,"disc")); }
    for r in &ctl  { all.push((*r,"ctl")); }
    for r in &fyl_diff { all.push((*r,"fyldiff")); }
    ...
    for (r,cls) in &all{
        ...
        let mut j=0u32; while (2f64.powi(j as i32) * cr.abs()) < 1.0 { j+=1; }
        let n_clean=2f64.powi(j as i32);
        for (tag,n) in [("n1",1.0),("n2",2.0),("nc",n_clean)]{
            for k in 0..128u64{
                let pv=f64::from_bits(base + k);
                push(&mut probes, format!("l1p-{}-{}-{:03}",cls,tag,idx), *r, n, pv);
                idx+=1;
            }
        }
        let sd_ulp=l1p_std(*r).to_bits() as i64 - cr.to_bits() as i64;
        let fy_ulp=l1p_fyl(*r).to_bits() as i64 - cr.to_bits() as i64;
        meta.push_str(&format!("{},{},1,2,{},0x{:016x},{},{}\n", h(*r), cls, n_clean as u64, cr.to_bits(), sd_ulp, fy_ulp));
    }
    let batch=format!("{{\"function\":\"PMT\",\"probes\":[{}]}}", probes);
    std::fs::write("../../work/w109/G6-solvers/batch-log1p-lane.json", &batch).unwrap();
    std::fs::write("../../work/w109/G6-solvers/batch-log1p-lane-meta.csv", &meta).unwrap();
    println!("wrote batch-log1p-lane.json : {} probes ({} r-values: {} disc, {} ctl, {} fyldiff; x3 n x 128 pv)",
        idx, all.len(), disc.len(), ctl.len(), fyl_diff.len());
    println!("meta csv rows (r, class, n1/n2/n_clean, CR-log1p bits, std-CR ulp, fyl-CR ulp) written.");
}
