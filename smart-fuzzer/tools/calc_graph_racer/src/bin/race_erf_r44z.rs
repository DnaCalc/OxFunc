//! z-cut joint vs unpoked-z+1. Not an identity.
...
fn main() {
    ...
    let rows: Vec<_> = map
        ...
        .collect();
    let ja = joint_a();
    let mut only_z: Vec<f64> = Vec::new();
    for &(z, bits) in &rows {
    ...
    print!("only_z1 n={} first", only_z.len());
    for z in only_z.iter().take(12) {
        print!(" {z:.6}");
    }
    println!(" last {:.6}", only_z.last().copied().unwrap_or(0.0));
    println!("z-cut joint-then-z+1 / z+1-then-joint:");
    let cuts = [
        1e-8, 1e-6, 1e-4, 0.001, 0.01, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45,
    ];
    ...
    for &c in &cuts {
        ...
        println!("  cut={c} j-then-z1={a} z1-then-j={b}");
    }
    println!("best j-then-z1={best_a} @{best_ac} z1-then-j={best_b} @{best_bc} (bar 866)");
}
