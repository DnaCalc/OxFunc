...
fn main(){
    ...
    for ci in 0..ncfg{
        ...
        let cb=center.to_bits() as i64; let mut cnt=0;
        for d in -30..=30i64{ let e=f64::from_bits((cb+d) as u64); if e>=0.0{continue}
            if rows.iter().all(|(pv,want)|((pv/e)*r).to_bits()==*want){ cnt+=1; } }
        if cnt>0{ pinned+=1; *widths.entry(cnt).or_default()+=1; }
    }
    println!("configs with a valid pin: {}", pinned);
    println!("pin-window multiplicity histogram (#em satisfying all 128 : #configs): {:?}", widths);
}
