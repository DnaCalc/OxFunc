//! Extra ±1 from PQR R[1]+1 (hits 5.333) vs 3 stubborn. Not an identity.
...
fn main() {
    ...
    let mut r1 = R0;
    r1[1] = poke(R0[1], 1);
    let (qe0, qd0, h0, _) = sc(&P0, &Q0, &r1);
    println!("R[1]+1 Q={qe0} d={qd0} hit={h0}/3 (bar PQR 680/42)");
    println!("extra ±1 (print hit>h0 or d>=qd0 or Q>=qe0):");
    let mut best_h = h0;
    let mut best_d = qd0;
    let mut lab = "R[1]+1".to_string();
    for i in 0..8 {
        for k in [-1i32, 1] {
            let mut p = P0;
            p[i] = poke(P0[i], k);
            let (qe, qd, h, zs) = sc(&p, &Q0, &r1);
            ...
            if h > best_h || (h == best_h && qd > best_d) {
                best_h = h;
                best_d = qd;
                lab = format!("R[1]+1 P[{i}] {k:+}");
            }
        }
        for k in [-1i32, 1] {
            let mut q = Q0;
            q[i] = poke(Q0[i], k);
            let (qe, qd, h, zs) = sc(&P0, &q, &r1);
            if h > h0 || qd >= qd0 || qe >= qe0 {
