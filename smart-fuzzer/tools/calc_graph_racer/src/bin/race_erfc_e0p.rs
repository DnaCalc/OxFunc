//! Extra ±1 from ccdd E0+1 vs 6 leftover-low. Not an identity.
...
fn main() {
    ...
    let e0p = poke(E0, 1);
    let (qe0, qd0, h0, _) = sc(&CC0, &DD0, e0p, E1, E2, E3);
    println!("E0+1 Q={qe0} d={qd0} hit={h0}/6 (bar ccdd 1466/46)");
    println!("extra ±1 (print hit>h0 or d>=qd0 or Q>=qe0):");
    let mut best_h = h0;
    let mut best_d = qd0;
    let mut lab = "E0+1".to_string();
    for i in 0..9 {
        for k in [-1i32, 1] {
            let mut c = CC0;
            c[i] = poke(CC0[i], k);
            let (qe, qd, h, zs) = sc(&c, &DD0, e0p, E1, E2, E3);
    ...
    for i in 1..10 {
        for k in [-1i32, 1] {
            let mut d = DD0;
            d[i] = poke(DD0[i], k);
            let (qe, qd, h, zs) = sc(&CC0, &d, e0p, E1, E2, E3);
            if h > h0 || qd >= qd0 || qe >= qe0 {
            ...
            if h > best_h || (h == best_h && qd > best_d) {
                best_h = h;
                best_d = qd;
                lab = format!("E0+1 D[{i}] {k:+}");
            }
        }
    }
    let es: [(&str, f64, f64, f64, f64); 6] = [
        ("E1+1", e0p, poke(E1, 1), E2, E3),
        ("E1-1", e0p, poke(E1, -1), E2, E3),
        ("E2+1", e0p, E1, poke(E2, 1), E3),
        ("E2-1", e0p, E1, poke(E2, -1), E3),
        ("E3+1", e0p, E1, E2, poke(E3, 1)),
        ("E3-1", e0p, E1, E2, poke(E3, -1)),
    ];
