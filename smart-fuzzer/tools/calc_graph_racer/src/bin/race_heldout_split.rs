...
fn main() {
    ...
    let (mut lc, mut ls) = (0u32,0u32); let mut ln=0u32;   // |tau|<1
    let (mut bc, mut bs) = (0u32,0u32); let mut bn=0u32;   // |tau|>=1
    for w in &ws.witnesses {
        ...
        if small { ln+=1; if cok{lc+=1;} if sok{ls+=1;} } else { bn+=1; if cok{bc+=1;} if sok{bs+=1;} }
    }
    println!("heldout |tau|<1  ({} rows, CONFOUNDED): CR {}/{}  std {}/{}   std-CR = {:+}", ln,lc,ln,ls,ln, ls as i32-lc as i32);
    println!("heldout |tau|>=1 ({} rows, CLEAN):      CR {}/{}  std {}/{}   std-CR = {:+}", bn,bc,bn,bs,bn, bs as i32-bc as i32);
    println!("TOTAL: CR {}/{}  std {}/{}", lc+bc, ln+bn, ls+bs, ln+bn);
}
