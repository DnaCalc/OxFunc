...
fn main() {
    ...
    let variants: Vec<(&str, Box<dyn Fn(f64) -> f64>)> = vec![
        (
            "plain u-1 (f64 exp)",
            Box::new(|t: f64| rx::excel_exp(t) - 1.0),
        ),
        (
            "plain RN53(u_ext-1)",
            Box::new(|t: f64| tf(&rx::ext_sub(&exp_ext(t), &rx::ext_one(), CW))),
        ),
        (
            "Kahan (ref)",
            Box::new(|t: f64| {
    ...
    for (name, f) in &variants {
        ...
        for (r, n, em) in &rows {
            ...
            if d == 0 {
                ...
                allmiss.push(((r.log2().round()) as i32, *n, d));
            }
        ...
        println!();
        if *name != "Kahan (ref)" && ok >= 38 {
            print!("      all misses:");
            for (k, n, d) in &allmiss {
                print!(" (2^{},{}):{:+}", k, n, d);
            }
            println!();
        }
