//! 24a5 last-store vs 3 stubborn leftover-low. Not an identity.
...
fn main() {
    ...
    let graphs: [(&str, Box<dyn Fn(f64) -> f64>); 5] = [
        ("24a5", Box::new(|z| qw(z, cephes(z)))),
        ("up", Box::new(|z| qw(z, cephes(z)).next_up())),
        ("up2", Box::new(|z| qw(z, cephes(z)).next_up().next_up())),
        (
            "up3",
            Box::new(|z| qw(z, cephes(z)).next_up().next_up().next_up()),
        ),
        ("upF", Box::new(|z| qw(z, cephes(z).next_up()))),
    ];
    println!("3 stubborn 24a5 last-store:");
    for &lz in &STUB {
    ...
    for (name, ev) in &graphs {
        ...
        for row in rows.iter().filter(|rr| rr.z >= 4.0) {
            if ulp_distance(ev(row.z), f64::from_bits(row.qbits)).unwrap_or(99) == 0 {
                qe += 1;
                if row.direct {
                    qd += 1;
                }
        ...
        println!("{name:6} hit={hit}/3 Q={qe} d={qd}");
    }
}
