//! P-side 0x555 mantissa census vs joint. Not an identity.
...
fn main() {
    ...
    let mut n555 = 0usize;
    let mut nall = rows.len();
    let mut ex555 = 0usize;
    println!("0x555 rows z, 3z, joint ulp, dir:");
    for &(z, bits) in &rows {
        if !is555(z) {
            continue;
        }
        ...
        println!(
            "  z={z:.16} zbits={:#x} 3z={:.16} j={d}{dir}",
            z.to_bits(),
            3.0 * z
        );
    }
    println!(
        "n555={n555}/{nall} exact={ex555} ulp0..5+ {} {} {} {} {} {}",
    ...
    for &(z, bits) in &rows {
        ...
        hex += 1;
        if ulp_distance(erf_a(z, &joint), f64::from_bits(bits)).unwrap_or(99) == 0 {
            eex += 1;
        }
    }
    println!("non-555 n={hex} exact={eex}  555 exact rate {ex555}/{n555} vs non {eex}/{hex}");
}
