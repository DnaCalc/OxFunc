//! P-side as 1−(w·C/D) and joint-then-complement cuts. Not an identity.
...
fn main() {
    ...
    println!("P-side rows={} z>=0.46875 n={n_hi} (bar joint 866)", rows.len());
    for (name, ev) in [
        ("joint A21", Box::new(|z: f64| erf_a(z, &joint)) as Box<dyn Fn(f64) -> f64>),
        ...
        println!("{name:14} {ex}/{} max={mx} in[0.46875,0.5) {hi}/{n_hi}", rows.len());
    }
    println!("joint-then-1-(w*CR) cuts:");
    for &c in &[0.375, 0.4, 0.45, 0.46875, 0.48] {
        let mut ex = 0usize;
        let mut mx = 0u64;
        for &(z, bits) in &rows {
            let p = if z < c {
                ...
                one_minus_q(z, cody(z, &C0, &D0, 0))
            };
            let d = ulp_distance(p, f64::from_bits(bits)).unwrap_or(99);
            if d == 0 {
                ex += 1;
            } else {
                mx = mx.max(d);
        ...
        println!("  cut={c:.5} {ex}/{} max={mx} (bar 866)", rows.len());
    }
}
