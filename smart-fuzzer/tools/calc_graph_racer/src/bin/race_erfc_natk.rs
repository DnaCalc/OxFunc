//! native vs x87 F ulp vs last-store k on 0x210 leftover-low tmu. Not an identity.
...
fn cody_nat(y: f64) -> f64 {
    let ye = y.abs();
    let mut xnum = C0[8] * ye;
    let mut xden = ye;
    for i in 0..7 {
        xnum = (xnum + C0[i]) * ye;
        xden = (xden + D0[i]) * ye;
...
fn main() {
    ...
    println!("native vs x87 Cody F ulp vs last-store k on 0x210 leftover-low tmu:");
    for r in rows.iter().filter(|rr| rr.direct && rr.z >= 0.5 && rr.z < 4.0) {
        ...
        println!(
            "  z={:.16} k={k:?} native-x87_F={du} match={m} Q_nat={} Q_x87={}",
            z,
            ulp_distance(mul(w, fnat), t).unwrap_or(99),
            ulp_distance(mul(w, fx), t).unwrap_or(99)
        );
    }
    println!("0x210 leftover-low tmu n={ntmu}/{nlo} native-x87 F ulp matches k={nmatch}/{ntmu}");
}
