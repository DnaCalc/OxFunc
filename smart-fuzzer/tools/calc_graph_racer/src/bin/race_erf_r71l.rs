//! remaining 71 1-ULP vs libm erf / 1-erfc. Not an identity.
...
fn main() {
    ...
    let mut h_e = 0usize;
    let mut h_u = 0usize;
    println!("remaining 1-ULP vs libm:");
    for &(z, bits) in &rows {
        ...
        n += 1;
        let l = ulp_distance(libm::erf(z), t).unwrap_or(99) == 0;
        let e = ulp_distance(1.0 - libm::erfc(z), t).unwrap_or(99) == 0;
        if l {
            h_l += 1;
        }
        if e {
        ...
        if l || e {
            h_u += 1;
            println!("  HIT z={z:.16} libm={l} 1-erfc={e}");
        }
    }
    println!("n={n} libm={h_l} 1-erfc={h_e} union={h_u}");
}
