...
fn mul_spill(a: f64, b: f64) -> f64 {
    ext_to_f64(&ext_mul(&ext_from_f64(a), &ext_from_f64(b), CW_PC64_RN), RN53)
}
// fl53(fl64(a/b)) : x87 divide at PC64, spill to double
fn div_spill(a: f64, b: f64) -> f64 {
    ext_to_f64(&ext_div(&ext_from_f64(a), &ext_from_f64(b), CW_PC64_RN), RN53)
}
...
fn main() {
    ...
    for line in csv.lines().skip(1) {
        ...
        let s4 = ext_to_f64(&ext_div(&ext_from_f64(num_sp), &ln_ext(u), CW_PC64_RN), RN53).to_bits();
        ...
        for i in 0..8 { if cands[i] == pin { score[i] += 1; } }
        if cands[best_idx_track] != pin { miss7.push((k, n, (cands[best_idx_track] as i64) - (pin as i64))); }
        tot += 1;
    }
    println!("=== SPILL-LOOP Kahan lattice (Fable H1), N={} ===", tot);
    for i in 0..8 {
        println!("  {:34} {:3}/{}  ({:.1}%)", labels[i], score[i], tot, 100.0 * score[i] as f64 / tot as f64);
    }
    println!("\nS2 (num spill+div spill) misses: {} ; dirs +{}/-{}",
             miss7.len(), miss7.iter().filter(|m| m.2 > 0).count(), miss7.iter().filter(|m| m.2 < 0).count());
    for m in miss7.iter().take(15) { println!("    k={:3} n={:3} d={:+}", m.0, m.1, m.2); }
}
