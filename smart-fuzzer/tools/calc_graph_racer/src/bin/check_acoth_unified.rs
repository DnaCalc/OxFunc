...
fn pair_recip_bin64(x: f64) -> f64 { // t = 1/x in binary64 (agent baseline)
    let t = 1.0 / x;
    let ln2 = rx::ext_ln2();
...
fn pair_recip_x87(x: f64) -> f64 { // t = RN53(RN64(1/x)) — Excel's POWER x87 reciprocal
...
fn pair_recip_extkeep(x: f64) -> f64 { // 1/x in x87 extended, kept extended (never stored)
    let ln2 = rx::ext_ln2();
...
fn main() {
    ...
    for &t in &[1.2f64, 1.5, 2.0, 3.0, 5.0, 10.0, 100.0, 1e3] {
        let mut ok = 0u32;
        for (x, want) in &rows {
            let v = if x.abs() < t { ratio_x87(*x) } else { uni_x87_abs(*x) };
    ...
    for &t1 in &cuts {
        for &t2 in &cuts {
            ...
            for (x, want) in &rows {
                let a = x.abs();
                let v = if a < t1 { ratio_x87(*x) } else if a < t2 { uni_x87_abs(*x) } else { pair_1overx(*x) };
                if v.to_bits() == *want { ok += 1; }
            }
            if ok > best3.0 { best3 = (ok, t1, t2); }
        }
    }
    println!("BEST 3-way: {}/{} at T1={:.4} T2={:.4}", best3.0, total, best3.1, best3.2);
    println!("\n--- misses under best 3-way (T1={:.4} T2={:.4}) ---", best3.1, best3.2);
    for (x, want) in &sorted {
        let a = x.abs();
        let v = if a < best3.1 { ratio_x87(*x) } else if a < best3.2 { uni_x87_abs(*x) } else { pair_1overx(*x) };
        if v.to_bits() != *want {
            let d = v.to_bits() as i64 - *want as i64;
            println!("   x={:+.8e}  |x|={:.8e}  got-want {:+} ulp", x, x.abs(), d);
        }
    ...
    for (x, want) in &sorted {
        if x.abs() < 3.0 { continue; }
        let pe = pair_1overx(*x).to_bits() == *want;
        println!("   x={:+.8e}  |x|={:.8e}  pair_1overx {}", x, x.abs(), if pe { "OK " } else { "..." });
    }
    ...
    println!("ratio_x87 total {}/{}   ratio_excel_ln total {}/{}",
        score_all(&rows, &ratio_x87), total, score_all(&rows, &ratio_excel_ln), total);
    ...
    for (name, f) in pairs {
        ...
        for (i, (ro, rn)) in r.iter().enumerate() {
            let (lo, hi) = REGIONS[i];
            print!("[{lo:.0e},{:.0e}):{ro}/{rn} ", if hi.is_finite() { hi } else { f64::MAX });
        }
        println!();
    }
    ...
    for (pname, pf) in pairs {
        for &t in &[2.0f64, 2.5, 3.0, 3.5, 4.0, 5.0] {
            ...
            if ok > best2.0 { best2 = (ok, t, pname); }
        }
    }
    println!("BEST 2-way: {}/{} at T={:.2} with {}", best2.0, total, best2.1, best2.2.trim());
    ...
    for (x, want) in &sorted {
        let v = if x.abs() < best2.1 { ratio_x87(*x) } else { bpf(*x) };
        if v.to_bits() != *want {
            println!("   x={:+.8e}  |x|={:.6e}  got-want {:+} ulp", x, x.abs(), v.to_bits() as i64 - *want as i64);
        }
    }
    ...
    let ratio_abs = |a: f64| { let r = (a + 1.0) / (a - 1.0); tof(&rx::ext_mul(&rx::ext_fyl2x(&rx::ext_ln2(), &e(r), CW), &e(0.5), CW)) };
    let pair_abs = |a: f64| { let ln2 = rx::ext_ln2(); let t = 1.0 / a; let p = rx::ext_fyl2xp1(&ln2, &e(t), CW); let m = rx::ext_fyl2xp1(&ln2, &e(-t), CW); tof(&rx::ext_mul(&rx::ext_sub(&p, &m, CW), &e(0.5), CW)) };
    ...
    for &t in &[2.5f64, 3.0, 3.5, 4.0, 5.0, 6.0, 8.0, 10.0, 100.0] {
        ...
        for (x, want) in &rows {
            ...
            if m.copysign(*x).to_bits() == *want { ok += 1; }
        }
        println!("  T={t:.1}: {ok}/{total}");
        if ok > bestodd.0 { bestodd = (ok, t); }
    }
    ...
    for (x, want) in &sorted {
        ...
        let v = m.copysign(*x);
        if v.to_bits() != *want {
            println!("   x={:+.8e}  |x|={:.6e}  got-want {:+} ulp", x, a, v.to_bits() as i64 - *want as i64);
        }
    }

    // ===== no-regression check: odd-form (T=3.5) vs current platform =====
    let oddform = |x: f64| { let a = x.abs(); let m = if a < 3.5 { ratio_abs(a) } else { pair_abs(a) }; m.copysign(x) };
