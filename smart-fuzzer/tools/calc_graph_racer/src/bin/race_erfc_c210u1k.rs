//! 1-ULP last-store k of unmask 0x210 vs 114. Not an identity.
...
fn main() {
    ...
    let mut hsh = [0usize; 9];
    let mut hconv = [0usize; 9];
    println!("1-ULP last-store k of unmask 0x210 vs 114:");
    for r in rows.iter().filter(|rr| rr.direct && rr.z >= 0.5 && rr.z < 4.0) {
        let t = f64::from_bits(r.qbits);
        let w = f::w_rn53(r.z);
        let d210 = ulp_distance(mul(w, cody(r.z, &C0, MASK)), t).unwrap_or(99);
        let d114 = ulp_distance(mul(w, cody(r.z, &c114, MASK)), t).unwrap_or(99);
        let k = k_of(r.z, t, cody(r.z, &C0, 0));
        let g = mul(w, cody(r.z, &C0, 0));
        let up = ulp_distance(g.next_up(), t).unwrap_or(99) == 0;
        let dn = ulp_distance(g.next_down(), t).unwrap_or(99) == 0;
        if d210 == 1 {
            n210 += 1;
            if let Some(kk) = k {
            ...
            if d114 == 0 {
                ...
                println!(
                    "  CONV z={:.16} unmask_k={k:?} last-mul up={up} dn={dn}",
                    r.z
                );
            }
        }
    ...
    print!("0x210 1-ULP n={n210} unmask |k|");
    for (i, c) in h210.iter().enumerate() {
        if *c > 0 {
            print!(" {i}:{c}");
        }
    }
    print!("\n114 1-ULP n={n114} unmask |k|");
    for (i, c) in h114.iter().enumerate() {
        if *c > 0 {
            print!(" {i}:{c}");
        }
    }
    print!("\nshared 1-ULP n={nsh} unmask |k|");
    for (i, c) in hsh.iter().enumerate() {
        if *c > 0 {
            print!(" {i}:{c}");
        }
    }
    print!("\nCONV n={nconv} unmask |k|");
    for (i, c) in hconv.iter().enumerate() {
        if *c > 0 {
            print!(" {i}:{c}");
        }
    }
