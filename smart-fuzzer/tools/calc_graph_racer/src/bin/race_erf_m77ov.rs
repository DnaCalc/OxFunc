//! P-side overlap of MATH77 tight-856 vs A21 joint-866. Not an identity.
...
fn main() {
    ...
    let mut mex = 0usize;
    let mut jex = 0usize;
    let mut band_m = [0usize; 5];
    let mut band_j = [0usize; 5];
    let mut band_n = [0usize; 5];
    for &(z, pbits) in &rows {
        let dm = ulp_distance(erf_m77_tight(z), f64::from_bits(pbits)).unwrap_or(99);
        let dj = ulp_distance(erf_a21j(z), f64::from_bits(pbits)).unwrap_or(99);
    ...
    println!(
        "n={} m77t={} a21j={} both={} only_m77={} only_a21={} neither={} union={}",
        rows.len(),
        mex,
        jex,
        both,
