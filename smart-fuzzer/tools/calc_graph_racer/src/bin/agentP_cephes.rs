...
const EP:[f64;3]=[1.2617719307481059087798E-4,3.0299440770744196129956E-2,9.9999999999999999991025E-1];
const EQ:[f64;4]=[3.0019850513866445504159E-6,2.5244834034968410419224E-3,2.2726554820815502876593E-1,2.0000000000000000000897E0];
fn polevl(x:f64,c:&[f64])->f64{ let mut a=c[0]; for &ci in &c[1..]{ a=a*x+ci; } a }
// SSE (f64) cephes expm1
fn cephes_sse(x:f64, thr:f64)->f64{
    if x>thr || x < -thr { return rx::excel_exp(x)-1.0; }
    let xx=x*x;
...
fn polevl_x87(x:f64,c:&[f64])->f64{ let mut a=c[0]; for &ci in &c[1..]{ a=tf(&rx::ext_add(&rx::ext_mul(&e(a),&e(x),CW),&e(ci),CW)); } a }
fn cephes_x87(x:f64, thr:f64)->f64{
    ...
    r=tf(&rx::ext_div(&e(r),&e(tf(&rx::ext_sub(&e(q),&e(r),CW))),CW));
    ...
    for thr in [0.5f64, 1.0]{
      for (name, x87) in [("cephes SSE",false),("cephes x87",true)]{
        ...
        for (r,n,em) in &rows{
            ...
            let val= if x87 { cephes_x87(t,thr) } else { cephes_sse(t,thr) };
            if val.to_bits()==em.to_bits(){ok+=1;} else { miss.push((*r,*n, val.to_bits() as i64-em.to_bits() as i64)); }
        }
        print!("  thr={:.1} {:12} {:2}/46", thr, name, ok);
        if ok>=38 { print!("  miss:"); for (r,n,d) in &miss{ print!(" (2^{},{}):{:+}",(r.log2().round()) as i32,n,d);} }
        println!();
      }
    }
}
