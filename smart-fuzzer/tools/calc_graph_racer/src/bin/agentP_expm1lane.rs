...
// --- baselines ---
fn v_base(r: f64, n: f64) -> f64 {
    // all-double Kahan = 163
    let t = tau_sse(r, n);
...
fn v_numdr(r: f64, n: f64) -> f64 {
    // num double-rounded = 165
    let t = tau_sse(r, n);
    let u = rx::excel_exp(t);
    let b = u - 1.0;
    let lnu = rx::excel_ln(u);
...
fn v_u80_fyl2x_rn(r: f64, n: f64) -> f64 {
    let t = e(tau_sse(r, n));
    let u = exp_ext(&t);
    let b = ext_sub(&u, &ext_one(), CW);
    let num = ext_mul(&b, &t, CW);
...
fn v_recip_rn(r: f64, n: f64) -> f64 {
    recip_em(r, n, false)
}
fn v_recip_rz(r: f64, n: f64) -> f64 {
    recip_em(r, n, true)
}
...
fn v_negw_over_1pw(r: f64, n: f64) -> f64 {
    ...
    if tf(&k) == 0.0 {
        ...
        if neg {
            return tf(&ext_div(&ext_chs(&w, CW), &ext_add(&w, &ext_one(), CW), CW));
        }
        return tf(&w);
    }
...
fn exp_pc53(t: f64) -> f64 {
    let tau = e(t);
    let tt = ext_mul(&tau, &ext_l2e(), CW53);
    let k = ext_rndint(&tt, CW53);
    let f = ext_sub(&tt, &k, CW53);
    let neg = ext_to_f64(&f, CW53) < 0.0;
    let w = ext_f2xm1(&ext_abs(&f, CW53), CW53);
    let mut m = ext_add(&w, &ext_one(), CW53);
    if neg {
        m = ext_div(&ext_one(), &m, CW53);
    }
    ext_to_f64(&ext_scale(&m, &k, CW53), CW53)
}
fn ln_pc53(u: f64) -> f64 {
    ext_to_f64(&ext_fyl2x(&ext_ln2(), &e(u), CW53), CW53)
}
...
fn v_pc53_allbody(r: f64, n: f64) -> f64 {
    ...
    let t = ext_to_f64(&ext_mul(&e(n), &e(lp), CW53), CW53);
    let t = -t;
    let u = exp_pc53(t);
    if u == 1.0 {
        return t;
...
fn main() {
    let cands: [(&str, fn(f64, f64) -> f64); 20] = [
        ("base_kahan(163)", v_base),
        ("numdr(165)", v_numdr),
        ("u80_fyl2x_RN", v_u80_fyl2x_rn),
        ("u80_fyl2x_RZ", v_u80_fyl2x_rz),
        ("u80_fyl2xp1_RN", v_u80_fyl2xp1_rn),
        ("u80_fyl2xp1_RZ", v_u80_fyl2xp1_rz),
        ("ud_fyl2xp1_RN", v_ud_fyl2xp1_rn),
        ("recip_RN", v_recip_rn),
        ("recip_RZ", v_recip_rz),
        ("negw_1pw", v_negw_over_1pw),
        ("numdr_divRZ", v_numdr_divrz),
        ("base_divRZ", v_base_divrz),
        ("chopexp_kahan", v_chopexp_kahan),
        ("chopexp_kahan_ndr", v_chopexp_kahan_numdr),
        ("chopexp_um1", v_chopexp_um1),
        ("chopexp_fyl2xp1", v_chopexp_fyl2xp1),
        ("pc53_kahan", v_pc53_kahan),
        ("pc53_kahan_ndr", v_pc53_kahan_numdr),
        ("pc53exp_pc64ln", v_pc53exp_pc64ln),
        ("pc53_allbody", v_pc53_allbody),
    ];
    for (src, path, po2) in [
        (
            "po2n",
        ...
        let tot_lt = pins
            .iter()
            .filter(|(r, n, _)| tau_sse(*r, *n).abs() < ln2)
            .count();
        println!(
            "=== {} ({} |tau|<1 pinned; |tau|<ln2 subset {}) ===",
            src,
            pins.len(),
            tot_lt
        );
        for (nm, f) in cands {
            ...
            for (r, n, ep) in &pins {
                let g = f(*r, *n).to_bits();
                let d = sordi(g) - sordi(ep.to_bits());
                if d == 0 {
                    ok += 1;
                    if tau_sse(*r, *n).abs() < ln2 {
                        ok_lt += 1;
                    }
                } else if d > 0 {
                    plus += 1
