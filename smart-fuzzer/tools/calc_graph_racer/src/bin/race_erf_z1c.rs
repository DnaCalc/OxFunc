//! 4 LOW: z-cut lead+1 RU vs two-factor. Not an identity.
...
fn main() {
    ...
    println!("cuts (bar union 91 leftover 4/4, joint 866, lead+2 33):");
    let cuts = [0.24, 0.30, 0.40, 0.45, 0.46, 0.462, 0.47, 0.472];
    for &c in &cuts {
        let mut rt = 0usize;
        let mut tr = 0usize;
        let mut hit_rt = 0usize;
        let mut hit_tr = 0usize;
        for &(z, bits) in &rows {
            let t = f64::from_bits(bits);
            let g = if z < c { ru(z) } else { tf(z) };
            let h = if z < c { tf(z) } else { ru(z) };
        ...
        for &lz in &LOWS {
            ...
            let t = f64::from_bits(*bits);
            let g = if lz < c { ru(lz) } else { tf(lz) };
            let h = if lz < c { tf(lz) } else { ru(lz) };
            if ulp_distance(g, t).unwrap_or(99) == 0 {
                hit_rt += 1;
            }
            if ulp_distance(h, t).unwrap_or(99) == 0 {
        ...
        println!(
            "  cut={c} RU-then-TF keep={rt} hit={hit_rt}/4  TF-then-RU keep={tr} hit={hit_tr}/4"
        );
    }
}
