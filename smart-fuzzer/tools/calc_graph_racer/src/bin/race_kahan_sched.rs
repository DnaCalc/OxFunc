...
fn main() {
    ...
    let scheds: [(&str, u16, u16, u16); 10] = [
        ...
        ("num64 div64 store64->RN53", CW_PC64_RN, CW_PC64_RN, CW_PC64_RN),
    ...
    println!("=== Kahan precision-schedule matrix (Excel ground-truth u,tau,lnu) N={} ===", tot);
    for (i, (lbl, _, _, _)) in scheds.iter().enumerate() {
        println!("  {:30} {:3}/{}  ({:.1}%)", lbl, score[i], tot, 100.0 * score[i] as f64 / tot as f64);
    }
    ...
    println!("\nbest = [{}]  miss dirs +{} / -{}", scheds[best].0, plus, minus);
    print!("  sample misses (k,n,d): ");
    let mut c = 0;
    for (j, r) in misses.iter().enumerate() {
        if r[best] != 0 && c < 15 { print!("({},{},{:+}) ", meta[j].0, meta[j].1, r[best]); c += 1; }
    }
    println!();
}
