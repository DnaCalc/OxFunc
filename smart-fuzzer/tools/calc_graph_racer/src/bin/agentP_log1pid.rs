...
fn kahan(l:f64, r:f64, n:i64)->f64{
    let t=-(n as f64)*l; let u=rx::excel_exp(t); let lnu=rx::excel_ln(u);
    if u==1.0 {t} else if t.abs()<1.0 {(u-1.0)*t/lnu} else {u-1.0}
}
...
fn main(){
    for (label,path) in [("HDF46","../../work/w109/G6-solvers/pmt_em_hdf_oracle.json"),
        ...
        for (r,n,em) in &rows{
            ...
            let l_patch= if k==-5||k==-4 { na(l_cr,1) } else { l_cr };
            let l_fd=fdlibm_log1p(*r);
            for (nm,l) in [("CR log1p",l_cr),("patch+1@{2^-5,2^-4}",l_patch),("fdlibm log1p",l_fd)]{
                let ok=kahan(l,*r,*n).to_bits()==em.to_bits();
                let e=m.entry(nm).or_insert((0,0)); e.1+=1; if ok{e.0+=1;}
                let pe=perr.entry(nm).or_default().entry(k).or_insert((0,0)); pe.1+=1; if ok{pe.0+=1;}
            }
        }
        println!("=== {} ===",label);
        for (nm,(o,t)) in &m{
            print!("  {:22} {:3}/{:3}  per-r:",nm,o,t);
            for (k,(a,b)) in &perr[nm]{ print!(" 2^{}:{}/{}",k,a,b); }
            println!();
        }
        ...
        if label=="HDF46"{
            ...
            for (r,_,_) in &rows{ let k=(r.log2().round()) as i32; if seen.insert(k){
                let d=fdlibm_log1p(*r).to_bits() as i64 - rx::excel_log1p(*r).to_bits() as i64;
                print!(" 2^{}:{:+}",k,d);
            }}
            println!();
        }
