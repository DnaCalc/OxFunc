//! 3 stubborn leftover: F(z±2/3 ULP). Not an identity.
...
fn main() {
    ...
    let graphs: [(&str, Box<dyn Fn(f64) -> f64>); 6] = [
        ...
        ("F(dn)", Box::new(|z| qw(z, cephes(z.next_down())))),
        (
            "F(dn2)",
            Box::new(|z| qw(z, cephes(z.next_down().next_down()))),
        ),
        (
            "F(dn3)",
            Box::new(|z| qw(z, cephes(z.next_down().next_down().next_down()))),
        ),
        ("F(up)", Box::new(|z| qw(z, cephes(z.next_up())))),
        (
            "F(up2)",
            Box::new(|z| qw(z, cephes(z.next_up().next_up()))),
        ),
    ];
    println!("3 stubborn F(z±k ulp):");
    for &lz in &STUB {
