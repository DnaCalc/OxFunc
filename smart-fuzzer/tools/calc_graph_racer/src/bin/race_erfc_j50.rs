//! j3432-hard 50 vs j+0x20/0x24 and remaining named F. Not an identity.
...
fn main() {
    ...
    println!("j3432-hard direct ulp>=2 n={}", hard.len());
    let graphs: [(&str, Box<dyn Fn(f64) -> f64>); 8] = [
        ("j+0x20", Box::new(|z| qw(z, cody(z, &jc, &jd, 0x20)))),
        ("j+0x24", Box::new(|z| qw(z, cody(z, &jc, &jd, 0x24)))),
        ("0x74", Box::new(|z| qw(z, cody(z, &C0, &D0, 0x74)))),
        ("0x200", Box::new(|z| qw(z, cody(z, &C0, &D0, 0x200)))),
    ...
    for (name, ev) in &graphs {
        ...
        for r in &hard {
            ...
            if d == 0 {
                hit += 1;
                if shown < 6 {
                    println!("  HIT {name} z={:.16} was_j_ulp>=2", r.z);
                    shown += 1;
                }
            } else {
                mx = mx.max(d);
        ...
        println!("{name:10} hard_hit={hit}/{} keep={keep}/7741 hard_max_miss={mx}", hard.len());
    }
}
