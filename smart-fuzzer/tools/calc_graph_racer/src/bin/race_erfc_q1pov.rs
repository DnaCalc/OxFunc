//! Overlap of 1−Schonfelder vs 1−A21-joint as Q for z<0.5. Not an identity.
...
fn main() {
    ...
    for r in &rows {
        ...
        n += 1;
        let ds = ulp_distance(q1p(erf_sch(r.z)), f64::from_bits(r.qbits)).unwrap_or(99);
        let dj = ulp_distance(q1p(erf_joint(r.z)), f64::from_bits(r.qbits)).unwrap_or(99);
        if ds == 0 {
            sex += 1;
        }
        if dj == 0 {
