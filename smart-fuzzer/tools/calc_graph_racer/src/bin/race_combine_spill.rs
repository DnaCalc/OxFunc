...
fn s_mul(a: f64, b: f64) -> f64 { ext_to_f64(&ext_mul(&ext_from_f64(a), &ext_from_f64(b), CW_PC64_RN), RN53) }
fn s_div(a: f64, b: f64) -> f64 { ext_to_f64(&ext_div(&ext_from_f64(a), &ext_from_f64(b), CW_PC64_RN), RN53) }
fn s_add(a: f64, b: f64) -> f64 { ext_to_f64(&rx::ext_add(&ext_from_f64(a), &ext_from_f64(b), CW_PC64_RN), RN53) }

// em variants
fn em_sse2(nl: f64) -> f64 { rx::excel_expm1_internal(nl) }
...
fn main() {
    let corpora = ["po2", "po2n", "genrate", "heldout", "combsweep", "fvty", "fv1sweep"];
    ...
    print!("{:11}{:>7}", "corpus", "N");
    for (nm, _, _) in &variants { print!(" {:>22}", nm); }
    println!();
    for cn in corpora {
        ...
        let mut sc = [0u32; 4]; let mut tot = 0u32;
        for w in &ws.witnesses {
            ...
            for (i, (_, ef, cf)) in variants.iter().enumerate() {
                let em = ef(nl); if em == 0.0 { continue; }
                if cf(r, pv, fv, ty, em).to_bits() == want { sc[i] += 1; }
            }
        }
        print!("{:11}{:>7}", cn, tot);
        for i in 0..4 { print!(" {:>21.1}%", 100.0 * sc[i] as f64 / tot as f64); }
        println!();
    }
}
