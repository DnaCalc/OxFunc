...
#[derive(Deserialize)]
struct WitnessSet { function: String, witnesses: Vec<Witness> }

#[derive(Deserialize)]
struct Witness { id: String, expected_bits: String }
...
struct Fitness { exact: usize, total: usize, sum_abs_ulp: u128, max_abs_ulp: u128 }
...
fn predict(row: &Row, constants: &BTreeMap<String, f64>, schedule: &str) -> u64 {
    ...
    match schedule {
        ...
        "pc64_div_mul_store_core" => {
            ...
            rx::ext_to_f64(
                &rx::ext_mul(&rx::ext_from_f64(stored), &rx::ext_from_f64(pow10(delta)), cw),
                cw,
            ).to_bits()
        }
        "pc64_div_mul_continuous_delta" => {
            ...
            let core = rx::ext_mul(&divided, &rx::ext_from_f64(to), cw);
            rx::ext_to_f64(
                &rx::ext_mul(&core, &rx::ext_from_f64(pow10(delta)), cw),
                cw,
            ).to_bits()
        }
...
fn load(root: &Path, dataset: &str, meta_name: &str, answer_name: &str) -> Vec<Row> {
    let metadata: MetaDocument = serde_json::from_slice(&std::fs::read(root.join(meta_name)).unwrap()).unwrap();
    let answers: WitnessSet = serde_json::from_slice(&std::fs::read(root.join(answer_name)).unwrap()).unwrap();
    assert_eq!(answers.function, "CONVERT");
    let by_id: BTreeMap<_, _> = answers.witnesses.iter().map(|w| (w.id.as_str(), w)).collect();
    metadata.rows.into_iter().filter(|row| row.category == "length").map(|row| Row {
        id: row.id.clone(),
...
fn main() {
    ...
    let datasets = [
        ("discovery", "batch-convert-discovery-20260809-meta.json", "answers-convert-discovery-20260809-clean.json"),
        ("retired-v1", "batch-convert-heldout-20260809-meta.json", "answers-convert-heldout-20260809.json"),
        ("retired-v2", "batch-convert-publication-heldout-v2-20260809-meta.json", "answers-convert-publication-heldout-v2-20260809.json"),
        ("v3-refinement", "batch-convert-v3-length-discriminator-20260809-meta.json", "answers-convert-v3-length-discriminator-20260809.json"),
    ];
    let rows: Vec<_> = datasets.into_iter().flat_map(|(dataset, meta, answers)| load(&root, dataset, meta, answers)).collect();
    let units = ["m", "in", "ft", "yd", "mi", "Nmi"];
    let mut candidates = Vec::new();
    for scale_exponent in -24..=24 {
        for construction in CONSTRUCTIONS {
            let constants: BTreeMap<_, _> = units.iter().map(|unit| ((*unit).to_string(), constant(unit, scale_exponent, construction))).collect();
            for schedule in SCHEDULES {
                ...
                for row in &rows {
                    ...
                    add_fit(dataset_fitness.entry(row.dataset.clone()).or_default(), predicted, row.actual);
                    if predicted != row.actual && first_misses.len() < 16 {
                        first_misses.push(format!(
                            "{} {} predicted=0x{predicted:016x} oracle=0x{:016x}", row.dataset, row.id, row.actual
                        ));
                    }
                }
                candidates.push(Candidate {
                    ...
                    fitness,
                    dataset_fitness,
                    constants_bits: constants.iter().map(|(unit, value)| (unit.clone(), format!("0x{:016x}", value.to_bits()))).collect(),
                    first_misses,
                });
            }
        }
    }
    candidates.sort_by_key(|candidate| (std::cmp::Reverse(candidate.fitness.exact), candidate.fitness.sum_abs_ulp, candidate.fitness.max_abs_ulp));
    for candidate in candidates.iter().take(20) {
        println!(
            "scale={} {} {}: {}/{} sum={} max={}",
            candidate.scale_exponent, candidate.construction, candidate.schedule,
            candidate.fitness.exact, candidate.fitness.total,
            candidate.fitness.sum_abs_ulp, candidate.fitness.max_abs_ulp
        );
    }
    let report = Report {
