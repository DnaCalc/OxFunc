...
fn fb(h: &str) -> f64 {
    f64::from_bits(u64::from_str_radix(h.trim().trim_start_matches("0x"), 16).unwrap())
}
fn e(x: f64) -> rx::Ext80 {
    rx::ext_from_f64(x)
}
...
fn load() -> Vec<Row> {
    ...
    for (i, line) in txt.lines().enumerate() {
        ...
        out.push(Row {
            k: c[0].trim().parse().unwrap(),
            n: c[1].trim().parse().unwrap(),
            tau: fb(c[2]),
            u: fb(c[3]),
            lnu: fb(c[4]),
            em: fb(c[5]),
...
fn main() {
    ...
    let bump =
        |name: &'static str, got: f64, em: f64, m: &mut std::collections::BTreeMap<&str, u32>| {
            if got.to_bits() == em.to_bits() {
    ...
    for r in &rows {
        ...
        bump(
            "M1 x87mul(b,tau)/lnu",
            rx::x87_mul(b, tau) / lnu,
            em,
            &mut cnt,
        );
        bump(
            "M1x x87mul then x87div",
            tf(&rx::ext_div(&e(rx::x87_mul(b, tau)), &e(lnu), CW)),
            em,
            &mut cnt,
        );
        // production internal
        bump(
            "PROD excel_expm1_internal(tau)",
    ...
    for r in &rows {
        if rx::excel_exp(r.tau).to_bits() == r.u.to_bits() {
            u_ok += 1;
        }
        if rx::excel_ln(r.u).to_bits() == r.lnu.to_bits() {
            lnu_ok += 1;
        }
    }
    println!("rows={n}");
