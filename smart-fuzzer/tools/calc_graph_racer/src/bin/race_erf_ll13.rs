//! Leftover-low 13: grid class and 1+w next_up. Not an identity.
...
fn one_w_up(z: f64, a: &[f64; 21]) -> f64 {
    let xe = ef(z);
    let t = ext_mul(&xe, &xe, CW);
    let mut acc = ef(0.0);
    for &c in a.iter().rev() {
...
fn main() {
    ...
    println!("leftover-low (joint ulp>=2 and graph<excel):");
    let mut n = 0usize;
    let mut up_hit = 0usize;
    for &(z, bits) in &rows {
        ...
        n += 1;
        let du = ulp_distance(one_w_up(z, &joint), t).unwrap_or(99);
        if du == 0 {
            up_hit += 1;
        }
        println!(
            "  z={z:.16} kind={} j_ulp={d} 1+w_up={du} bits={:#x}",
            kind(z),
            z.to_bits()
        );
    }
    println!("n={n} 1+w_up hit={up_hit}");
}
