//! A0+4 vs Taylor 2/sqrt(pi)−1. Not an identity.
...
fn main() {
    ...
    let a0f = 2.0 / PI.sqrt() - 1.0;
    let a0x = ext_to_f64(
        &ext_sub(&ext_div(&ef(2.0), &ext_sqrt(&ef(PI), CW), CW), &ef(1.0), CW),
        CW,
    );
    println!(
        "A0 pub {:016x} A0+4 {:016x} tay_f64 {:016x} tay_x87_store {:016x}",
        AS0[0].to_bits(),
        a0p.to_bits(),
        a0f.to_bits(),
        a0x.to_bits()
    ...
    for (name, a) in [
        ("A0 pub CR", AS0),
        ("A0+4 rest pub", ap),
        ("A0=tay_f64 rest pub", at),
        ("A0=tay_x87 rest pub", ax),
    ] {
        let mut ex = 0usize;
        let mut n = 0usize;
        let mut maxu = 0u64;
