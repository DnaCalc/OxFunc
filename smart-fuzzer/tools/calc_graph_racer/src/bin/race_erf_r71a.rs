//! 71 remaining 1-ULP vs unpoked A21. Not an identity.
...
fn main() {
    ...
    let mut kj = 0usize;
    let mut kc = 0usize;
    println!("remaining 1-ULP vs unpoked A21:");
    for &(z, bits) in &rows {
        ...
        n += 1;
        let hit = ulp_distance(cr(z), t).unwrap_or(99) == 0;
        if hit {
            h += 1;
        }
        println!(
            "  z={z:.16} unpoked={} d={}",
            hit,
            ulp_distance(cr(z), t).unwrap_or(99)
        );
    }
    println!("n={n} unpoked-hit={h} keep joint={kj} unpoked={kc}/{}", rows.len());
}
