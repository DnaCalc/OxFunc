//! 66 leftover-low vs unmask F last-store. Not an identity.
...
fn main() {
    ...
    let mut cov = 0usize;
    let mut miss = 0usize;
    println!("66 leftover-low vs unmask F last-store DIRECT z>=4:");
    for r in &tail {
        let t = f64::from_bits(r.qbits);
        let g = mul(f::w_rn53(r.z), cephes(r.z, &p66, &q66, MASK));
        let d = ulp_distance(g, t).unwrap_or(99);
        if !(d >= 2 && g < t) {
            continue;
        }
        nlo += 1;
        let k = signed_k(r.z, t, &P0, &Q0, 0);
        match k {
            Some(kk) => {
            ...
            None => {
                miss += 1;
                println!("  z={:.16} d={d} unmask MISS", r.z);
            }
        }
    }
    print!("leftover-low unmask F± {cov}/{nlo} miss={miss} |k|");
