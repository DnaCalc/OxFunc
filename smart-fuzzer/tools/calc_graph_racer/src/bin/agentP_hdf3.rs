...
fn main(){
    ...
    let variants:Vec<(&str,V)>=vec![
        ...
        ("A1 (u-1)+(u-1)*(t-lnu)/lnu", Box::new(|u,t,lnu|{ let a=u-1.0; a + a*(t-lnu)/lnu })),
        ("A2 (u-1)*(1+(t-lnu)/lnu)", Box::new(|u,t,lnu|{ let a=u-1.0; a*(1.0+(t-lnu)/lnu) })),
        ("A3 (u-1)+(u-1)/lnu*(t-lnu)", Box::new(|u,t,lnu|{ let a=u-1.0; a + a/lnu*(t-lnu) })),
        ("A4 (u-1)+ (t-lnu)*((u-1)/lnu)", Box::new(|u,t,lnu|{ let a=u-1.0; a + (t-lnu)*(a/lnu) })),
        // extended: correction term extended, added f64
        ("A5 (u-1)+ext[(u-1)*(t-lnu)/lnu]", Box::new(|u,t,lnu|{ let a=u-1.0;
            let corr=tf(&rx::ext_div(&rx::ext_mul(&e(a),&e(t-lnu),CW),&e(lnu),CW)); a+corr })),
        // t-lnu computed extended (t,lnu both f64 here)
        ("A6 (u-1)+(u-1)*ext(t-lnu)/lnu", Box::new(|u,t,lnu|{ let a=u-1.0;
            let d=tf(&rx::ext_sub(&e(t),&e(lnu),CW)); a + a*d/lnu })),
        // whole additive form in x87 PC64, single store
        ("A7 ext-all additive", Box::new(|u,t,lnu|{ let a=e(u-1.0);
    ...
    for (name,f) in &variants{
        ...
        for (r,n,em) in &rows{
            ...
            let val= if u==1.0 { t } else if t.abs()<1.0 { f(u,t,rx::excel_ln(u)) } else { u-1.0 };
            if val.to_bits()==em.to_bits(){ok+=1;} else { miss.push((*r,*n, val.to_bits() as i64-em.to_bits() as i64)); }
        }
        print!("  {:34} {:2}/46", name, ok);
        if ok>=39 { print!("  miss:"); for (r,n,d) in &miss{ print!(" (2^{},{}):{:+}",(r.log2().round()) as i32,n,d);} }
        println!();
    }
}
