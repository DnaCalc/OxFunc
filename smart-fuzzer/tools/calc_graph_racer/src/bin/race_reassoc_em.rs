...
fn main() {
    ...
    for line in csv.lines().skip(1) {
        ...
        let ae = ext_from_f64(a); let te = ext_from_f64(tau); let le = ext_from_f64(lnu);
        ...
        for i in 0..8 { if cands[i]==emb { score[i]+=1; if base_miss { score_miss[i]+=1; } } }
    ...
    for i in 0..8 {
        println!("{:<40} {:>4}/{} ({:4.1}%) {:>4}/{}", names[i], score[i], tot,
                 100.0*score[i] as f64/tot as f64, score_miss[i], nmiss);
    }
    println!("\ncorr-form(1) FIXES {} miss rows, BREAKS {} hit rows", fix1.len(), break1);
    print!("fixed: "); for x in fix1.iter().take(20) { print!("({},{}) ", x.0, x.1); } println!();
}
