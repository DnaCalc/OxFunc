...
fn log1p_ext_fyl2x(r:f64)->Ext80{ ext_mul(&ext_ln2(),&ext_fyl2x(&ext_one(),&ext_add(&ext_one(),&e(r),CW),CW),CW) }
fn log1p_ext_fyl2xp1(r:f64)->Ext80{
    if r.abs()<0.292893218813452 { ext_mul(&ext_ln2(),&ext_fyl2xp1(&ext_one(),&e(r),CW),CW) }
    else { log1p_ext_fyl2x(r) }
}
...
fn em(r:f64,n:f64, fyl2xp1:bool, num_from_ext:bool)->f64{
    let l1p = if fyl2xp1 { log1p_ext_fyl2xp1(r) } else { log1p_ext_fyl2x(r) };
...
fn main(){
    for (src,path,po2) in [("po2n","../../work/w109/G6-solvers/answers-pmt-po2n.json",true),
        ...
        let refok=pins.iter().filter(|(r,n,ep)| em_ref(*r,*n).to_bits()==ep.to_bits()).count();
        println!("=== {} ({} pts)  ref(double Kahan)={} ===", src, pins.len(), refok);
        for (nm,fp,ne) in [("fyl2x, num=tau_d",false,false),("fyl2x, num=ext",false,true),
                            ("fyl2xp1, num=tau_d",true,false),("fyl2xp1, num=ext",true,true)]{
            let ok=pins.iter().filter(|(r,n,ep)| em(*r,*n,fp,ne).to_bits()==ep.to_bits()).count();
            // how many does the ext-u path FIX vs ref, and how many does it BREAK
            let mut fixed=0; let mut broke=0;
            for (r,n,ep) in &pins{
                let rf=em_ref(*r,*n).to_bits()==ep.to_bits();
                let ex=em(*r,*n,fp,ne).to_bits()==ep.to_bits();
                if ex && !rf {fixed+=1;} if rf && !ex {broke+=1;}
            }
            println!("  {:<20} {:>3}/{}   (fixed {} / broke {})", nm, ok, pins.len(), fixed, broke);
        }
    }
}
