//! Pin 4 exclusive leftover z of 66 vs Hamming family. Not an identity.
...
const FOUR: [(&'static str, f64); 4] = [
    ("lo", 4.5833333333333330),
    ("lo", 5.3020833333333330),
    ("hi", 4.3645833333333330),
    ("hi", 5.8333333333333330),
];
const P0: [f64; 9] = [
...
fn buck(g: f64, t: f64) -> &'static str {
...
fn main() {
    ...
    q66[1] = poke(Q0[1], -1);
    q66[3] = poke(Q0[3], 1);
    println!("pin 4 exclusive leftover z of 66 vs Hamming:");
    for &(side, lz) in &FOUR {
        let Some(r) = tail.iter().find(|rr| (rr.z - lz).abs() < 1e-12) else {
            println!("  {side} z={lz:.16} MISSING");
            continue;
        };
        let t = f64::from_bits(r.qbits);
        let w = f::w_rn53(lz);
        let ff = cephes(lz, &P0, &Q0, 0);
        let g = mul(w, ff);
        let k = k_of(lz, t, ff);
        let g66 = mul(w, cephes(lz, &p66, &q66, 0x24a5));
        let g24 = mul(w, cephes(lz, &P0, &Q0, 0x24a5));
        println!(
            "  {side} z={lz:.16} bits={:#x} grid={} unmask_d={} k={k:?} last-mul up={:?} dn={:?} w+1={:?} w-1={:?} 66={} 0x24a5={} 4005={} 4e05={} 5005={}",
            lz.to_bits(),
            grid(lz),
            ...
            buck(g66, t),
            buck(g24, t),
            buck(mul(w, cephes(lz, &P0, &Q0, 0x4005)), t),
            buck(mul(w, cephes(lz, &P0, &Q0, 0x4e05)), t),
            buck(mul(w, cephes(lz, &P0, &Q0, 0x5005)), t)
        );
    }
}
