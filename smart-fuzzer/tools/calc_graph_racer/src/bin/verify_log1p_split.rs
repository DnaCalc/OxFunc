...
fn main(){
    let ws:WitnessSet=serde_json::from_str(&std::fs::read_to_string("../../work/w109/G6-solvers/answers-pmt-heldout.json").unwrap()).unwrap();
    ...
    for w in &ws.witnesses{
        ...
        if hi { n_hi+=1; if cok{cr_hi+=1;} if sok{std_hi+=1;} } else { n_lo+=1; if cok{cr_lo+=1;} if sok{std_lo+=1;} }
    }
    println!("heldout split by |tau|:");
    println!("  |tau|<1  (n={}): CR {}  std {}  (std-CR={})", n_lo, cr_lo, std_lo, std_lo as i64-cr_lo as i64);
    println!("  |tau|>=1 (n={}): CR {}  std {}  (std-CR={})", n_hi, cr_hi, std_hi, std_hi as i64-cr_hi as i64);
    println!("=> if std-CR is +large in |tau|<1 and <=0 in |tau|>=1, log1p is CR (std's edge is expm1-wall compensation)");
}
