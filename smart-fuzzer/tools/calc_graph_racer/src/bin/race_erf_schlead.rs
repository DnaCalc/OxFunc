//! 4 LOW: Schon 862 lead z+2 original inner y. Not an identity.
...
fn sch(lead: f64, inner: f64) -> f64 {
    ext_to_f64(&ext_mul(&ef(lead.abs()), &y_of(inner), CW), CW)
}
...
fn main() {
    ...
    let graphs: [(&str, Box<dyn Fn(f64) -> f64>); 4] = [
        ("sch862", Box::new(|z| sch(z, z))),
        ("lead+1", Box::new(|z| sch(z.next_up(), z))),
        ("lead+2", Box::new(|z| sch(z.next_up().next_up(), z))),
        (
            "both+2",
            Box::new(|z| {
    ...
    println!("4 LOW sch862 lead+k orig inner:");
    for &lz in &LOWS {
