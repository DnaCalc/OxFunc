        ...
        for ((rb, nb), rows) in &data {
            ...
            if let Some(e) = pin_gen(rows, center, r) {
                let key = match (tau.abs() >= 1.0, exact_1pr(r)) {
                    (true, true) => "CLEAN_exactR",
                    (true, false) => "CLEAN_inexactR",
                    (false, true) => "cont_exactR",
                    (false, false) => "cont_inexactR",
                };
        ...
        for (bk, v) in &buckets {
            ...
            if bk.starts_with("CLEAN") {
                ...
                for (r, n, ep) in v {
                    ...
                    for d in -8..=8i64 {
                        let l = na(lcr, d);
                        if rx::excel_expm1_internal(-(n * l)).to_bits() == ep.to_bits() {
                            hits.push(d);
                        }
                    }
                    if hits.is_empty() {
                        *hist.entry(99).or_insert(0) += 1;
                    } else {
                        for d in hits {
                ...
                println!(
                    "      recovered log1p-dev(ulp) hist (99=no-hit): {:?}",
                    hist
                );
            }
        }
