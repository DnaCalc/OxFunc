...
use rx::{
    CW_PC53_RN, CW_PC64_RN, ext_div, ext_from_f64, ext_fyl2xp1, ext_ln2, ext_mul, ext_to_f64,
};
...
fn main() {
    ...
    for line in csv.lines().skip(1) {
        ...
        if ext_to_f64(&tau_ext, RN53).to_bits() == tau.to_bits() {
            tau_ext_ok += 1;
        }
        let a = u - 1.0;
    ...
    println!(
        "N={} misses={} | tau_ext spills==captured tau on {}/{}",
        tot, nmiss, tau_ext_ok, tot
    );
    ...
    println!(
        "\nnum-tau-ext(2) FIXES {} miss rows, BREAKS {} hit rows",
        fix1.len(),
        break1
    );
    print!("fixed: ");
