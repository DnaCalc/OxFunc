//! 1-bit and 2-bit Cephes Horner stores vs 0x5005 DIRECT z>=4. Not an identity.
...
fn main() {
    ...
    let s0 = score(0);
    let s5 = score(0x5005);
    println!("DIRECT z>=4 unmask={s0} 0x5005={s5}/{n}");
    println!("1-bit:");
    let mut best1 = s0;
    for b in 0..17u32 {
        let s = score(1u32 << b);
        if s != s0 {
            println!("  bit={b} fused={s} Δ={:+}", s as i32 - s0 as i32);
        }
        if s > best1 {
            best1 = s;
        }
    }
    println!("best 1-bit={best1}");
    let mut ranked: Vec<(usize, u32, u32, u32)> = Vec::new();
    for a in 0..17u32 {
        for b in (a + 1)..17 {
            let mask = (1u32 << a) | (1u32 << b);
            ranked.push((score(mask), mask, a, b));
        }
    }
    ranked.sort_by(|x, y| y.0.cmp(&x.0));
    println!("top 8 2-bit:");
    for (s, mask, a, b) in ranked.iter().take(8) {
        println!(
            "  fused={s} mask={mask:#x} bits {a}+{b} {}",
            if *mask == 0x5005 { "<- 0x5005" } else { "" }
        );
    }
    let nge = ranked.iter().filter(|t| t.0 >= s5).count();
    let ngt = ranked.iter().filter(|t| t.0 > s5).count();
    println!(
        "best 2-bit={} mask={:#x} ge5005={nge} gt5005={ngt} (bar {s5})",
