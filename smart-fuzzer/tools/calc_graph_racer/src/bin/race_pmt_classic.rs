...
fn powi_ext(opr: f64, mut e: u32) -> f64 {
    let cw = CW_PC64_RN;
    let mut res = ext_one();
    let mut b = ext_from_f64(opr);
    while e > 0 {
        if e & 1 == 1 {
            res = ext_mul(&res, &b, cw);
        }
        e >>= 1;
        if e > 0 {
            b = ext_mul(&b, &b, cw);
...
fn main() {
    ...
    for (mn, ml) in [(0u8, "Pd"), (1u8, "Pext"), (2u8, "Pexplog")] {
        cands.push((
            format!("classic/{}", ml),
            Box::new(move |r, n, pv, fv, ty| v_classic(r, n, pv, fv, ty, mn)),
        ));
        cands.push((
            format!("classic_rlast/{}", ml),
            Box::new(move |r, n, pv, fv, ty| v_classic_rlast(r, n, pv, fv, ty, mn)),
        ));
        cands.push((
            format!("recip_em/{}", ml),
            Box::new(move |r, n, pv, fv, ty| v_recip_em(r, n, pv, fv, ty, mn)),
        ));
        cands.push((
            format!("classic_qf/{}", ml),
            Box::new(move |r, n, pv, fv, ty| v_classic_qf(r, n, pv, fv, ty, mn)),
        ));
    }
    // header
    print!("{:16}", "corpus/N");
    ...
    for cn in corpora {
        ...
        for i in 0..cands.len() {
            print!(" {:>15.0}%", 100.0 * sc[i] as f64 / tot as f64);
        }
        println!();
    }
}
