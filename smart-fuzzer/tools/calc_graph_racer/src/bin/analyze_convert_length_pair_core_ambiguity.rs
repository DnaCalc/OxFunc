...
fn process(root: &Path, meta_name: &str, answer_name: &str, pairs: &mut BTreeMap<String, PairScore>) -> usize {
...
fn main() {
    ...
    let rows = datasets.into_iter().map(|(m, a)| process(&root, m, a, &mut pairs)).sum();
    let conflict_free_ratio_switch_pairs = pairs.iter()
        .filter(|(_, score)| score.ratio_wins > 0 && score.primary_wins == 0 && score.neither_exact == 0)
    ...
    println!("rows={} conflict_free_switch_pairs={:?}", rows, report.conflict_free_ratio_switch_pairs);
