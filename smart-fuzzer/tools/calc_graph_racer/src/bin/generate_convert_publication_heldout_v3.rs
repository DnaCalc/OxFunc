...
fn model_predictions(
    ...
    MODEL_NAMES
        ...
        .map(|(name, variant)| {
            (
                (*name).to_string(),
                model_v3::predict(number, category, from, to, variant).display(),
            )
        })
        .collect()
}
...
fn old_seen(root: &Path) -> BTreeSet<(u64, String, String)> {
    ...
    for row in readback.rows {
        seen.insert((
            common::f64_from_hex(&row.requested_bits).unwrap().to_bits(),
            row.from_unit,
            row.to_unit,
        ));
    }
...
fn find_discriminators(
    ...
    println!(
        "selected {added} {class} rows for {} after {attempts} attempts",
        category.name()
    );
    added
}
...
fn main() {
        ...
        for pass in 0..2 {
            for from in &units {
                for to in &units {
                    ...
                    builder.push(
                        if pass == 0 {
                            "publication-all-pairs-a"
                        } else {
                            "publication-all-pairs-b"
                        },
                        category.name(),
                        number,
                        &from.name,
                ...
                for exponent in [-220, -128, -64, -1, 0, 1, 64, 128, 220] {
                    ...
                    for number in [
                        ...
                        if !builder.contains(number, &from.name, &to.name) {
                            builder.push(
                                "publication-direct-power-ladder",
                                category.name(),
                                number,
                                &from.name,
                                &to.name,
    ...
    let synthetic_path =
        root.join("synthetic-answers-convert-publication-heldout-v3-20260809.json");
    ...
    println!(
        "offline candidate-informative rows: {}",
        metadata.rows.iter().filter(|row| row.informative).count()
    );
    for class in [
        "publication-all-pairs-a",
        "publication-all-pairs-b",
