...
fn exp_ext_arg(arg:&rx::Ext80)->rx::Ext80{
    let z=rx::ext_mul(arg,&rx::ext_l2e(),CW); let k=rx::ext_rndint(&z,CW); let f=rx::ext_sub(&z,&k,CW);
...
fn main(){
    ...
    let variants:Vec<(&str, Box<dyn Fn(f64,i64)->f64>)>=vec![
        ...
        ("v=exp(RN(-n*ln(1+r)))-1", Box::new(|r,n|{ let l=rx::excel_ln(1.0+r); let t2=rx::x87_mul(-(n as f64),l); rx::excel_exp(t2)-1.0 })),
        // pow chain SSE product
        ("v=exp(SSE -n*ln(1+r))-1", Box::new(|r,n|{ let l=rx::excel_ln(1.0+r); let t2=-(n as f64)*l; rx::excel_exp(t2)-1.0 })),
        // ln(1+r) EXTENDED, *-n extended, exp extended, v-1 f64
        ("v=exp_ext(-n*lnext(1+r)) f64 -1", Box::new(|r,n|{ let le=rx::ext_fyl2x(&rx::ext_ln2(),&e(1.0+r),CW); let t2=rx::ext_mul(&e(-(n as f64)),&le,CW); let v=tf(&exp_ext_arg(&t2)); v-1.0 })),
        // ln(1+r) EXTENDED, exp extended, RN53(v_ext-1)
        ("RN53(exp_ext(-n*lnext)-1)", Box::new(|r,n|{ let le=rx::ext_fyl2x(&rx::ext_ln2(),&e(1.0+r),CW); let t2=rx::ext_mul(&e(-(n as f64)),&le,CW); let ve=exp_ext_arg(&t2); tf(&rx::ext_sub(&ve,&rx::ext_one(),CW)) })),
        // log1p (fyl2xp1) extended, exp extended, RN53(v_ext-1) -- the em stable chain but v-1 not Kahan
        ("RN53(exp_ext(-n*log1pext)-1)", Box::new(|r,n|{ let le=rx::ext_fyl2xp1(&rx::ext_ln2(),&e(r),CW); let t2=rx::ext_mul(&e(-(n as f64)),&le,CW); let ve=exp_ext_arg(&t2); tf(&rx::ext_sub(&ve,&rx::ext_one(),CW)) })),
    ];
