//! 0x210 leftover-low last-mul vs last-store of unmask. Not an identity.
...
fn main() {
    ...
    println!("0x210 leftover-low last-mul vs last-store of unmask:");
    for r in rows.iter().filter(|rr| rr.direct && rr.z >= 0.5 && rr.z < 4.0) {
        let t = f64::from_bits(r.qbits);
        let g210 = mul(f::w_rn53(r.z), cody(r.z, MASK));
