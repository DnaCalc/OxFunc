...
fn main() {
    ...
    for (ln, lf) in logs {
        ...
        for (r, n, em) in &rows {
            ...
            if e.to_bits() == em.to_bits() {
                ok += 1;
            } else {
                miss.push((*r, *n, e.to_bits() as i64 - em.to_bits() as i64));
            }
        }
        print!("  {:12} {:2}/46", ln, ok);
        if !miss.is_empty() {
    ...
    for (r, _, _) in &rows {
        ...
        println!(
            "  2^{:<3}: port=0x{:016x} fyl2xp1=0x{:016x} ln(1+r)=0x{:016x}  d(ln(1+r)-fyl2xp1)={:+}",
            k,
            a.to_bits(),
            b.to_bits(),
            c.to_bits(),
            c.to_bits() as i64 - b.to_bits() as i64
        );
    }
}
