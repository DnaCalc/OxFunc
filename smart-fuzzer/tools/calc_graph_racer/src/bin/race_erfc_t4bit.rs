//! Exhaustive 4-bit Cephes Horner stores DIRECT z>=4. Not an identity.
...
fn main() {
    ...
    let s4 = score(0x4005);
    let s5 = score(0x5005);
    println!("DIRECT z>=4 n={n} unmask={s0} 0x4005={s4} 0x5005={s5}");
    println!("0x4005 ∪ extra bit:");
    let mut best_add = s4;
    let mut best_add_b = 99u32;
    for b in 0..17u32 {
        if 0x4005 & (1u32 << b) != 0 {
            continue;
        }
        let m = 0x4005 | (1u32 << b);
        let s = score(m);
        let d = s as i32 - s4 as i32;
        if d != 0 {
            println!("  ∪bit{b} mask={m:#x} fused={s} Δ={d:+}");
        }
        if s > best_add {
            best_add = s;
            best_add_b = b;
    ...
    println!("best 0x4005∪bit fused={best_add} extra_bit={best_add_b}");
    let mut ranked: Vec<(usize, u32)> = Vec::new();
    for a in 0..17u32 {
        for b in (a + 1)..17 {
            for c in (b + 1)..17 {
                for d in (c + 1)..17 {
                    let mask = (1u32 << a) | (1u32 << b) | (1u32 << c) | (1u32 << d);
    ...
    println!("top 12 4-bit:");
    for (s, mask) in ranked.iter().take(12) {
        let tag = if *mask == 0x5005 { " <- 0x5005" } else { "" };
        println!("  fused={s} mask={mask:#x}{tag}");
    }
    let best = ranked[0].0;
    ...
    println!(
        "best 4-bit={best} mask={:#x} unique={} nbest={nbest} ge57={nge57} gt57={ngt57} nquads={}",
        ranked[0].1,
        nbest == 1,
        ranked.len()
    );
    println!("all fused>57:");
    for (s, mask) in ranked.iter().filter(|t| t.0 > 57) {
        println!("  fused={s} mask={mask:#x}");
    }
    println!("all fused==57 4-bit:");
    for (s, mask) in ranked.iter().filter(|t| t.0 == 57) {
        println!("  fused={s} mask={mask:#x}");
    }
}
