//! 4 LOW: F(z.next_down/up) of joint / m856 / fd P[0]+4. Not an identity.
...
fn main() {
    ...
    let graphs: [(&str, Box<dyn Fn(f64) -> f64>); 6] = [
        ("j(dn)", Box::new(|z| joint(z.next_down()))),
        ("j(up)", Box::new(|z| joint(z.next_up()))),
        ("m(dn)", Box::new(|z| m856(z.next_down()))),
        ("m(up)", Box::new(|z| m856(z.next_up()))),
        ("fd(dn)", Box::new(|z| fd4(z.next_down()))),
        ("fd(up)", Box::new(|z| fd4(z.next_up()))),
    ];
    println!("4 LOW F(z±1ulp):");
    for &lz in &LOWS {
