//! 4 LOW: joint with t=(z+k ulp)^2, lead z un-nudged. Not an identity.
...
fn main() {
    ...
    let graphs: [(&str, Box<dyn Fn(f64) -> f64>); 5] = [
        ...
        ("t=z+1", Box::new(|z| joint_t(z, z.next_up()))),
        ("t=z+2", Box::new(|z| joint_t(z, z.next_up().next_up()))),
        ("t=z-1", Box::new(|z| joint_t(z, z.next_down()))),
        ("t=z-2", Box::new(|z| joint_t(z, z.next_down().next_down()))),
    ];
    println!("4 LOW joint t=(z±k)^2 lead z:");
    for &lz in &LOWS {
