...
fn exp_store(t:f64, cw_store:u16)->f64{
    ...
    let neg=tf(&f)<0.0; let w=rx::ext_f2xm1(&rx::ext_abs(&f,CW),CW);
    let mut m=rx::ext_add(&w,&rx::ext_one(),CW); if neg{ m=rx::ext_div(&rx::ext_one(),&m,CW);} 
    rx::ext_to_f64(&rx::ext_scale(&m,&k,CW), cw_store)
}
fn kahan(u:f64,t:f64)->f64{ if u==1.0 {t} else if t.abs()<1.0 { (u-1.0)*t/rx::excel_ln(u) } else { u-1.0 } }
fn main(){
    ...
    for (r,n,em) in &rows{
        ...
        let u=tf(&ue);
        let val= if u==1.0 {t} else if t.abs()<1.0 {
