//! Cephes 0x5005 bit ablation on Q-tail; SPECFUN C/D 0x74 + P/Q @4.
//! Not an identity.
...
impl Acc {
    fn add(&mut self, dist: u64, direct: bool) {
        self.n += 1;
        if direct {
            self.dn += 1;
        }
        if dist == 0 {
            ...
            self.max_ulp = self.max_ulp.max(dist);
        }
    }
}
fn fmt(a: &Acc) -> String {
    format!("{}/{} max={} d={}/{}", a.exact, a.n, a.max_ulp, a.d, a.dn)
}

fn main() {
    let dir = env::args().nth(1).expect("dir");
    let rows = f::load_q_rows_tagged(&dir);
    let bits = [0u32, 2, 12, 14];
    println!("## Cephes mask ablation Q-tail z>=4 flush(w*F)");
    let mut masks = vec![0u32, 0x5005];
    for &b in &bits {
        masks.push(1u32 << b);
    }
    for i in 0..4 {
        for j in (i + 1)..4 {
            masks.push((1u32 << bits[i]) | (1u32 << bits[j]));
        }
    }
    for i in 0..4 {
        let mut m = 0x5005u32;
    ...
    for mask in masks {
        ...
        for r in &rows {
            ...
            let qg = qw(r.z, cephes_mask(r.z, mask));
            if !qg.is_finite() {
                continue;
            }
        ...
        println!("mask={mask:#06x} tail {}", fmt(&tail));
    }

    println!("\n## SPECFUN C/D mask + P/Q @ cut 4");
    for (name, mask) in [("C/D 0", 0u32), ("C/D 0x74", 0x74)] {
        ...
        println!("{name:12} mid {} tail {}", fmt(&mid), fmt(&tail));
    }
}
