...
fn v_prod(r: f64, n: f64, pv: f64, fv: f64, ty: f64) -> f64 {
    // recip-mult combine + x87 substrate
    let nl = -(n * rx::excel_log1p(r));
    let invf = rx::excel_exp(nl);
    let denom = -rx::excel_expm1_internal(nl);
    let tf = 1.0 + r * ty;
    let num = pv + fv * invf;
    let recip = 1.0 / (tf * denom);
    -num * r * recip
}
fn v_qf_div(r: f64, n: f64, pv: f64, fv: f64, ty: f64) -> f64 {
    // quotient-first, tf=divide, v=1+em
    let nl = -(n * rx::excel_log1p(r));
...
fn v_qf_recip(r: f64, n: f64, pv: f64, fv: f64, ty: f64) -> f64 {
    // quotient-first, tf=reciprocal-multiply
    let nl = -(n * rx::excel_log1p(r));
    let em = rx::excel_expm1_internal(nl);
    let v = 1.0 + em;
    let tf = 1.0 + r * ty;
...
fn v_qf_vexp(r: f64, n: f64, pv: f64, fv: f64, ty: f64) -> f64 {
    // quotient-first, v=exp(tau) directly
    let nl = -(n * rx::excel_log1p(r));
    let em = rx::excel_expm1_internal(nl);
...
fn v_qf_num165(r: f64, n: f64, pv: f64, fv: f64, ty: f64) -> f64 {
    // qf_div but em via x87 double-rounded numerator (165)
    let nl = -(n * rx::excel_log1p(r));
    let t = nl;
    let em = if t == 0.0 {
        0.0
...
fn main() {
    ...
    let variants: [(&str, fn(f64, f64, f64, f64, f64) -> f64); 5] = [
        ("prod(recip-mult)", v_prod),
        ("qf_div", v_qf_div),
        ("qf_recip", v_qf_recip),
        ("qf_vexp", v_qf_vexp),
        ("qf_num165", v_qf_num165),
    ];
    println!(
        "{:12} {:>6}  {}",
        "corpus",
    ...
    for cn in corpora {
        ...
        for i in 0..5 {
            print!(
                "{:>16}",
                format!("{}({:.0}%)", sc[i], 100.0 * sc[i] as f64 / tot as f64)
            );
        }
        println!();
    }
