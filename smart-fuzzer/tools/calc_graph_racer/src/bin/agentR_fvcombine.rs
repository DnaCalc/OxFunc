...
fn load(path: &str) -> Vec<Row> {
    ...
    for w in v["witnesses"].as_array().unwrap() {
        ...
        o.push(Row {
            r: g(0), n: g(1), pv: g(2), fv: g(3), ty: g(4),
...
enum M { Sse, X87 }
fn mul(m: M, a: f64, b: f64) -> f64 { match m { M::Sse => a * b, M::X87 => xmul(a, b) } }
fn div(m: M, a: f64, b: f64) -> f64 { match m { M::Sse => a / b, M::X87 => xdiv(a, b) } }
fn add(m: M, a: f64, b: f64) -> f64 { match m { M::Sse => a + b, M::X87 => xadd(a, b) } }
fn sub(m: M, a: f64, b: f64) -> f64 { match m { M::Sse => a - b, M::X87 => xsub(a, b) } }
...
fn main() {
    ...
    let sub: Vec<Row> = all.iter().cloned().filter(|r| (r.n - 360.0).abs() < 0.5).collect();
    let n0 = sub.iter().filter(|r| r.ty == 0.0).count();
    let n1 = sub.iter().filter(|r| r.ty == 1.0).count();
    println!("accurate-em subset: {} rows (ty0={}, ty1={}), rate={}", sub.len(), n0, n1, sub[0].r);

    // Pin u/em for the (single) rate/n and print candidate values.
    let r = sub[0].r; let n = sub[0].n;
    for tm in 0..4u8 {
        let (u, em0, emx) = u_em(tm, r, n);
        println!("  tau_mode={}: u=0x{:016x} em_sse=0x{:016x} em_x87=0x{:016x} 1+em=0x{:016x}",
                 tm, u.to_bits(), em0.to_bits(), emx.to_bits(), (1.0 + em0).to_bits());
    }
    ...
    let forms: Vec<(&str, fn(M, f64, f64, f64, f64) -> f64)> = vec![
        ("A1 (num/em)/tf*r ", |m, num, em, tf, r| mul(m, div(m, div(m, num, em), tf), r)),
        ("A2 (num/em)*r/tf ", |m, num, em, tf, r| div(m, mul(m, div(m, num, em), r), tf)),
        ("B1 (num*r)/em/tf ", |m, num, em, tf, r| div(m, div(m, mul(m, num, r), em), tf)),
        ("B2 (num*r)/(em*tf)", |m, num, em, tf, r| div(m, mul(m, num, r), mul(m, em, tf))),
        ("B3 (num*r)/tf/em ", |m, num, em, tf, r| div(m, div(m, mul(m, num, r), tf), em)),
        ("C1 num/(em*tf)*r ", |m, num, em, tf, r| mul(m, div(m, num, mul(m, em, tf)), r)),
        ("C2 (num/tf)/em*r ", |m, num, em, tf, r| mul(m, div(m, div(m, num, tf), em), r)),
        ("C3 (num/tf)*r/em ", |m, num, em, tf, r| div(m, mul(m, div(m, num, tf), r), em)),
    ];
    ...
    // tf formations
    let tf_of = |m: M, r: f64, ty: f64| -> f64 { if ty == 0.0 { 1.0 } else { add(m, 1.0, mul(m, r, ty)) } };

    // Full sweep
    println!("\n=== FULL SWEEP (match counts ty0/ty1/total out of {}/{}/{}) ===", n0, n1, sub.len());
    ...
    for ty_pick in [0.0f64, 1.0] {
        ...
        for row in sub.iter().filter(|r| r.ty == ty_pick) {
            ...
            let tf = if row.ty == 0.0 { 1.0 } else { 1.0 + row.r * row.ty };
    ...
    for &row in rows0.iter().take(4).chain(rows0.iter().rev().take(2)) {
        ...
        println!("  pv=0x{:016x} num=0x{:016x} want=0x{:016x} got(v=u)=0x{:016x} d={}",
                 row.pv.to_bits(), num.to_bits(), row.want, got.to_bits(),
