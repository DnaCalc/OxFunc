...
fn main() {
    ...
    for cn in corpora {
        ...
        for w in &ws.witnesses {
            ...
            if got.to_bits() == want.to_bits() { hit += 1; continue; }
            let tau = (n * rx::excel_log1p(r)).abs();
            if tau < 1.0 { m_smalltau += 1; } else { m_bigtau += 1; }
            if fv == 0.0 && ty == 0.0 { m_fv0ty0 += 1; }
            if fv != 0.0 { m_fvnz += 1; }
            if ty != 0.0 { m_tynz += 1; }
            let d = (got.to_bits() as i64 - want.to_bits() as i64).abs();
            if d <= 2 { ulp_small += 1; } else { ulp_big += 1; }
        }
        let miss = tot - hit;
        println!("{:10} N={:6} hit={:5} ({:4.1}%) miss={:5} | tau<1:{:4} tau>=1:{:4} | fv0ty0:{:4} fv!=0:{:4} ty!=0:{:4} | <=2ulp:{:4} >2ulp:{:4}",
                 cn, tot, hit, 100.0 * hit as f64 / tot as f64, miss,
                 m_smalltau, m_bigtau, m_fv0ty0, m_fvnz, m_tynz, ulp_small, ulp_big);
    }
}
