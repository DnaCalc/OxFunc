...
fn main() {
    ...
    for _ in 0..200000 {
        ...
        let fy = l1p_fyl(r);
        let sd = l1p_std(r);
        let df = ulp_diff(fy, cr);
        let ds = ulp_diff(sd, cr);
        let dfs = ulp_diff(fy, sd);
        *hf.entry(df).or_insert(0) += 1;
        *hs.entry(ds).or_insert(0) += 1;
        *hfs.entry(dfs).or_insert(0) += 1;
        if df != 0 {
            nz_f += 1;
        }
        if ds != 0 {
    ...
    println!(
        "=== log1p signature over {} general-r pts (log-uniform 3e-4..0.29) ===",
        tot
    );
    println!(
        "FYL2XP1-CR ulp hist: {:?}   (nonzero: {}/{} = {:.1}%)",
        hf,
        nz_f,
        tot,
        100.0 * nz_f as f64 / tot as f64
    );
    println!(
        "std_ln1p-CR ulp hist: {:?}   (nonzero: {}/{} = {:.1}%)",
        hs,
        nz_s,
        tot,
        100.0 * nz_s as f64 / tot as f64
    );
    println!(
        "FYL2XP1-std ulp hist: {:?}  (this is where CR-class vs std split; both-move {}/{})",
        hfs, both_move, tot
    );
    ...
    let mut n = 0;
    let mut cr_fy_diff = 0;
    let mut cr_std_diff = 0;
    let mut cr_only = 0;
    let mut fy_only = 0;
    let mut neither_crfy = 0;
    let mut both_crfy = 0;
    let mut crw = 0;
    let mut stdw = 0;
    let mut tie_crstd = 0;
    for ((rb, nb), rows) in &data {
        ...
        let nn = f64::from_bits(*nb);
        if rows.len() < 8 || exact_1pr(r) {
            continue;
        }
        let center = em_for(r, nn, l1p_cr);
        if let Some(ep) = pin_gen(rows, center, r) {
            n += 1;
            let ecr = em_for(r, nn, l1p_cr);
            let efy = em_for(r, nn, l1p_fyl);
            let esd = em_for(r, nn, l1p_std);
            if ecr.to_bits() != efy.to_bits() {
                cr_fy_diff += 1;
                let c = ecr.to_bits() == ep.to_bits();
                let f = efy.to_bits() == ep.to_bits();
                match (c, f) {
                    (true, false) => cr_only += 1,
                    (false, true) => fy_only += 1,
                    (true, true) => both_crfy += 1,
                    _ => neither_crfy += 1,
                }
            }
            if ecr.to_bits() != esd.to_bits() {
                cr_std_diff += 1;
                let c = ecr.to_bits() == ep.to_bits();
                let s = esd.to_bits() == ep.to_bits();
                match (c, s) {
                    (true, false) => crw += 1,
                    (false, true) => stdw += 1,
                    (true, true) => tie_crstd += 1,
                    _ => {}
                }
            }
        }
    ...
    println!(
        "CR!=FYL2XP1 on {} rows -> oracle: CR-only:{} FYL-only:{} both:{} neither:{}",
        cr_fy_diff, cr_only, fy_only, both_crfy, neither_crfy
    );
    println!(
        "CR!=std_ln1p on {} rows -> oracle: CR-right:{} std-right:{} both-right:{}",
        cr_std_diff, crw, stdw, tie_crstd
    );
}
