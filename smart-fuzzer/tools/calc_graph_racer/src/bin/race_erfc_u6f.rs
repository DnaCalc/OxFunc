//! leftover-low 6 of 0x5005 vs F±k last-store. Not an identity.
...
fn main() {
    ...
    println!("6 leftover-low 5005 F±k:");
    ...
    for &lz in &SIX {
        ...
        let ff = cephes(lz);
        print!("  z={lz:.16}");
        for (i, k) in (-4i32..=4).enumerate() {
            let g = mul(w, poke(ff, k));
            if ulp_distance(g, t).unwrap_or(99) == 0 {
                hit[i] += 1;
            }
            print!(" F{k:+}={}", signed(g, t));
        }
        println!();
    }
    for (i, k) in (-4i32..=4).enumerate() {
        print!("F{k:+} leftover {}/6  ", hit[i]);
    }
    println!();
    for k in -4i32..=4 {
        let mut qe = 0usize;
        ...
        for row in rows.iter().filter(|rr| rr.z >= 4.0) {
            ...
            let g = mul(f::w_rn53(row.z), poke(cephes(row.z), k));
            if ulp_distance(g, t).unwrap_or(99) == 0 {
                qe += 1;
                if row.direct {
                    qd += 1;
