...
fn binexp_sse(base: f64, mut n: u64) -> f64 { let mut a=1.0; let mut b=base; while n>0 { if n&1==1 {a*=b;} n>>=1; if n>0 {b*=b;} } a }
fn binexp_x87(base: f64, mut n: u64) -> f64 { let mut a=1.0; let mut b=base; while n>0 { if n&1==1 {a=xmul(a,b);} n>>=1; if n>0 {b=xmul(b,b);} } a }
...
fn invf_candidates(r: f64, n: f64) -> Vec<(&'static str, f64)> {
    ...
    vec![
        ...
        ("recipX87 exp(n*l1p)",  rx::x87_recip(rx::excel_exp(n * log1p))),
        ("exp(-n*lnx87(1+r))",   rx::excel_exp(-(n * lnb))),
        ("1/binexpSSE(1+r)",     1.0 / binexp_sse(base_sse, n as u64)),
        ("recipX87 binSSE",      rx::x87_recip(binexp_sse(base_sse, n as u64))),
        ...
        ("recipX87 powkern",     rx::x87_recip(power_kernel(base, n).unwrap())),
        ("binexpSSE(1/(1+r))",   binexp_sse(1.0 / base_sse, n as u64)),
        ("binexpX87(recip)",     binexp_x87(rx::x87_recip(base), n as u64)),
    ]
}
...
fn disc_b_x87(invf: f64, r: f64, pv: f64, fv: f64, ty: f64) -> f64 {
    let tf = if ty == 0.0 { 1.0 } else { xadd(1.0, xmul(r, ty)) };
...
fn main() {
    ...
    let r0 = res360[0].r; let n0 = res360[0].n;
    println!("rate={} n={}", r0, n0);
    for (nm, iv) in invf_candidates(r0, n0) {
        println!("  invF {:22} = 0x{:016x}", nm, iv.to_bits());
    }
    let groups = [("fv-1 ty0", -1.0, 0.0), ("fv-1 ty1", -1.0, 1.0), ("fv1 ty0", 1.0, 0.0), ("fv1 ty1", 1.0, 1.0)];
    ...
    for combo in ["x87", "sse"] {
        ...
        for (nm, _) in invf_candidates(r0, n0) {
            ...
            for (gn, fv, ty) in groups {
                let mut ok = 0; let mut cnt = 0;
                for row in res360.iter().filter(|r| r.fv == fv && r.ty == ty) {
                    let iv = invf_candidates(row.r, row.n).into_iter().find(|(x,_)| *x==nm).unwrap().1;
                    let got = if combo=="x87" { disc_b_x87(iv, row.r, row.pv, row.fv, row.ty) }
                              else { disc_b_sse(iv, row.r, row.pv, row.fv, row.ty) };
                    if got.to_bits() == row.want { ok += 1; } cnt += 1;
                }
                let _ = gn; line.push_str(&format!(" {}:{:3}", gn, ok)); tot += ok;
            }
    ...
    for (nm, _) in invf_candidates(r0, n0) {
        for ty in [0.0f64, 1.0] {
            ...
            let mut hist: BTreeMap<i64, usize> = BTreeMap::new(); let mut ok = 0;
            for row in &rows {
                let iv = invf_candidates(row.r, row.n).into_iter().find(|(x,_)| *x==nm).unwrap().1;
            ...
            println!("  {:22} ty={}: match={}/{} hist={:?}", nm, ty, ok, rows.len(), hist);
        }
    }
}
