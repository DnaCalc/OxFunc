//! A[2]+1 lost P-row; Q[0/1/2]+1 full DIRECT GAIN/LOSE. Not an identity.
...
fn main() {
    ...
    let mut a2 = joint;
    a2[2] = poke(joint[2], 1);
    println!("A[2]+1 LOSE (joint exact, poke miss):");
    for &(z, bits) in &rows {
        let t = f64::from_bits(bits);
        let dj = ulp_distance(erf_a(z, &joint), t).unwrap_or(99);
        let d2 = ulp_distance(erf_a(z, &a2), t).unwrap_or(99);
        if dj == 0 && d2 != 0 {
            println!("  z={z:.20} poke_ulp={d2}");
        }
    }

    let qrows = f::load_q_rows_tagged(&dir);
    let dirs: Vec<&f::QRow> = qrows
        .iter()
        .filter(|r| r.direct && r.z >= 4.0)
        .collect();
    let pins = [4.0f64, 5.0, 8.0];
    for qi in [0usize, 1, 2] {
        let mut q = CEPHES_Q;
        q[qi] = poke(CEPHES_Q[qi], 1);
        println!("Q[{qi}]+1 GAIN/LOSE / pins:");
        for r in &dirs {
            let t = f64::from_bits(r.qbits);
            let db = ulp_distance(qwf(r.z, cephes(r.z, &CEPHES_Q)), t).unwrap_or(99);
            let dp = ulp_distance(qwf(r.z, cephes(r.z, &q)), t).unwrap_or(99);
            if db == 0 && dp != 0 {
                println!("  LOSE z={:.16} poke_ulp={dp}", r.z);
            }
            if db != 0 && dp == 0 {
                println!("  GAIN z={:.16} base_ulp={db}", r.z);
            }
        }
        for &p in &pins {
            if let Some(r) = dirs.iter().find(|r| (r.z - p).abs() < 1e-12) {
                let t = f64::from_bits(r.qbits);
                let db = ulp_distance(qwf(p, cephes(p, &CEPHES_Q)), t).unwrap_or(99);
                let dp = ulp_distance(qwf(p, cephes(p, &q)), t).unwrap_or(99);
                println!("  pin {p} base={db} poke={dp}");
            }
        }
    }
}
