//! unpoked leftover-low 117 last-store k split by joint bucket. Not an identity.
...
fn main() {
    ...
    let mut nlo = 0usize;
    let mut k_all = [0usize; 9];
    let mut k_jlo = [0usize; 9];
    let mut k_j1 = [0usize; 9];
    let mut n_jlo = 0usize;
    let mut n_j1 = 0usize;
    let mut n_jf = 0usize;
    let mut n_jhi = 0usize;
    let mut n_up = 0usize;
    let mut none = 0usize;
    println!("unpoked leftover-low last-store k by joint bucket:");
    for (&zb, &eb) in &map {
        ...
        let k = k_of(z, t, ff);
        let up = ulp_distance(ff.next_up(), t).unwrap_or(99) == 0;
        if up {
            n_up += 1;
        }
        match k {
    ...
    pr("unpoked leftover-low", nlo, &k_all);
    pr("  as joint leftover-low", n_jlo, &k_jlo);
    pr("  as joint 1-ULP", n_j1, &k_j1);
    println!("  as joint fused={n_jf} leftover-high={n_jhi} none={none} last-mul next_up={n_up}");
}
