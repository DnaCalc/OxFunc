...
fn is_po2(rb: u64) -> bool {
    (rb & 0x000f_ffff_ffff_ffff) == 0
}
...
fn main() {
    ...
    for (rb, rows) in &byr {
        ...
        for o in -4..=4 {
            let mut fail = 0u32;
            for (pv, want) in rows {
                if pmt_off(r, *pv, o).to_bits() != *want {
                    fail += 1;
                }
            }
            if fail < best.0 {
    ...
    println!(
        "r with NO single-offset close (expm1/combine sub-ulp): {}",
        noclose
    );
    println!("\nEXACT power-of-two r rows (em model-free): r_log2  best_o  residual");
    for (r, o, res) in &po2rows {
        println!("  2^{:+.4}  o={:+}  res={}", r.log2(), o, res);
    }
