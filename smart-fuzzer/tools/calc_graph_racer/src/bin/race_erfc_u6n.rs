//! leftover-low 6 of 0x5005 vs last-store of other named F. Not an identity.
...
fn main() {
    ...
    println!("6 leftover-low 5005 named-F last-store:");
    let mut h5 = 0usize;
    let mut h0 = 0usize;
    let mut hf = 0usize;
    let mut hl = 0usize;
    for &lz in &SIX {
        ...
        let a = anyk(lz, t, cephes_mask(lz, 0x5005));
        let b = anyk(lz, t, cephes_mask(lz, 0));
        let c = anyk(lz, t, f::cephes_f(lz));
        let d = anyk(lz, t, libm_f(lz));
        if a {
            h5 += 1;
        }
        if b {
        ...
        println!("  z={lz:.16} 5005={a} mask0={b} cephes_f={c} libm={d}");
    }
    println!("leftover 5005-F±={h5}/6 mask0={h0} cephes_f={hf} libm={hl}");
}
