...
fn pin_em(rows:&[(f64,u64)], r:f64, center:f64)->Option<f64>{
    ...
    for d in -12..=12i64{
        ...
        if em>=0.0 { continue; }
        let ok=rows.iter().filter(|(pv,want)|{ ((pv/em)*r).to_bits()==*want }).count() as u32;
...
fn main(){
    let ws:WitnessSet=serde_json::from_str(&std::fs::read_to_string(
        "../../work/w109/G6-solvers/answers-pmt-genrate.json").unwrap()).unwrap();
    ...
    let mut pinned=0u32; let mut kmatch=0u32; let mut unpinnable=0u32;
    ...
    for ((rb,nb),rows) in &byrn{
        ...
        let km=kahan(r,n);
        let em=match pin_em(rows,r,km){Some(e)=>e,None=>{unpinnable+=1; continue}};
        pinned+=1;
        let d=(sordi(em.to_bits())-sordi(km.to_bits())).clamp(-3,3);
        *resid.entry(d).or_default()+=1;
        if tau.abs()<0.1{ *small_resid.entry(d).or_default()+=1; }
        if d==0{kmatch+=1;}
    }
    println!("generic-rate (r=m*2^-k) |tau|<1: pinned {}, unpinnable {}", pinned, unpinnable);
    println!("Kahan-model matches: {}/{} ({:.0}%)", kmatch, pinned, 100.0*kmatch as f64/pinned as f64);
    println!("signed residual (em_pinned - Kahan, real-value ulps): {:?}",
             resid.iter().map(|(k,v)|(*k,*v)).collect::<Vec<_>>());
    println!("  small |tau|<0.1 subset: {:?}",
             small_resid.iter().map(|(k,v)|(*k,*v)).collect::<Vec<_>>());
