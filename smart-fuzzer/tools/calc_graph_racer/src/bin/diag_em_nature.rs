...
fn main() {
    ...
    let (mut miss, mut fix_true_rz, mut fix_qrz, mut fix_lnu_rz, mut fix_num_rz) =
    ...
    for line in csv.lines().skip(1) {
        ...
        let num_d = (u - 1.0) * tau;
        let q_rz = ext_to_f64(
            &ext_div(&ext_from_f64(num_d), &ext_from_f64(rx::excel_ln(u)), cw),
            RZ53,
        )
        .to_bits();
        ...
        let num_rz = ext_to_f64(
            &ext_mul(
                &ext_sub(&ext_from_f64(u), &ext_one(), cw),
                &ext_from_f64(tau),
                cw,
            ),
            RZ53,
    ...
    println!(
        "  double-Kahan (RN)      == pin : {}/{}  <- production baseline",
        n_kahan, tot
    );
