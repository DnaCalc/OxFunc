...
#[derive(Clone,Copy)] enum Pow{ BinexpDouble, X87Chain, Powf, BinexpX87, ExcelPow }
fn ipow(base:f64, n:f64, m:Pow)->f64{
    match m {
        Pow::ExcelPow => if base>0.0 { rx::excel_pow_positive(base, n) } else { f64::NAN },
        Pow::BinexpDouble => {
            if n.fract()==0.0 && n>=0.0 && n<1e9 {
                let mut e=n as u64; let mut b=base; let mut r=1.0;
                while e>0 { if e&1==1 {r*=b;} e>>=1; if e>0 {b*=b;} } r
            } else { base.powf(n) }
        }
        Pow::BinexpX87 => {
            if n.fract()==0.0 && n>=0.0 && n<1e9 {
                let mut e=n as u64; let mut b=base; let mut r=1.0;
                while e>0 { if e&1==1 {r=rx::x87_mul(r,b);} e>>=1; if e>0 {b=rx::x87_mul(b,b);} } r
            } else { base.powf(n) }
        }
...
#[derive(Clone,Copy)] enum Arr{ InvRPlusType, OnePlusRType }
#[derive(Clone,Copy)] enum BalMode{ Double, Ext80, PerOp }

use rx::{Ext80, CW_PC64_RN as CW, ext_from_f64, ext_to_f64, ext_add, ext_sub, ext_mul, ext_div, ext_one};
fn e(x:f64)->Ext80{ ext_from_f64(x) }
// per-op double-rounded x87 ops: RN53(RN64(a op b)) -- the XNPV spill-loop model.
fn pa(a:f64,b:f64)->f64{ ext_to_f64(&ext_add(&e(a),&e(b),CW),CW) }
fn ps(a:f64,b:f64)->f64{ ext_to_f64(&ext_sub(&e(a),&e(b),CW),CW) }
fn pmul(a:f64,b:f64)->f64{ ext_to_f64(&ext_mul(&e(a),&e(b),CW),CW) }
fn pdiv(a:f64,b:f64)->f64{ ext_to_f64(&ext_div(&e(a),&e(b),CW),CW) }

fn balance_perop(r:f64, nper:f64, pmt:f64, pv:f64, fv:f64, ty:f64, pm:Pow, ar:Arr)->f64{
    if r==0.0 { return pa(pa(pv, pmul(pmt,nper)), fv); }
    let p = ipow(1.0+r, nper, pm);
    let pm1 = ps(p, 1.0);
    match ar {
        Arr::InvRPlusType => { // pv*P + pmt*(1/r+ty)*(P-1) + fv
            let coef = pa(pdiv(1.0,r), ty);
            let term = pmul(pmul(pmt,coef), pm1);
            pa(pa(pmul(pv,p), term), fv)
        }
        Arr::OnePlusRType => { // pv*P + pmt*(1+r*ty)*(P-1)/r + fv
...
fn balance_x87(r:f64, nper:f64, pmt:f64, pv:f64, fv:f64, ty:f64, pm:Pow, ar:Arr)->f64{
    if r==0.0 { // pv + pmt*nper + fv
        let t = ext_add(&ext_add(&e(pv), &ext_mul(&e(pmt),&e(nper),CW),CW), &e(fv), CW);
...
fn balance(r:f64, nper:f64, pmt:f64, pv:f64, fv:f64, ty:f64, pm:Pow, ar:Arr)->f64{
    if r==0.0 { return pv + pmt*nper + fv; }
    let p = ipow(1.0+r, nper, pm);
    match ar {
...
fn bal(r:f64, nper:f64, pmt:f64, pv:f64, fv:f64, ty:f64, pm:Pow, ar:Arr, bm:BalMode)->f64{
    match bm { BalMode::Double=>balance(r,nper,pmt,pv,fv,ty,pm,ar),
               BalMode::Ext80=>balance_x87(r,nper,pmt,pv,fv,ty,pm,ar),
               BalMode::PerOp=>balance_perop(r,nper,pmt,pv,fv,ty,pm,ar) }
}

#[derive(Clone,Copy)] enum Deriv{ FdRel1e6, FdRel1e7, FdAbs1e6, Analytic }
fn fprime(x:f64, f:f64, nper:f64, pmt:f64, pv:f64, fv:f64, ty:f64, pm:Pow, ar:Arr, dv:Deriv, bm:BalMode)->f64{
    match dv {
        Deriv::Analytic => {
        ...
        _ => {
            let h = match dv {
                Deriv::FdRel1e6 => if x!=0.0 {1e-6*x} else {1e-6},
                Deriv::FdRel1e7 => if x!=0.0 {1e-7*x} else {1e-7},
                Deriv::FdAbs1e6 => 1e-6,
...
fn rate_solve(nper:f64,pmt:f64,pv:f64,fv:f64,ty:f64,guess:f64,pm:Pow,ar:Arr,dv:Deriv,bm:BalMode)->Option<f64>{
    let mut x=guess;
    for _ in 0..CAP {
        let f = bal(x,nper,pmt,pv,fv,ty,pm,ar,bm);
        let d = fprime(x,f,nper,pmt,pv,fv,ty,pm,ar,dv,bm);
        if !f.is_finite() || !d.is_finite() || d==0.0 { return None; }
        let xn = x - f/d;
        if !xn.is_finite() { return None; }
        if f.abs() < TOL { return Some(xn); }  // stop on residual, publish stepped iterate
        x = xn;
    }
    None
}
...
fn main(){
    ...
    for f in files{
        ...
        if std::path::Path::new(&p).exists(){ for row in load(&p){ all.push((f,row)); } }
    }
    println!("total rate witnesses: {}", all.len());
    let pows=[("binexpDbl",Pow::BinexpDouble),("x87chain",Pow::X87Chain),("powf",Pow::Powf),("binexpX87",Pow::BinexpX87),("excelPow",Pow::ExcelPow)];
    let arrs=[("(1/r+ty)",Arr::InvRPlusType),("(1+r*ty)/r",Arr::OnePlusRType)];
    let derivs=[("fdRel1e6",Deriv::FdRel1e6),("fdRel1e7",Deriv::FdRel1e7),("fdAbs1e6",Deriv::FdAbs1e6),("analytic",Deriv::Analytic)];
    let bals=[("dbl",BalMode::Double),("x87",BalMode::Ext80),("perop",BalMode::PerOp)];
    let mut best=(0i64,String::new());
    for (bn,bm) in bals{
    for (dn,dv) in derivs{
        for (pn,pm) in pows{
            for (an,ar) in arrs{
                let (mut exact,mut num_ok,mut val_ok,mut numtot,mut valtot,mut w32)=(0,0,0,0,0,0);
                for (_f,(a,want,_id)) in &all{
                    let got=rate_solve(a[0],a[1],a[2],a[3],a[4],a[5],pm,ar,dv,bm);
                    match (got,want){
                        (None,None)=>{num_ok+=1; numtot+=1;}
                        (Some(g),Some(w))=>{ valtot+=1;
                            let d=(g.to_bits() as i64 - *w as i64).abs();
                            if d==0 {exact+=1; val_ok+=1;} if d<=32 {w32+=1;} }
                        (Some(_),None)=>{ numtot+=1; }
                        (None,Some(_))=>{ valtot+=1; }
                    }
                }
                println!("bal={:3} deriv={:9} pow={:9} arr={:11}: exact {:3}/{:3}  w<=32 {:3}  #NUM! {:2}/{:2}",
                    bn,dn,pn,an,val_ok,valtot,w32,num_ok,numtot);
                if exact as i64>best.0 { best=(exact as i64, format!("bal={}/{}/{}/{}",bn,dn,pn,an)); }
            }
        }
    }
    }
    ...
    for (bn,bm) in [("x87",BalMode::Ext80),("dbl",BalMode::Double)]{
        println!("\n--- detail: bal={} / fdRel1e6 / x87chain / (1/r+ty) ---", bn);
        let mut basin_bad=0; let mut shown=0;
        for (_f,(a,want,id)) in &all{
            let got=rate_solve(a[0],a[1],a[2],a[3],a[4],a[5],Pow::X87Chain,Arr::InvRPlusType,Deriv::FdRel1e6,bm);
            let m=match(got,want){
                (Some(g),Some(w))=>if g.to_bits()==*w{None}else{Some(format!("{:+} ulp",g.to_bits() as i64 - *w as i64))},
                (None,None)=>None,
                (Some(_),None)=>{basin_bad+=1; Some("BASIN got val want NUM".into())},
                (None,Some(_))=>{basin_bad+=1; Some("BASIN got NUM want val".into())}};
            if let Some(s)=m { if shown<20 {println!("  {:16} {}", id, s); shown+=1;} }
        }
    ...
    let near: Vec<_> = all.iter().filter(|(_f,(a,_w,_i))| (a[5]-ROOT_A).abs()<1e-7 && a[0]==10.0).collect();
    println!("\n=== NEAR-ROOT one-step ladder: {} rows ===", near.len());
    let mut rank=Vec::new();
    for (bn,bm) in [("dbl",BalMode::Double),("x87",BalMode::Ext80),("perop",BalMode::PerOp)]{
        for (dn,dv) in derivs{
            for (pn,pm) in pows{
                for (an,ar) in arrs{
                    let (mut ex,mut w4,mut w32)=(0i32,0i32,0i32);
                    for (_f,(a,want,_i)) in &near{
                        if let (Some(g),Some(w))=(rate_solve(a[0],a[1],a[2],a[3],a[4],a[5],pm,ar,dv,bm),want){
                            let d=(g.to_bits() as i64 - *w as i64).abs();
                            if d==0{ex+=1;} if d<=4{w4+=1;} if d<=32{w32+=1;}
                        }
                    }
    ...
    for (ex,w4,w32,name) in rank.iter().take(12){
        println!("  near-root exact {:2}  w<=4 {:2}  w<=32 {:2}   {}", ex,w4,w32,name);
    }
    ...
    println!("  {:>6} {:>8} {:>8} {:>8}  {:>14} {:>14} {:>8}", "guessΔ","exOut-g","myOut-g","out-ex","my_f","implied_f","fΔulp");
    let (pm,ar,dv,bm)=(Pow::X87Chain,Arr::InvRPlusType,Deriv::FdRel1e6,BalMode::Double);
    let root_bits = ROOT_A.to_bits() as i64;
    for (_f,(a,want,_i)) in near.iter().take(24){
        let g=a[5]; let gb=g.to_bits() as i64;
        ...
        if let Some(w)=want{
            ...
            println!("  {:>6} {:>8} {:>8} {:>8}  {:>14.4e} {:>14.4e} {:>8}",
                gb-root_bits, *w as i64 - gb, myout.to_bits() as i64 - gb,
                myout.to_bits() as i64 - *w as i64, f, implied_f, fdulp);
        }
    }
}
