...
fn pmt(r:f64,n:f64,pv:f64,fv:f64,ty:f64, v_is_exp:bool)->f64{
    ...
    let v = if v_is_exp { rx::excel_exp(tau) } else { 1.0+em };
    let tf=1.0+r*ty;
    let num=pv+fv*v;
    let q1=num/em; let q2=q1/tf;
    q2*r
}
fn main(){
    let corpora=["heldout","combsweep","po2","r25","pvladder","fvty","fvsweep","fv1sweep"];
    for cn in corpora{
        ...
        let mut ok1=0u32; let mut okE=0u32; let mut tot=0u32;
        // fv!=0 subset
        let mut ok1_fv=0u32; let mut okE_fv=0u32; let mut tot_fv=0u32;
        for w in &ws.witnesses{
            ...
            tot+=1; if h1{ok1+=1;} if hE{okE+=1;}
            if a[3]!=0.0{ tot_fv+=1; if h1{ok1_fv+=1;} if hE{okE_fv+=1;} }
        }
        println!("{:10} N={:6}  v=1+em {:>5}/{} ({:.0}%)   v=exp {:>5}/{} ({:.0}%)   [fv!=0: 1+em {}/{}  exp {}/{}]",
            cn, tot, ok1, tot, 100.0*ok1 as f64/tot as f64, okE, tot, 100.0*okE as f64/tot as f64,
            ok1_fv, tot_fv, okE_fv, tot_fv);
    }
}
