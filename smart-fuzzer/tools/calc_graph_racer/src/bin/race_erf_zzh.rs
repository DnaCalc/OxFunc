//! 4 LOW: joint at (z+z)/2 and 2z store. Not an identity.
...
fn main() {
    ...
    let graphs: [(&str, Box<dyn Fn(f64) -> f64>); 6] = [
        ("joint", Box::new(joint)),
        ("(z+z)/2", Box::new(|z| joint((z + z) * 0.5))),
        ("z+z x87 /2", Box::new(|z| {
            let s = ext_to_f64(&ext_add(&ef(z), &ef(z), CW), CW);
            joint(s * 0.5)
        })),
        ("x87 (z+z)*0.5", Box::new(|z| {
            joint(ext_to_f64(
                &ext_mul(&ext_add(&ef(z), &ef(z), CW), &ef(0.5), CW),
                CW,
            ))
        })),
        ("2z f64/2", Box::new(|z| joint((2.0 * z) * 0.5))),
        ("z*1", Box::new(|z| joint(z * 1.0))),
    ];
    println!("4 LOW (z+z)/2 arg:");
    for &lz in &LOWS {
    ...
    for (name, ev) in &graphs {
        ...
        println!("{name:14} hit={hit}/4 keep={keep}/{}", rows.len());
    }
}
