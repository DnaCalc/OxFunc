...
use rx::{
    CW_PC53_RN, CW_PC64_RN, ext_add, ext_from_f64, ext_fyl2x, ext_fyl2xp1, ext_ln2, ext_mul,
    ext_one, ext_to_f64,
};
...
fn c_cr(r: f64) -> f64 {
    rx::excel_log1p(r)
} // portable correctly-rounded
fn c_ln_dbl(r: f64) -> f64 {
    rx::excel_ln(1.0 + r)
} // ln(1+r), 1+r formed in double
fn c_fyl2xp1_64(r: f64) -> f64 {
    // raw FYL2XP1 hw, PC64
    ext_to_f64(
        &ext_fyl2xp1(&ext_ln2(), &ext_from_f64(r), CW_PC64_RN),
        CW_PC64_RN,
    )
}
fn c_fyl2xp1_53(r: f64) -> f64 {
    // raw FYL2XP1 hw, PC53
    ext_to_f64(
        &ext_fyl2xp1(&ext_ln2(), &ext_from_f64(r), CW_PC53_RN),
        CW_PC53_RN,
    )
}
fn c_fyl2x_extu(r: f64) -> f64 {
    // FYL2X on EXACT ext(1+r), PC64
    let u = ext_add(&ext_one(), &ext_from_f64(r), CW_PC64_RN); // exact for |r|<1
    ext_to_f64(&ext_fyl2x(&ext_ln2(), &u, CW_PC64_RN), CW_PC64_RN)
}
fn c_fyl2x_dblu(r: f64) -> f64 {
    // FYL2X on double 1+r, PC64
...
fn main() {
    ...
    println!(
        "unambiguous rows: {}   (non-CR among them: {})",
        tot_unamb, nonc
    );
    ...
    for (i, (nm, _)) in cands.iter().enumerate() {
        println!(
            "{:<16} {:>4}/{:<4} {:>5}/{:<4}",
            nm, score[i], tot_unamb, nonc_hit[i], nonc
        );
    }
    println!(
        "\nFYL2XP1 pc64 misses (r_log2, excel-hw ulps): {}",
        fy_miss.len()
    );
