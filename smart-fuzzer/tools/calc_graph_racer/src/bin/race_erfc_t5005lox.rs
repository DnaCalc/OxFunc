//! Exclusive leftover-low z of 0x5005 vs 66 and 1-ULP |k|. Not an identity.
...
fn main() {
    ...
    let mut n66 = 0usize;
    let mut nsh = 0usize;
    println!("exclusive leftover-low 0x5005 vs 66:");
    for r in rows.iter().filter(|rr| rr.direct && rr.z >= 4.0) {
        let t = f64::from_bits(r.qbits);
        let w = f::w_rn53(r.z);
        let g5 = mul(w, cephes(r.z, &P0, &Q0, 0x5005));
        ...
        if lo5 && lo66 {
            ...
            println!(
                "  only5005 z={:.16} grid={} k={k:?} last-mul_up={up} 66={}",
                r.z,
                grid(r.z),
                bucket(g66, t)
            );
            ...
            println!(
                "  only66 z={:.16} grid={} k={k:?} last-mul_up={up} 5005={}",
                r.z,
                grid(r.z),
                bucket(g5, t)
            );
    ...
    println!("leftover-low n5005={n5} n66={n66} shared={nsh}");
    let mut u5 = [0usize; 9];
    ...
    println!("1-ULP |k| 0x5005 vs 66:");
    for r in rows.iter().filter(|rr| rr.direct && rr.z >= 4.0) {
        let t = f64::from_bits(r.qbits);
        let w = f::w_rn53(r.z);
        let d5 = ulp_distance(mul(w, cephes(r.z, &P0, &Q0, 0x5005)), t).unwrap_or(99);
        let d66 = ulp_distance(mul(w, cephes(r.z, &p66, &q66, 0x24a5)), t).unwrap_or(99);
    ...
    print!("0x5005 1-ULP n={nu5} unmask |k|");
    for (i, c) in u5.iter().enumerate() {
        if *c > 0 {
            print!(" {i}:{c}");
        }
    }
    print!("\n66 1-ULP n={nu66} unmask |k|");
