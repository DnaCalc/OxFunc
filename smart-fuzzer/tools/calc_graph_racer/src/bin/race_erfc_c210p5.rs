//! 0x210 Cody Horner + C/D 5-coeff ±1 DIRECT [0.5,4). Not an identity.
...
fn main() {
    ...
    let mut best_s = base;
    let mut best_t = (0usize, 0i32, 0, 0i32, 0, 0i32, 0, 0i32, 0, 0i32);
    let mut nbeat = 0usize;
    let mut ngt114 = 0usize;
    let mut nge114 = 0usize;
    let mut ncfg = 0usize;
    let mut top: Vec<(usize, usize, i32, usize, i32, usize, i32, usize, i32, usize, i32)> =
        Vec::new();
    for i in 0..17usize {
        for j in (i + 1)..17 {
            for k in (j + 1)..17 {
                                ...
                                for &sk in &[-1i32, 1] {
                                    for &sm in &[-1i32, 1] {
                                        for &sn in &[-1i32, 1] {
                                            ...
                                            if s > 114 {
                                                ngt114 += 1;
                                                top.push((s, i, si, j, sj, k, sk, m, sm, n, sn));
                                            }
                                            if s > best_s {
                                                best_s = s;
                                                best_t = (i, si, j, sj, k, sk, m, sm, n, sn);
                                            }
                                        }
                                    }
                                }
    ...
    println!(
        "5-coeff on 0x210 n={ncfg} beat105={nbeat} best={best_s} {}{:+} {}{:+} {}{:+} {}{:+} {}{:+} ge114={nge114} gt114={ngt114}",
        name(best_t.0),
        best_t.1,
        name(best_t.2),
        best_t.3,
