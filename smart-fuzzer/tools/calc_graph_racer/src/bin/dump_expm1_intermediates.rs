...
fn main(){
    ...
    for ((rb,nb),rows) in &byrn{
        ...
        let t=tau_d(r,n); if t.abs()>=1.0{continue}
        let em_ex=match pin_em(rows,r){Some(e)=>e,None=>continue};
        let u=rx::excel_exp(t); let lnu=rx::excel_ln(u);
        let aprod=(u-1.0)*t/lnu;
        s.push_str(&format!("{},{},{:016x},{:016x},{:016x},{:016x},{:016x}\n",
            r.log2() as i64, n as i64, t.to_bits(), u.to_bits(), lnu.to_bits(),
            em_ex.to_bits(), aprod.to_bits()));
    }
    std::fs::write("../../work/w109/G6-solvers/expm1_intermediates.csv", &s).unwrap();
    println!("wrote expm1_intermediates.csv ({} lines)", s.lines().count()-1);
}
