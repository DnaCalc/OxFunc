...
fn l1p_lnfl(r: f64) -> f64 {
    rx::excel_ln(1.0 + r)
} // ln(fl(1+r)) double-rounded 1+r, ln=x87 -- control
...
fn l1p_fdlibm(x: f64) -> f64 {
    ...
    if k != 0 {
        if hx < 0x43400000 {
            ...
            let mut u2 = u;
            if hub < 0x6a09e {
                let bits = (u2.to_bits() & 0x0000_0000_ffff_ffff)
                    | (((hub | 0x3ff00000) as u32 as u64) << 32);
                u2 = f64::from_bits(bits);
            } else {
                k += 1;
                let bits = (u2.to_bits() & 0x0000_0000_ffff_ffff)
                    | (((hub | 0x3fe00000) as u32 as u64) << 32);
                u2 = f64::from_bits(bits);
...
fn l1p_cephes(x: f64) -> f64 {
    ...
    } // cephes uses log(z); use x87 ln (matches Excel ln substrate)
    let z2 = x * x;
...
const CANDS: [(&str, L1p); 10] = [
    ("CR", l1p_cr),
    ("FYL2XP1hyb", l1p_fyl),
    ("FYL2XP1all", l1p_fylall),
    ("std_ln1p", l1p_std),
    ("kahan", l1p_kahan),
    ("kahan2", l1p_kahan2),
    ("fdlibm", l1p_fdlibm),
    ("cephes", l1p_cephes),
    ("cephes_crln", l1p_cephes_crln),
    ("lnfl_ctrl", l1p_lnfl),
];
...
fn main() {
    ...
    // buckets by |tau_cr|
    let mut hi = [0u32; 10];
    let mut hi_tot = 0u32; // |tau|>=1 : CLEAN log1p
    let mut lo = [0u32; 10];
    let mut lo_tot = 0u32; // |tau|<1  : expm1-confounded
    ...
    println!(
        "{:<14} {:>14} {:>18} {:>14}",
        "candidate", "clean |tau|>=1", "confound |tau|<1", "TOTAL"
    );
    ...
    for w in &ws.witnesses {
        ...
        if pmt_full(r, n, pv, fv, ty, l1p_cr).to_bits() == want {
            continue;
        }
        miss += 1;
        // nudge tau directly
        ...
        match fixed {
            Some(d) => *hist.entry(d).or_insert(0) += 1,
            None => unfixed += 1,
        }
    }
    println!("clean CR misses: {}", miss);
    for (d, c) in &hist {
        println!("  tau nudge {:+} ulp : {}", d, c);
    }
    println!(
        "  unfixed within +-6 ulp (NOT a small-tau issue): {}",
        unfixed
    );
}
