//! leftover-high of 0x5005 vs last-store of other named F. Not an identity.
...
fn anyk(z: f64, t: f64, ff: f64) -> bool {
    (1..=5).any(|k| ulp_distance(mul(f::w_rn53(z), poke(ff, -k)), t).unwrap_or(99) == 0)
}
fn libm_f(z: f64) -> f64 {
    let w = f::w_rn53(z);
    if w == 0.0 {
...
fn main() {
    ...
    println!("0x5005 leftover-high n={} named-F last-store:", highs.len());
    let mut h5 = 0usize;
    let mut h0 = 0usize;
    let mut hf = 0usize;
    let mut hl = 0usize;
    ...
    for &(z, bits) in &highs {
        ...
        let a = anyk(z, t, cephes_mask(z, 0x5005));
        let b = anyk(z, t, cephes_mask(z, 0));
        ...
        if !(b || c || d) {
            println!("  MISS-other z={z:.16} 5005={a}");
        }
    }
    println!(
        "leftover-high 5005-F-={h5}/{} mask0={h0} cephes_f={hf} libm={hl} other-union={hu}",
        highs.len()
    );
}
