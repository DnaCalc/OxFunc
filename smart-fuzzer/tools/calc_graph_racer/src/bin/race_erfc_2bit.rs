//! Exhaustive 2-bit Cody Horner stores vs 0x210 DIRECT [0.5,4). Not an identity.
...
fn main() {
    ...
    for i in 0..bits.len() {
        for j in (i + 1)..bits.len() {
            ...
            let s = score(mask);
            ranked.push((s, mask, a, b));
        }
    }
    ranked.sort_by(|x, y| y.0.cmp(&x.0));
    let s210 = score(0x210);
    println!("unmask={} 0x210={s210}/{n} pairs={}", score(0), ranked.len());
    println!("top 12 2-bit:");
    for (s, mask, a, b) in ranked.iter().take(12) {
        println!(
            "  fused={s} mask={mask:#x} {} + {} {}",
            label(*a),
            label(*b),
            if *mask == 0x210 { "<- 0x210" } else { "" }
        );
    }
