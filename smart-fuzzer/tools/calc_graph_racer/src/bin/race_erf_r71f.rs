//! 71 remaining 1-ULP vs fused fd P[0]+4 and m856. Not an identity.
...
fn fd4(z: f64) -> f64 {
    ...
    ext_to_f64(&ext_add(&ef(z), &ext_mul(&ef(z), &y, CW), CW), CW)
}
fn m856(z: f64) -> f64 {
    let mut a = M77;
    a[1] = poke(M77[1], -4);
    ...
    ext_to_f64(
        &ext_mul(&ef(z.abs()), &ext_add(&ef(1.0), &y, CW), CW),
        CW,
    )
}
...
fn main() {
    ...
    let mut h_m = 0usize;
    let mut h_u = 0usize;
    println!("remaining 1-ULP vs fused fd/m856:");
    for &(z, bits) in &rows {
        ...
        n += 1;
        let fd = ulp_distance(fd4(z), t).unwrap_or(99) == 0;
        let mm = ulp_distance(m856(z), t).unwrap_or(99) == 0;
        if fd {
            h_fd += 1;
        }
        if mm {
    ...
    println!("n={n} fused-fd={h_fd} fused-m856={h_m} union={h_u}");
}
