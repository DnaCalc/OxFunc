//! 3 stubborn 0x5005 leftover-low vs named F / last-store. Not an identity.
...
fn main() {
    ...
    let graphs: [(&str, Box<dyn Fn(f64) -> f64>); 8] = [
        ("5005", Box::new(|z| qw(z, cephes_mask(z, 0x5005)))),
        (
            "up(5005)",
            Box::new(|z| qw(z, cephes_mask(z, 0x5005)).next_up()),
        ),
        (
            "up2",
            Box::new(|z| qw(z, cephes_mask(z, 0x5005)).next_up().next_up()),
        ),
        ("24a5", Box::new(|z| qw(z, cephes_mask(z, 0x24a5)))),
        ("codyPQ", Box::new(|z| qw(z, cody_pq(z)))),
        ("codyF", Box::new(|z| qw(z, f::cody_erfcx_f(z)))),
        ("cephes_f", Box::new(|z| qw(z, f::cephes_f(z)))),
        ("libm", Box::new(|z| libm::erfc(z))),
    ];
    println!("3 stubborn leftover-low:");
    for &lz in &STUB {
