//! 4 LOW: (1+w)±k ulp × original z. Dual of lead+2. Not an identity.
...
fn mul(z: f64, ow: f64) -> f64 {
    ext_to_f64(&ext_mul(&ef(z), &ef(ow), CW), CW)
}
...
fn main() {
    ...
    let graphs: [(&str, Box<dyn Fn(f64) -> f64>); 6] = [
        ("fused", Box::new(|z| mul(z, one_w(z)))),
        ("ow+1", Box::new(|z| mul(z, one_w(z).next_up()))),
        (
            "ow+2",
            Box::new(|z| mul(z, one_w(z).next_up().next_up())),
        ),
        ("ow-1", Box::new(|z| mul(z, one_w(z).next_down()))),
        (
            "ow+3",
            Box::new(|z| mul(z, one_w(z).next_up().next_up().next_up())),
        ),
        (
            "f64mul",
            Box::new(|z| z * one_w(z)),
