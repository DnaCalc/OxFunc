//! 0x210 DIRECT 1-ULP vs unmask/cephes/libm F±1..2. Not an identity.
...
fn main() {
    ...
    let mut hli = 0usize;
    let mut hu = 0usize;
    println!("0x210 DIRECT 1-ULP vs other named F±1..2:");
    for r in rows.iter().filter(|rr| rr.direct && rr.z >= 0.5 && rr.z < 4.0) {
        ...
        n1 += 1;
        let b = anyk(r.z, t, cody_mask(r.z, 0));
    ...
    println!("n={n1} unmask={h0} cephes={hce} libm={hli} other-union={hu}");
}
