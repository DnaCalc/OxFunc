...
fn sm(a: f64, b: f64) -> f64 { ext_to_f64(&ext_mul(&ext_from_f64(a), &ext_from_f64(b), CW_PC64_RN), RN53) }
fn sd(a: f64, b: f64) -> f64 { ext_to_f64(&ext_div(&ext_from_f64(a), &ext_from_f64(b), CW_PC64_RN), RN53) }
fn sa(a: f64, b: f64) -> f64 { ext_to_f64(&ext_add(&ext_from_f64(a), &ext_from_f64(b), CW_PC64_RN), RN53) }
...
fn main() {
    ...
    for cn in corpora {
        ...
        let names = ["A1 landed", "A2 v=u", "A3 r-in-num", "A4 tf-recip", "A5 posP",
                     "A6 spill-comb", "A7 num=fv*v+pv", "A8 tf-last-div"];
        let mut sc = [0u32; 8];
        let mut tot = 0u32;
        for w in &ws.witnesses {
            ...
            if nl.abs() < 1.0 { continue; } // only |tau|>=1 where em is exact
            let u = rx::excel_exp(nl);
            let em = u - 1.0; if em == 0.0 { continue; }
            let v = 1.0 + em; let tf = 1.0 + r * ty;
            ...
            let cands = [
                ...
                { let pp = 1.0 / v; -(pv * pp + fv) * r / ((pp - 1.0) * tf) }, // A5 positive-power
                { let vv = sa(1.0, em); let tff = sa(1.0, sm(r, ty)); let num = sa(pv, sm(fv, vv));
                  sm(sd(sd(num, em), tff), r) },                   // A6 spill combine
                ((fv * v + pv) / em / tf) * r,                     // A7 order swap
                ((pv + fv * v) / em / tf * r) ,                    // A8 (= A1 assoc)
            ];
            for i in 0..8 { if cands[i].to_bits() == want { sc[i] += 1; } }
        }
        print!("{:9} |tau>=1| N={:5} :", cn, tot);
        for i in 0..8 { print!("  {}={:.0}%", names[i], 100.0 * sc[i] as f64 / tot.max(1) as f64); }
        println!();
    }
}
