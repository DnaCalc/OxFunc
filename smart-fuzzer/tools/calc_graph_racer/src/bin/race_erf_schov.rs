//! Overlap of Schonfelder x87-cont vs NSWC A21 exacts on P-side z<0.5.
//! Not an identity.
...
fn main() {
    ...
    let mut band_a = [0usize; 5];
    let mut band_n = [0usize; 5];
    let mut or_exact = 0usize;
    for &(z, pbits) in &rows {
        let ds = ulp_distance(erf_sch(z), f64::from_bits(pbits)).unwrap_or(99);
        let da = ulp_distance(erf_a21(z), f64::from_bits(pbits)).unwrap_or(99);
    ...
    println!(
        "n={} both={} only_sch={} only_a21={} neither={} union={}",
        rows.len(),
        both,
        only_s,
        only_a,
