//! joint ∪ last-store ± ∪ two-mode-up ∪ two-mode-down. Not an identity.
...
fn main() {
    ...
    let hit_ls = |z: f64, t: f64| {
        let ow = one_w(z);
        let owf = ext_to_f64(&ow, CW);
        [-2i32, -1, 1, 2].iter().any(|&k| {
            ulp_distance(ext_to_f64(&ext_mul(&ef(poke(z, k)), &ow, CW), CW), t)
                .unwrap_or(99)
                == 0
                || ulp_distance(
                    ext_to_f64(&ext_mul(&ef(z), &ef(poke(owf, k)), CW), CW),
                    t,
                )
                .unwrap_or(99)
                    == 0
    ...
    let hit_up = |z: f64, t: f64| {
        ulp_distance(
            ext_to_f64(&ext_mul(&ef(z.next_up()), &one_w(z), CW_RU), CW_RU),
            t,
        )
        .unwrap_or(99)
            == 0
    ...
    let hit_dn = |z: f64, t: f64| {
        ulp_distance(
            ext_to_f64(&ext_mul(&ef(z.next_down()), &one_w(z), CW_RD), CW_RD),
            t,
        )
        .unwrap_or(99)
            == 0
    ...
    let mut remain: Vec<(f64, u64)> = Vec::new();
    for &(z, bits) in &rows {
        ...
        if ej || el || eu || ed {
            ...
            remain.push((z, ulp_distance(g, t).unwrap_or(99)));
        }
        ...
        if d >= 2 && g > t {
            ...
            if el || eu || ed {
                ...
                println!("  HIGH-MISS z={z:.16}");
            }
        }
    }
    println!(
        "keep J={n_j} ls={n_ls} up={n_up} down={n_dn} any={n_any}/{} leftover-LOW {hit_low}/{n_low} leftover-HIGH {hit_high}/{n_high}",
        rows.len()
    );
    let mut n1 = 0usize;
    let mut nge2 = 0usize;
    println!("remaining {}:", remain.len());
    for (z, d) in &remain {
        if *d == 1 {
            n1 += 1;
        } else {
    ...
    println!("remaining ulp1={n1} ulp>=2={nge2}");
}
