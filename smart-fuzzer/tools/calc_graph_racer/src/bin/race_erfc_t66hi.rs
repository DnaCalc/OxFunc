//! 66 leftover-high as 0x5005 buckets DIRECT z>=4. Not an identity.
...
fn main() {
    ...
    let mut n = 0usize;
    let mut b = [0usize; 4];
    println!("66 leftover-high as 0x5005:");
    for r in &tail {
        let t = f64::from_bits(r.qbits);
        let g66 = mul(f::w_rn53(r.z), cephes(r.z, &p66, &q66, 0x24a5));
        let d66 = ulp_distance(g66, t).unwrap_or(99);
        if !(d66 >= 2 && g66 > t) {
            continue;
        }
        n += 1;
        let bb = bucket(mul(f::w_rn53(r.z), cephes(r.z, &P0, &Q0, 0x5005)), t);
        b[idx(bb)] += 1;
        if bb != "leftover-high" {
            println!("  z={:.16} d66={d66} 0x5005={bb}", r.z);
        }
    }
    print!("66 leftover-high n={n} as 0x5005");
