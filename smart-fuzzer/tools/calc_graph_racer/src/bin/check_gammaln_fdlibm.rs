...
impl V {
    ...
    fn add(self, o: V) -> V {
        V { e: rx::ext_add(&self.e, &o.e, CW), mode: self.mode }.op()
    }
    fn sub(self, o: V) -> V {
        V { e: rx::ext_sub(&self.e, &o.e, CW), mode: self.mode }.op()
    }
    fn mul(self, o: V) -> V {
        V { e: rx::ext_mul(&self.e, &o.e, CW), mode: self.mode }.op()
    }
    fn div(self, o: V) -> V {
        V { e: rx::ext_div(&self.e, &o.e, CW), mode: self.mode }.op()
    }
    fn neg(self) -> V {
...
fn fdlibm_log(x0: f64, mode: Mode) -> f64 {
    ...
    if (0x000fffff & (2 + hx)) < 3 {
        if f.f() == 0.0 {
            ...
            let dk = k as f64;
            return V::new(dk * LN2_HI, m).add(V::new(dk, m).mul(V::new(LN2_LO, m))).f();
        }
        let r = f.mul(f).mul(V::new(0.5, m).sub(V::new(0.33333333333333333, m).mul(f))).st();
        if k == 0 {
            return f.sub(r).f();
        }
        let dk = V::new(k as f64, m);
        return dk.mul(V::new(LN2_HI, m)).st()
    ...
    if (ii | j) > 0 {
        ...
        dk.mul(V::new(LN2_HI, m)).st()
            .sub(
                hfsq.sub(s.mul(hfsq.add(r)).add(dk.mul(V::new(LN2_LO, m)))).st()
                    .sub(f)
                    .st(),
            )
            .f()
        ...
        dk.mul(V::new(LN2_HI, m)).st()
            .sub(s.mul(f.sub(r)).sub(dk.mul(V::new(LN2_LO, m))).st().sub(f).st())
            .f()
    }
}
...
const VC: [f64; 5] = [
    2.45597793713041134822e+00, 2.12848976379893395361e+00, 7.69285150456672783825e-01,
...
const RC: [f64; 6] = [
    1.39200533467621045958e+00, 7.21935547567138069525e-01, 1.71933865632803078993e-01,
...
fn lgamma_pos(x: f64, mode: Mode, log: &dyn Fn(f64) -> f64) -> f64 {
    ...
    if ix < 0x40000000 {
        ...
        match i {
            0 => {
                ...
                let p2 = z.mul(horner(z, &[A[11], A[9], A[7], A[5], A[3], A[1]])).st();
                let p = y.ma(p1, p2).st();
    ...
    if ix < 0x40200000 {
        ...
        let p = y.mul(horner(y, &[S[6], S[5], S[4], S[3], S[2], S[1], S[0]])).st();
...
fn main() {
    ...
    for (mode, mname) in modes {
        ...
        for (lname, log) in &logs {
            ...
            println!("{mname:6}+{lname:11} {exact}/{} max {max_ulp}  miss {regmiss:?}", rows.len());
        }
    }
}
