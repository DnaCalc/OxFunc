//! Exclusive leftover-low z of 0x24a5 vs 66 and 1-ULP |k|. Not an identity.
...
fn main() {
    ...
    let mut n66 = 0usize;
    let mut nsh = 0usize;
    println!("exclusive leftover-low 0x24a5 vs 66:");
    for r in rows.iter().filter(|rr| rr.direct && rr.z >= 4.0) {
        let t = f64::from_bits(r.qbits);
        let w = f::w_rn53(r.z);
        let g5 = mul(w, cephes(r.z, &P0, &Q0, 0x24a5));
        ...
        if lo5 && lo66 {
            ...
            println!(
                "  only24a5 z={:.16} grid={} k={k:?} last-mul_up={up} 66={}",
                r.z,
                grid(r.z),
                bucket(g66, t)
            );
            ...
            println!(
                "  only66 z={:.16} grid={} k={k:?} last-mul_up={up} 24a5={}",
                r.z,
                grid(r.z),
                bucket(g5, t)
            );
    ...
    println!("leftover-low n24a5={n5} n66={n66} shared={nsh}");
    let mut u5 = [0usize; 9];
    ...
    println!("1-ULP |k| 0x24a5 vs 66:");
    for r in rows.iter().filter(|rr| rr.direct && rr.z >= 4.0) {
        let t = f64::from_bits(r.qbits);
        let w = f::w_rn53(r.z);
        let d5 = ulp_distance(mul(w, cephes(r.z, &P0, &Q0, 0x24a5)), t).unwrap_or(99);
    ...
    print!("0x24a5 1-ULP n={nu5} unmask |k|");
