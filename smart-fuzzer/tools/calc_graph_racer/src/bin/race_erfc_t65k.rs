//! ±2 steps on Q[1],Q[2],Q[3] of the 65 triple on 0x4e05. Not an identity.
...
fn main() {
    ...
    let n = tail.len();
    let score = |q: &[f64; 8]| -> usize {
        tail.iter()
            .filter(|rr| {
                let t = f64::from_bits(rr.qbits);
                ulp_distance(mul(f::w_rn53(rr.z), cephes(rr.z, q)), t).unwrap_or(99) == 0
            })
            .count()
    };
    println!("0x4e05 Q[1],Q[2],Q[3] ±2 DIRECT z>=4 n={n}");
    let mut ranked: Vec<(usize, i32, i32, i32)> = Vec::new();
    for a in -2i32..=2 {
        for b in -2i32..=2 {
            for c in -2i32..=2 {
                let mut q = Q0;
                ...
                q[2] = poke(Q0[2], b);
                q[3] = poke(Q0[3], c);
                ranked.push((score(&q), a, b, c));
            }
        }
    }
    ranked.sort_by(|x, y| y.0.cmp(&x.0).then(x.1.cmp(&y.1)));
    println!("top 12:");
    for (s, a, b, c) in ranked.iter().take(12) {
        let tag = if *a == 1 && *b == 1 && *c == -1 {
            " <- 65"
        } else {
            ""
        };
        println!("  fused={s} Q[1]{a:+} Q[2]{b:+} Q[3]{c:+}{tag}");
    }
    let best = ranked[0].0;
    let nbest = ranked.iter().filter(|t| t.0 == best).count();
    let ngt65 = ranked.iter().filter(|t| t.0 > 65).count();
    println!(
        "best={best} unique={} nbest={nbest} gt65={ngt65} n={}",
        nbest == 1,
        ranked.len()
    );
}
