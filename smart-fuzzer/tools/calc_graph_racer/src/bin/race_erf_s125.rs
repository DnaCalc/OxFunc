//! P-side joint as z*1.125 + z*(w-0.125) Boost-style split. Not an identity.
...
fn main() {
    ...
    let evals: [(&str, Box<dyn Fn(f64) -> f64>); 6] = [
        ...
        (
            "z*1.125 + z*(w-0.125) x87",
            Box::new(|z| {
                let w = w_x87(z, &joint);
                let a = ext_mul(&ef(z), &ef(1.125), CW);
                let b = ext_mul(&ef(z), &ext_sub(&w, &ef(0.125), CW), CW);
                ext_to_f64(&ext_add(&a, &b, CW), CW)
            }),
        ),
        (
            "z*1.125 + z*(w-0.125) f64",
            Box::new(|z| {
                let w = ext_to_f64(&w_x87(z, &joint), CW);
                z * 1.125 + z * (w - 0.125)
            }),
        ),
        (
            ...
            Box::new(|z| {
                let w = w_x87(z, &joint);
                let a = ext_div(&ext_mul(&ef(z), &ef(9.0), CW), &ef(8.0), CW);
                let b = ext_mul(&ef(z), &ext_sub(&w, &ext_div(&ef(1.0), &ef(8.0), CW), CW), CW);
                ext_to_f64(&ext_add(&a, &b, CW), CW)
            }),
        ),
        (
    ...
    println!("P-side 1.125-split of joint (bar 866):");
    for (name, ev) in &evals {
        ...
        println!("{name:32} {ex}/{} max={mx} hard_hit={hit_h}", rows.len());
    }
}

fn ext_div(a: &Ext80, b: &Ext80, cw: u16) -> Ext80 {
    rx::ext_div(a, b, cw)
}
