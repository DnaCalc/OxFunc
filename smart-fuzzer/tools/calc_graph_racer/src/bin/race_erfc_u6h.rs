//! leftover-high of 0x5005 z>=4 DIRECT vs F−k. Not an identity.
...
fn main() {
    ...
    for r in rows.iter().filter(|rr| rr.direct && rr.z >= 4.0) {
        ...
        let g = mul(f::w_rn53(r.z), cephes(r.z));
        let d = ulp_distance(g, t).unwrap_or(99);
        if d >= 2 && g > t {
            highs.push((r.z, r.qbits, d));
        }
    }
    println!("0x5005 leftover-high DIRECT z>=4 n={}", highs.len());
    let mut h = [0usize; 6];
    let mut hu = 0usize;
    ...
    println!(
        "leftover F-1={}/{} F-2={}/{} F-3={}/{} F-4={}/{} F-5={}/{} union={}/{}",
        h[1],
        highs.len(),
        h[2],
        highs.len(),
    ...
    for row in rows.iter().filter(|rr| rr.z >= 4.0) {
        ...
        let uk = (1..=5).any(|k| hitk(row.z, t, -k));
        if c0 {
            q0 += 1;
            if row.direct {
                d0 += 1;
    ...
    println!("Q z>=4 5005={q0} d={d0} F-1..5={qe} d={qd}");
}
