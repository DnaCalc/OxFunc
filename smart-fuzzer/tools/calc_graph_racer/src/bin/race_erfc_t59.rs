//! 0x4605 vs 0x4005 leftover DIRECT z>=4. Not an identity.
...
fn main() {
    ...
    let n = tail.len();
    println!("DIRECT z>=4 n={n} 0x4605 leftover:");
    for &(name, m) in &[
        ("unmask", 0u32),
        ("0x4005", 0x4005u32),
        ("0x4d", 0x4du32),
        ...
        println!(
            "  vs4005 {name} fused={nf} both={both} only_m={only_m} only_4005={only_4} leftover-low {hit_lo}/{nlo} leftover-high {hit_hi}/{nhi}"
        );
    }
    ...
    for r in &tail {
        ...
        if d >= 2 && g4 < t {
            println!(
                "  z={:.16} d={d} 0x4005={} 0x4d={} 0x4805={} 0x4605={}",
                r.z,
                hit(r.z, t, 0x4005),
                hit(r.z, t, 0x4d),
                hit(r.z, t, 0x4805),
    ...
    println!("0x4605 vs unmask CONV/LOSE:");
    ...
    for r in &tail {
        ...
        match (ku, km) {
            ...
            (Some(0), Some(k)) if k != 0 => {
                n_lose += 1;
                println!("  LOSE z={:.16} k={k}", r.z);
            }
            _ => {}
        }
    }
    print!(
        "CONV={n_conv} LOSE={n_lose} net={:+} |k|",
    ...
    println!();
    println!("0x4605 leftover last-store of itself:");
    let mut nlo = 0usize;
    let mut nhi = 0usize;
    let mut n1 = 0usize;
    ...
    for r in &tail {
        let t = f64::from_bits(r.qbits);
        let ff = cephes_mask(r.z, 0x4605);
        let g = mul(f::w_rn53(r.z), ff);
        let d = ulp_distance(g, t).unwrap_or(99);
        let k = signed_k(r.z, t, 0x4605);
        if d == 0 {
            fused += 1;
        } else if d == 1 {
            n1 += 1;
            if k.is_some() {
                ...
                println!("  1-ULP MISS z={:.16}", r.z);
            }
        } else if g < t {
            nlo += 1;
            match k {
                Some(kk) if kk > 0 => lo_cov += 1,
                other => println!("  leftover-low MISS z={:.16} k={other:?} d={d}", r.z),
            }
        } else {
            nhi += 1;
            match k {
                Some(kk) if kk < 0 => hi_cov += 1,
                other => println!("  leftover-high MISS z={:.16} k={other:?} d={d}", r.z),
            }
        }
    }
    println!("fused={fused} leftover-low {lo_cov}/{nlo} leftover-high {hi_cov}/{nhi} 1-ULP {u1_cov}/{n1}");
}
