...
fn ulp_dist(x: f64, y: f64) -> i64 {
    x.to_bits() as i64 - y.to_bits() as i64
} // same-sign magnitudes
...
fn main() {
    ...
    for line in csv.lines().skip(1) {
        ...
        if base_miss {
            ...
            if dump.len() < 24 {
                ...
                dump.push(format!(
                    "k={:3} n={:3} em-base={:+} toward0={} reqD_off_ulp(lnu)={:+} c1hit={}",
                    k,
                    n,
                    d,
                    toward_zero,
    ...
    println!(
        "N={} baseline misses={} (toward-zero={}, away={}, |off|>1ulp={})",
        tot, nmiss, toward, away, faroff
    );
    ...
    println!(
        "\nchop-final-divide(1) FIXES {} miss rows, BREAKS {} hit rows",
        c1_fix, c1_break
    );
