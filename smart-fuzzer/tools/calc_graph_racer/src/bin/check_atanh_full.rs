...
fn main() {
    ...
    let cands = [
        8e-5f64, 9e-5, 9.5e-5, 1e-4, 1.05e-4, 1.1e-4, 1.15e-4, 1.2e-4, 1.25e-4, 1.3e-4, 1.4e-4,
    ];
    ...
    for (x, want) in &rows {
        if f(*x).to_bits() != *want && shown < 20 {
            shown += 1;
            println!(
                "  MISS x={x:.6e} got-want {:+} ulp",
                f(*x).to_bits() as i64 - *want as i64
            );
        }
    }
}
