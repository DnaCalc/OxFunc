//! Exclusive leftover-high z of 0x5005 vs 66. Not an identity.
...
fn main() {
    ...
    q66[3] = poke(Q0[3], 1);
    let mut nsh = 0usize;
    println!("exclusive leftover-high 0x5005 vs 66:");
    for r in rows.iter().filter(|rr| rr.direct && rr.z >= 4.0) {
        ...
        if hi5 && hi66 {
            ...
            println!(
                "  only5005 z={:.16} k={k:?} last-mul_dn={dn} 66={}",
                r.z,
                bucket(g66, t)
            );
        } else if hi66 {
            println!(
                "  only66 z={:.16} k={k:?} last-mul_dn={dn} 5005={}",
                r.z,
                bucket(g5, t)
            );
        }
