//! leftover-high last-store k of unmask 0x4005 vs 0x4e05. Not an identity.
...
fn main() {
    ...
    let mut n4e = 0usize;
    let mut h4 = [0usize; 9];
    let mut h4e = [0usize; 9];
    println!("leftover-high last-store k of unmask 0x4005 vs 0x4e05:");
    for r in rows.iter().filter(|rr| rr.direct && rr.z >= 4.0) {
        let t = f64::from_bits(r.qbits);
        let w = f::w_rn53(r.z);
        let g4 = mul(w, cephes(r.z, 0x4005));
        let d4 = ulp_distance(g4, t).unwrap_or(99);
        let g4e = mul(w, cephes(r.z, 0x4e05));
        let d4e = ulp_distance(g4e, t).unwrap_or(99);
        let k = k_of(r.z, t, cephes(r.z, 0));
        if d4 >= 2 && g4 > t {
            n4 += 1;
            if let Some(kk) = k {
    ...
    print!("0x4005 leftover-high n={n4} unmask |k|");
    for (i, c) in h4.iter().enumerate() {
        if *c > 0 {
            print!(" {i}:{c}");
        }
    }
    print!("\n0x4e05 leftover-high n={n4e} unmask |k|");
    for (i, c) in h4e.iter().enumerate() {
        if *c > 0 {
            print!(" {i}:{c}");
        }
    }
