...
fn ln_ext(u: &Ext80) -> Ext80 {
    ext_fyl2x(&ext_ln2(), u, CW)
}

// ---- tau builders ----
fn tau_dbl(r: f64, n: f64) -> f64 {
    -(n * rx::excel_log1p(r))
} // double n*log1p_cr, negate
fn tau_ext_val(r: f64, n: f64) -> Ext80 {
    // -n*ln(1+r) all ext (1+r exact @ po2)
...
fn main() {
    ...
    let cands: [(&str, fn(f64, f64) -> f64); 6] = [
        ("dbl_kahan(prod)", s_dbl_kahan),
        ("ext_kahan", s_ext_kahan),
        ("exttau_dblkahan", s_exttau_dblkahan),
        ("dbltau_extkahan", s_dbltau_extkahan),
        ("ext_uspill", s_ext_uspill),
        ("ext_reassoc", s_ext_reassoc),
    ];
    ...
    for (i, (nm, _)) in cands.iter().enumerate() {
        println!("  {:<18} {}/{}", nm, score[i], tot);
    }
    println!(
        "\nmisses per n (n : {}):",
        cands.iter().map(|c| c.0).collect::<Vec<_>>().join(" ")
    );
