//! 0x24a5 + P[4]-1 Q[0]+1 Q[1]-1 leftover DIRECT z>=4. Not an identity.
...
fn main() {
    ...
    let mut n1 = 0usize;
    let mut u1 = 0usize;
    println!("DIRECT z>=4 n={n} 0x24a5 P[4]-1 Q[0]+1 Q[1]-1 vs 0x24a5 leftover:");
    for r in &tail {
        let t = f64::from_bits(r.qbits);
        let em = hit(r.z, t, &p65, &q65);
        ...
        let g = mul(f::w_rn53(r.z), cephes(r.z, &p65, &q65));
        let d = ulp_distance(g, t).unwrap_or(99);
        let k = signed_k(r.z, t, &p65, &q65);
        if d == 1 {
            n1 += 1;
            if k.is_some() {
                u1 += 1;
    ...
    println!(
        "fused={nf} both={both} only_m={only_m} only_24a5={only_b} leftover-low {hit_lo}/{nlo} leftover-high {hit_hi}/{nhi} CONV={} LOSE={}",
        only_m, only_b
    );
