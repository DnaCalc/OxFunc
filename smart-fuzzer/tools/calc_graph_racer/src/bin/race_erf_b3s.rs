//! B[3]−1 A/B last-store vs 4 LOW. Not an identity.
...
fn main() {
    ...
    let graphs: [(&str, Box<dyn Fn(f64) -> f64>); 10] = [
        ("B3", Box::new(b3f)),
        ("up(B3)", Box::new(move |z| b3f(z).next_up())),
        ("up2(B3)", Box::new(move |z| b3f(z).next_up().next_up())),
        (
            "up3(B3)",
            Box::new(move |z| b3f(z).next_up().next_up().next_up()),
        ),
        ("dn(B3)", Box::new(move |z| b3f(z).next_down())),
        ("joint", Box::new(jf)),
        ("up(j)", Box::new(move |z| jf(z).next_up())),
        ("up2(j)", Box::new(move |z| jf(z).next_up().next_up())),
        ("dn(j)", Box::new(move |z| jf(z).next_down())),
        (
            "up(B3A2)",
            Box::new(move |z| {
                let mut a = AA;
                ...
                let mut b = BB;
                b[3] = poke(BB[3], -1);
                cody_ab(z, &a, &b).next_up()
            }),
        ),
    ];
    println!("4 LOW B[3]-1 last-store:");
    for &lz in &LOWS {
