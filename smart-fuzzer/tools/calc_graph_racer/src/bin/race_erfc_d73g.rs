//! D[7]−3 vs 0x210 DIRECT GAIN/LOSE. Not an identity.
...
fn main() {
    ...
    for r in &dirs {
        ...
        let b = ulp_distance(qwf(r.z, cody(r.z, &C0, &D0)), t).unwrap_or(99) == 0;
        let n = ulp_distance(qwf(r.z, cody(r.z, &C0, &d3)), t).unwrap_or(99) == 0;
        match (b, n) {
            (true, true) => both += 1,
            (false, true) => gain.push(r.z),
            (true, false) => lose.push(r.z),
            _ => {}
        }
    }
    println!(
        "D[7]-3 vs 0x210 both={both} GAIN={} LOSE={} dmid={}",
        gain.len(),
        lose.len(),
        both + gain.len()
    );
