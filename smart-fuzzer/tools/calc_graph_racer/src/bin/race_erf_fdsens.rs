//! fdlibm PP/QQ x87 ±1 keep-and-hit vs 4 LOW. Not an identity.
...
fn main() {
    ...
    println!("4 LOW fdlibm CR:");
    for &lz in &LOWS {
        let Some((_, bits)) = rows.iter().find(|(z, _)| (*z - lz).abs() < 1e-14) else {
            continue;
        };
        let t = f64::from_bits(*bits);
        let g = fd_x87(lz, &PP, &QQ);
        let d = ulp_distance(g, t).unwrap_or(99);
        let dir = if g < t {
            "L"
        } else if g > t {
    ...
    let k0 = keep(&PP, &QQ);
    let (h0, _) = hit4(&PP, &QQ);
    println!("fdlibm x87 CR keep={k0}/{} hit={h0}/4 (bar joint 866)", rows.len());
