...
use rx::{ext_from_f64 as e, ext_fyl2xp1, ext_ln2, ext_to_f64, CW_PC64_RN as CW};
use std::collections::BTreeMap;

fn tau_d(r:f64,n:f64)->f64{ -(n*rx::excel_log1p(r)) }
// ln(u) deliveries:
fn lnu_fyl2x(u:f64)->f64{ rx::excel_ln(u) }                       // fyl2x(u) [current]
fn lnu_fyl2xp1(u:f64)->f64{ ext_to_f64(&ext_fyl2xp1(&ext_ln2(),&e(u-1.0),CW),CW) } // log1p(u-1) x87
fn lnu_log1pcr(u:f64)->f64{ rx::excel_log1p(u-1.0) }              // log1p(u-1) portable CR
...
fn main(){
    ...
    let denoms:[(&str,fn(f64)->f64);3]=[("fyl2x(u)",lnu_fyl2x),("fyl2xp1(u-1)",lnu_fyl2xp1),("log1pCR(u-1)",lnu_log1pcr)];
    let mut tot=0u32; let mut score=[[0u32;3];3];
    for ((rb,nb),rows) in &byrn{
        ...
        for (di,(_,ld)) in denoms.iter().enumerate(){
            for asc in 0..3u8{
                if em(r,n,*ld,asc).to_bits()==em_ex.to_bits(){ score[di][asc as usize]+=1; }
            }
        }
    }
    println!("|tau|<1 pinned: {}", tot);
    println!("{:<16} {:>10} {:>10} {:>10}", "denom\\assoc", "prod", "kahan", "divfirst");
    for (di,(nm,_)) in denoms.iter().enumerate(){
        println!("{:<16} {:>10} {:>10} {:>10}", nm, score[di][0], score[di][1], score[di][2]);
    }
}
