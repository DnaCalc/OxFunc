...
use rx::{CW_PC64_RN, ext_add, ext_from_f64, ext_fyl2x, ext_fyl2xp1, ext_ln2, ext_one, ext_to_f64};
...
fn main() {
    ...
    println!(
        "distinct r: {}   pv/r: {}",
        nr,
        byr.values().next().map(|v| v.len()).unwrap_or(0)
    );

    let cands: [(&str, fn(f64) -> f64); 4] = [
        ("CR", c_cr),
        ("FYL2XP1", c_fyl2xp1),
        ("FYL2Xext", c_fyl2x_extu),
        ("hybrid", c_hybrid),
    ];
    for (nm, f) in cands {
        let mut full = 0u32;
        ...
        println!(
            "{:<10} full-r {:>3}/{:<3}   pv {:>6}/{:<6}",
            nm, full, nr, okpv, totpv
        );
    }
    ...
    for (_rb, rows) in &byr {
        ...
        let mut fail = 0;
        for (pv, want) in rows {
            if pmt_n1(r, *pv, c_cr).to_bits() != *want {
                fail += 1;
            }
        }
        if fail == 0 {
        ...
        let tau = -c_cr(r);
        let em0 = rx::excel_expm1_internal(tau);
        ...
        if printed < 80 {
            println!(
                "  r=2^{:+8.4}  fail {:>3}/256   best em-nudge {:+} -> {} left",
                r.log2(),
                fail,
                best.1,
                best.0
