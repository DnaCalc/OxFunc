...
fn log1p_x87(r: f64) -> f64 {
    ext_to_f64(&ext_fyl2xp1(&ext_ln2(), &ext_from_f64(r), CW), RN53)
}
...
fn main() {
    ...
    let names = [
        "L0 portable-CR [landed]",
        "L1 x87 FYL2XP1 spill",
        "L2 x87 nl fully-ext",
    ];
    print!("{:11}{:>7}", "corpus", "N");
    for nm in &names {
    ...
    for cn in corpora {
        ...
        for w in &ws.witnesses {
            ...
            for i in 0..3 {
                ...
                if pmt_a1(r, pv, fv, ty, em).to_bits() == want {
                    sc[i] += 1;
                }
            }
        }
        print!("{:11}{:>7}", cn, tot);
        for i in 0..3 {
            print!(" {:>23.1}%", 100.0 * sc[i] as f64 / tot as f64);
        }
        println!();
    }
}
