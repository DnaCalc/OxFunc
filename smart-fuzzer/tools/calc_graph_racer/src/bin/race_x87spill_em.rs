...
use rx::{ext_div, ext_from_f64 as e, ext_mul, ext_to_f64, CW_PC64_RN as CW};
...
fn ddiv(a:f64,b:f64)->f64{ ext_to_f64(&ext_div(&e(a),&e(b),CW),CW) }

fn tau_sse(r:f64,n:f64)->f64{ -(n*rx::excel_log1p(r)) }
fn tau_x87(r:f64,n:f64)->f64{ -dmul(n, rx::excel_log1p(r)) }   // n*log1p double-rounded, negate
...
fn main(){
    let variants=[("sse(base)",false,false,false),("num_dr",true,false,false),
        ("div_dr",false,true,false),("num+div_dr",true,true,false),
        ("tau_dr",false,false,true),("num+tau_dr",true,false,true),
        ("num+div+tau_dr",true,true,true),("div+tau_dr",false,true,true)];
    for (src,path,po2) in [("po2n","../../work/w109/G6-solvers/answers-pmt-po2n.json",true),
        ...
        for (nm,a,b,c) in variants{
            let ok=pins.iter().filter(|(r,n,ep)| em(*r,*n,a,b,c).to_bits()==ep.to_bits()).count();
            // signed residual dist for this variant
            let mut plus=0; let mut minus=0;
            for (r,n,ep) in &pins{ let d=sordi(em(*r,*n,a,b,c).to_bits())-sordi(ep.to_bits());
                if d>0{plus+=1}else if d<0{minus+=1} }
            println!("  {:<16} {:>3}/{}   (miss +{} / -{})", nm, ok, pins.len(), plus, minus);
        }
    }
}
