//! Two-factor up(y)*up(q) of A/B variants vs 4 LOW. Not an identity.
...
fn parts(z: f64, a: &[f64; 5], b: &[f64; 4]) -> (Ext80, Ext80, Ext80) {
    let ye = ef(z);
    ...
    (
        ye,
        ext_add(&xnum, &ef(a[3]), CW),
        ext_add(&xden, &ef(b[3]), CW),
    )
}
fn spec(z: f64, a: &[f64; 5], b: &[f64; 4]) -> f64 {
    let (ye, num, den) = parts(z, a, b);
    ext_to_f64(&ext_div(&ext_mul(&ye, &num, CW), &den, CW), CW)
}
fn tf(z: f64, a: &[f64; 5], b: &[f64; 4]) -> f64 {
    let (_ye, num, den) = parts(z, a, b);
    z.next_up() * ext_to_f64(&ext_div(&num, &den, CW), CW).next_up()
}
fn joint(z: f64) -> f64 {
    let mut a = AS0;
    a[0] = poke(AS0[0], 4);
...
fn joint_tf(z: f64) -> f64 {
    ...
    let w = ext_to_f64(&acc, CW).next_up();
    z.next_up() * (1.0 + w)
}
...
fn main() {
    ...
    let graphs: [(&str, Box<dyn Fn(f64) -> f64>); 8] = [
        ("CR", Box::new(|z| spec(z, &AA, &BB))),
        ("tfCR", Box::new(|z| tf(z, &AA, &BB))),
        (
            "A3p",
            Box::new(|z| {
                let mut a = AA;
        ...
        ("joint", Box::new(joint)),
        ("tfj", Box::new(joint_tf)),
    ];
    println!("4 LOW two-factor control:");
    for &lz in &LOWS {
