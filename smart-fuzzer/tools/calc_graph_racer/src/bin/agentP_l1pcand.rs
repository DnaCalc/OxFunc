...
fn kahan_exttau(tau_ext: &rx::Ext80) -> f64 {
    let tf64 = tf(tau_ext);
    let u = rx::excel_exp(tf64);
    let l = rx::excel_ln(u);
    if u == 1.0 {
        tf64
    } else if tf64.abs() < 1.0 {
        let num = tf(&rx::ext_mul(&e(u - 1.0), tau_ext, CW));
        num / l
    } else {
        u - 1.0
    }
...
fn main() {
    ...
    for (r, em) in &rows {
        // A portable log1p
        if kahan_f64(-rx::excel_log1p(*r)).to_bits() == em.to_bits() {
            *res.entry("A portable-log1p f64").or_default() += 1;
        }
        // B fyl2xp1 stored
        let l_f2 = tf(&rx::ext_fyl2xp1(&rx::ext_ln2(), &e(*r), CW));
        if kahan_f64(-l_f2).to_bits() == em.to_bits() {
            *res.entry("B fyl2xp1-stored f64").or_default() += 1;
        }
        ...
        if kahan_exttau(&tau_ext).to_bits() == em.to_bits() {
            *res.entry("C fyl2xp1-EXT in numer").or_default() += 1;
        } else {
            miss_ext.push(*r);
        }
        // D fdlibm log1p f64
        if kahan_f64(-fdlibm_log1p(*r)).to_bits() == em.to_bits() {
            *res.entry("D fdlibm-log1p f64").or_default() += 1;
        }
        // E ln(1+r) x87 (1+r f64)
        if kahan_f64(-rx::excel_ln(1.0 + *r)).to_bits() == em.to_bits() {
            *res.entry("E ln(1+r) x87").or_default() += 1;
        }
    }
