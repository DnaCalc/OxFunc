...
fn tf64(e:&rx::Ext80)->f64{ rx::ext_to_f64(e, CW) }
fn tf64rz(e:&rx::Ext80)->f64{ rx::ext_to_f64(e, CW_RZ) }

// natural log1p(r) as Ext80
#[derive(Clone,Copy)] enum LP{ Fyl2xp1, Log1pCR, LnOf1PlusR }
...
#[derive(Clone,Copy)] enum EP{ Internal, PortCR, F2xm1FoldRN, F2xm1FoldRZ, F2xm1NoFold, ExpM1RN }
        ...
        _ => {
            ...
            match ep{
                EP::F2xm1FoldRN | EP::F2xm1FoldRZ => {
                    ...
                    let em_ext = if kf==0.0 { fm1 } else {
                        let scaled = rx::ext_scale(&p2f, &k, CW);
                        rx::ext_sub(&scaled, &rx::ext_one(), CW)
                    };
                    if matches!(ep,EP::F2xm1FoldRZ){ tf64rz(&em_ext) } else { tf64(&em_ext) }
                }
                EP::F2xm1NoFold => {
...
fn em_ext(lp:LP, n:f64, r:f64)->rx::Ext80{
    ...
    let fm1=if neg{ let twof=rx::ext_add(&w,&rx::ext_one(),CW);
        rx::ext_div(&rx::ext_sub(&rx::ext_from_f64(0.0),&w,CW),&twof,CW)} else {w};
    let kf=rx::ext_to_f64(&k,CW);
    if kf==0.0 { fm1 } else {
        let p2f=rx::ext_add(&fm1,&rx::ext_one(),CW);
...
fn pmt_spill(lp:LP, r:f64, n:f64, pv:f64, chop:bool)->f64{
    ...
    let numr=rx::ext_mul(&rx::ext_from_f64(pv),&rx::ext_mul(&rx::ext_from_f64(1.0),&rx::ext_from_f64(r),CW),CW);
    let q=rx::ext_div(&numr,&em,CW);
    if chop { rx::ext_to_f64(&q,CW_RZ) } else { rx::ext_to_f64(&q,CW) }
}
...
fn load(path:&str)->Vec<(f64,f64,f64,u64)>{
    ...
    for w in &ws.witnesses{
        ...
        if a.len()!=5 {continue}
        if a[3]!=0.0 || a[4]!=0.0 {continue} // fv=0, ty=0 only
        if let Some(want)=parse_bits_hex(&w.expected_bits){ o.push((a[0],a[1],a[2],want.to_bits())); }
    }
    o
}
...
fn main(){
    ...
    let path=av.get(1).cloned().unwrap_or_else(||"../../work/w109/G6-solvers/answers-pmt-em.json".into());
    let rows=load(&path);
    let pv1:Vec<_>=rows.iter().filter(|(_,_,pv,_)| *pv==1.0).cloned().collect();
    println!("collapsed-grid rows: {} (pv=1: {})", rows.len(), pv1.len());
    let lps=[("fyl2xp1",LP::Fyl2xp1),("log1pCR",LP::Log1pCR),("ln(1+r)",LP::LnOf1PlusR)];
    let eps=[("internal",EP::Internal),("portCR",EP::PortCR),("f2xm1FoldRN",EP::F2xm1FoldRN),
             ("f2xm1FoldRZ",EP::F2xm1FoldRZ),("f2xm1NoFold",EP::F2xm1NoFold),("expM1",EP::ExpM1RN)];
        ...
        for (en,ep) in eps{
            for tstore in [false,true]{
                ...
                for (r,n,pv,want) in &pv1{
                    ...
                    if tstore { t=rx::ext_from_f64(rx::ext_to_f64(&t,CW)); }
                    let em=expm1_of(ep,&t);
                    let g=(pv*r)/em;
                    if g.to_bits()==*want{ok+=1;}
                }
                let tag=format!("L={:8} E={:12} tstore={}", ln,en,tstore);
                if ok>best.0 {best=(ok,tag.clone());}
                println!("  {:5}/{}  {}", ok, pv1.len(), tag);
            }
        }
    }
    println!("\nBEST pv=1 (em-stored, SSE2 div): {}/{}  {}", best.0, pv1.len(), best.1);
    ...
    for (ln,lp) in lps{
        for chop in [false,true]{
            let mut ok=0u32;
            for (r,n,pv,want) in &pv1{
                if pmt_spill(lp,*r,*n,*pv,chop).to_bits()==*want{ok+=1;}
            }
            let tag=format!("L={:8} chop={}", ln, chop);
            if ok>bs.0{bs=(ok,tag.clone());}
            println!("  {:5}/{}  {}", ok, pv1.len(), tag);
        }
    }
    ...
    let em_internal=|r:f64,n:f64|{ let l=rx::excel_log1p(r); rx::excel_expm1_internal(rx::x87_mul(-n,l)) };
    let em_powchain=|r:f64,n:f64|{ // v=exp(x87mul(-n, ln(1+r))), em=v-1
        let ln1pr=rx::excel_ln(1.0+r); let v=rx::excel_exp(rx::x87_mul(-n,ln1pr)); v-1.0 };
    let em_expfold=|r:f64,n:f64|{ let l=rx::excel_log1p(r); let v=rx::excel_exp(rx::x87_mul(-n,l)); v-1.0 };
    let bins=[(0.0,0.25),(0.25,1.0),(1.0,4.0),(4.0,1e9)];
    for (name,f) in [("internalKahan",&em_internal as &dyn Fn(f64,f64)->f64),
                     ("powchain(1+r)",&em_powchain),("expfold(log1p)",&em_expfold)]{
        print!("  {:16}", name);
        for (lo,hi) in bins{
            ...
            for (r,n,pv,want) in &pv1{
                ...
                tot+=1;
                if ((pv*r)/f(*r,*n)).to_bits()==*want{ok+=1;}
            }
    ...
    for thr in [0.5f64,1.0,2.0,4.0]{
        ...
        for (r,n,pv,want) in &pv1{
            let t=(*n*rx::excel_log1p(*r)).abs();
            let em=if t<thr {em_expfold(*r,*n)} else {em_powchain(*r,*n)};
            if ((pv*r)/em).to_bits()==*want{ok+=1;}
        }
    ...
    if !ho.is_empty(){
        ...
        for (r,n,pv,want) in &ho{
            if pmt_spill(LP::Log1pCR,*r,*n,*pv,false).to_bits()==*want{ok+=1;}
        }
        println!("\nspill (log1pCR,RN) on held-out fv=0/ty=0 subset: {}/{}", ok, ho.len());
    }
}
