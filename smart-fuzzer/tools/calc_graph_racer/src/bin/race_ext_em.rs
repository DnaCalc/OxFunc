...
fn log2_1p(r:f64)->Ext80{
    if r.abs()<0.292893218813452 { ext_fyl2xp1(&ext_one(), &e(r), CW) }
    else { ext_fyl2x(&ext_one(), &ext_add(&ext_one(),&e(r),CW), CW) }
}
// ln(1+r) in ext = ln2 * log2(1+r)
fn ln_1p(r:f64)->Ext80{ ext_mul(&ext_ln2(), &log2_1p(r), CW) }
...
fn a_f2xm1(r:f64,n:f64)->f64{
    ...
    let k=ext_rndint(&y,CW); let f=ext_sub(&y,&k,CW);   // |f|<=0.5
    let w=ext_f2xm1(&f,CW);                              // 2^f - 1 (f can be negative; F2XM1 domain |f|<=1 ok)
    // 2^y - 1 = 2^k*(2^f) - 1 = 2^k*(w+1) - 1 = scale(w+1,k) - 1
    let em=ext_sub(&ext_scale(&ext_add(&w,&ext_one(),CW),&k,CW),&ext_one(),CW);
    ext_to_f64(&em,CW)
}
...
fn a2_f2xm1_direct(r:f64,n:f64)->f64{
    ...
    if tf(&y).abs()<=1.0 { return ext_to_f64(&ext_f2xm1(&y,CW),CW); }
    let k=ext_rndint(&y,CW); let f=ext_sub(&y,&k,CW);
    let w=ext_f2xm1(&f,CW);
    ext_to_f64(&ext_sub(&ext_scale(&ext_add(&w,&ext_one(),CW),&k,CW),&ext_one(),CW),CW)
}
...
fn k_dbl(r:f64,n:f64)->f64{ let t=tau_d(r,n); let u=rx::excel_exp(t); (u-1.0)*t/rx::excel_ln(u) }
...
fn main(){
    ...
    let cands:[(&str,fn(f64,f64)->f64);7]=[
        ("K_dbl(base 163)",k_dbl),("A_f2xm1_split",a_f2xm1),("A2_f2xm1_direct",a2_f2xm1_direct),
        ("B_extu-1",b_uminus1),("C_extkahan",c_extkahan),
        ("D_f2xm1_dbltau",d_f2xm1_dbltau),("E_extu-1_dbltau",e_uminus1_dbltau),
    ];
    let mut sc=[0u32;7]; let mut tot=0u32;
    // split by |tau| vs ln2 to see the F2XM1 domain boundary
    let mut sc_lt=[0u32;7]; let mut tot_lt=0u32; // |tau|<ln2
    for ((rb,nb),rows) in &byrn{
        ...
        for (i,(_,f)) in cands.iter().enumerate(){
            if f(r,n).to_bits()==em_ex.to_bits(){sc[i]+=1; if lt{sc_lt[i]+=1;}}
        }
    }
    println!("po2n |tau|<1 pinned: {}   (|tau|<ln2 subset: {})", tot, tot_lt);
    for (i,(nm,_)) in cands.iter().enumerate(){
        println!("  {:<18} {:>3}/{}   (|tau|<ln2: {}/{})", nm, sc[i], tot, sc_lt[i], tot_lt);
    }
}
