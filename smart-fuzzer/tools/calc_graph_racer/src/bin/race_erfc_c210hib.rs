//! 0x210 leftover-high last-store k of unmask and CONV z. Not an identity.
...
fn main() {
    ...
    let mut nsh = 0usize;
    println!("leftover-high last-store k of unmask:");
    for r in rows.iter().filter(|rr| rr.direct && rr.z >= 0.5 && rr.z < 4.0) {
        ...
        let hi210 = d210 >= 2 && g210 > t;
        let hi114 = d114 >= 2 && g114 > t;
        ...
        if hi210 {
            ...
            if d114 == 0 {
                println!(
                    "  CONV z={:.16} unmask_k={k:?} last-mul next_down={dn} unmask_d={}",
                    r.z,
        ...
        if hi114 {
            ...
            if d210 == 1 {
                println!(
                    "  114-HIGH from 0x210 1-ULP z={:.16} unmask_k={k:?} last-mul next_down={dn}",
                    r.z
                );
            }
        }
    ...
    print!("0x210 leftover-high n={n210} unmask |k|");
    for (i, c) in h210.iter().enumerate() {
        if *c > 0 {
            print!(" {i}:{c}");
        }
    }
    print!("\n114 leftover-high n={n114} unmask |k|");
    for (i, c) in h114.iter().enumerate() {
        if *c > 0 {
            print!(" {i}:{c}");
        }
    }
    print!("\nshared leftover-high n={nsh} unmask |k|");
