//! 0x4d / 0x4805 vs 0x4005 leftover DIRECT z>=4. Not an identity.
        ...
        if g < t {
            ...
            if let Some(kk) = k {
                if kk > 0 {
                    ...
                    println!("  {name} leftover-low wrong-sign z={:.16} k={kk}", r.z);
                }
            } else {
                println!("  {name} leftover-low MISS z={:.16} d={d}", r.z);
            }
        } else {
            nhi += 1;
            if let Some(kk) = k {
                if kk < 0 {
                    ...
                    println!("  {name} leftover-high wrong-sign z={:.16} k={kk}", r.z);
                }
            } else {
                println!("  {name} leftover-high MISS z={:.16} d={d}", r.z);
            }
        }
    }
    print!("  {name} fused={fused} leftover-low {lo_cov}/{nlo} F+");
    for (i, c) in lo_k.iter().enumerate() {
        if *c > 0 {
            print!(" {i}:{c}");
        }
    }
    print!(" leftover-high {hi_cov}/{nhi} F-");
    for (i, c) in hi_k.iter().enumerate() {
        if *c > 0 {
            print!(" {i}:{c}");
        }
    }
    println!(" 1-ULP {u1_cov}/{n1}");
}
...
fn main() {
    ...
    println!("DIRECT z>=4 n={n} 58 vs 0x4005 leftover:");
    for &(name, m) in &masks {
        let mut both = 0usize;
        let mut only_m = 0usize;
        let mut only_4 = 0usize;
        let mut nf = 0usize;
        let mut n4 = 0usize;
        let mut nlo = 0usize;
        ...
        for r in &tail {
            ...
            let g4 = mul(f::w_rn53(r.z), cephes_mask(r.z, 0x4005));
            let d = ulp_distance(g4, t).unwrap_or(99);
            if d >= 2 && g4 < t {
                nlo += 1;
                if em {
                    hit_lo += 1;
        ...
        println!(
            "  vs4005 {name} fused={nf} 4005={n4} both={both} only_m={only_m} only_4005={only_4} leftover-low {hit_lo}/{nlo} leftover-high {hit_hi}/{nhi}"
        );
    }

    println!("0x4005 leftover-low hits:");
    for r in &tail {
        let t = f64::from_bits(r.qbits);
        let g4 = mul(f::w_rn53(r.z), cephes_mask(r.z, 0x4005));
        let d = ulp_distance(g4, t).unwrap_or(99);
        if d >= 2 && g4 < t {
            println!(
                "  z={:.16} d={d} unmask={} 0x4005={} 0x4d={} 0x4805={}",
                r.z,
                hit(r.z, t, 0),
                hit(r.z, t, 0x4005),
                hit(r.z, t, 0x4d),
    ...
    println!("0x4d vs 0x4805 fused CONV/LOSE:");
    let mut only_a = 0usize;
    let mut only_b = 0usize;
    let mut both = 0usize;
    for r in &tail {
        let t = f64::from_bits(r.qbits);
        let ea = hit(r.z, t, 0x4d);
        let eb = hit(r.z, t, 0x4805);
        match (ea, eb) {
            (true, true) => both += 1,
            (true, false) => {
                only_a += 1;
                println!(
                    "  0x4d-only z={:.16} 4805_k={:?}",
                    r.z,
                    signed_k(r.z, t, 0x4805)
                );
            }
            (false, true) => {
                only_b += 1;
                println!(
                    "  0x4805-only z={:.16} 4d_k={:?}",
                    r.z,
                    signed_k(r.z, t, 0x4d)
                );
            }
    ...
    println!("both={both} only_4d={only_a} only_4805={only_b}");

    println!("CONV/LOSE vs unmask:");
    for &(name, m) in &[("0x4d", 0x4du32), ("0x4805", 0x4805u32)] {
        ...
        for r in &tail {
            ...
            match (ku, km) {
                (Some(u), Some(0)) if u != 0 => {
                    n_conv += 1;
                    println!("  {name} CONV z={:.16} unmask_k={u}", r.z);
                }
                (Some(0), Some(k)) if k != 0 => {
                    n_lose += 1;
                    println!("  {name} LOSE z={:.16} k={k}", r.z);
                }
                _ => {}
            }
        }
    ...
    leftover_self(&tail, 0x4d, "0x4d");
    leftover_self(&tail, 0x4805, "0x4805");
}
