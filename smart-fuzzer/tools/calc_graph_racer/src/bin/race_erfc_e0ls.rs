//! ccdd E0+1 last-store vs 6 leftover-low / 5.333. Not an identity.
...
fn main() {
    ...
    let graphs: [(&str, Box<dyn Fn(f64) -> f64>); 6] = [
        ("E0+1", Box::new(|z| qw(z, acc_x(z)))),
        ("upQ", Box::new(|z| qw(z, acc_x(z)).next_up())),
        (
            "up2Q",
            Box::new(|z| qw(z, acc_x(z)).next_up().next_up()),
        ),
        ("upF", Box::new(|z| qw(z, acc_x(z).next_up()))),
        (
            "up2F",
            Box::new(|z| qw(z, acc_x(z).next_up().next_up())),
        ),
        (
            "upw*upF",
            Box::new(|z| {
                let v = f::w_rn53(z).next_up() * acc_x(z).next_up();
                if v.abs() < f64::MIN_POSITIVE {
                    0.0
                } else {
                    v
    ...
    println!("6 leftover-low E0+1 last-store:");
    for &lz in &SIX {
