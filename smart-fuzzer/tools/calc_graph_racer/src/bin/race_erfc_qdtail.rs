//! Direct-only Q-tail named F. Not an identity.
...
fn main() {
    ...
    type Ev = fn(f64) -> f64;
    let graphs: [(&str, Ev); 7] = [
        ("cephes mask0", |z| qw(z, cephes_mask(z, 0))),
        ("cephes 0x5005", |z| qw(z, cephes_mask(z, 0x5005))),
        ("cephes native f", |z| qw(z, f::cephes_f(z))),
        ("cody P/Q x87", |z| qw(z, cody_pq(z))),
        ("nswc_ccdd_f", |z| qw(z, f::nswc_ccdd_f(z))),
    ...
    for (name, ev) in graphs {
        ...
        println!(
            "{name:18} exact={exact}/{n} maxa={max_a} m1={m1} dtail={dtail}/{dn} m1d={m1d} maxd={max_d} w1d={}",
            dtail + m1d
        );
    }
}
