...
fn ename(e: EP) -> &'static str {
    match e {
        EP::Expm1Int => "expm1Int",
        EP::Expm1CR => "expm1CR",
        EP::ExpMinus1 => "exp-1",
    }
}
fn aname(a: AR) -> &'static str {
    match a {
        AR::StableEm => "stableEm",
        AR::Stable1mv => "stable1mv",
        AR::VIndep => "vIndep",
        AR::ForwardX87 => "fwdX87",
    }
}
...
fn expm1p(ep: EP, t: f64) -> f64 {
    match ep {
        EP::Expm1Int => rx::excel_expm1_internal(t),
        EP::Expm1CR => rx::excel_expm1(t),
        EP::ExpMinus1 => rx::excel_exp(t) - 1.0,
    }
}
...
fn score(o: &[(Vec<f64>, u64)], lp: LP, ep: EP, ar: AR, m: u8) -> u32 {
    o.iter()
        .filter(|(a, w)| model(lp, ep, ar, m, a[0], a[1], a[2], a[3], a[4]).to_bits() == *w)
        .count() as u32
}
...
fn main() {
    ...
    for &lp in &LPS {
        for &ep in &EPS {
            for &ar in &ARS {
                for m in 0u8..=255u8 {
                    cand.push((
                        score(&hoo, lp, ep, ar, m),
                        score(&tro, lp, ep, ar, m),
                        lp,
                        ep,
                        ar,
                        m,
    ...
    for (h, t, lp, ep, ar, m) in cand.iter().take(20) {
        println!(
            "  held {:4}/{}  train {:2}/{}  L={:8} E={:8} arr={:9} mask={:08b}",
            h,
            hoo.len(),
            t,
            tro.len(),
    ...
    println!(
        "\n=== champion residual: L={} E={} arr={} mask={:08b} ===",
        lname(lp),
        ename(ep),
        aname(ar),
        m
    ...
    for (a, w) in &hoo {
        ...
        *hist.entry(d.clamp(-6, 6)).or_insert(0) += 1;
        let e = byn.entry(a[1] as i64).or_insert((0, 0));
        if d == 0 {
            e.0 += 1
        } else {
            e.1 += 1
        ...
        let e2 = reg.entry(rk).or_insert((0, 0));
        if d == 0 { e2.0 += 1 } else { e2.1 += 1 }
    }
