...
use rx::{CW_PC64_RN as CW, ext_div, ext_from_f64 as ef, ext_mul, ext_to_f64};
use std::collections::BTreeMap;

// combine models: given pv, em, r -> pmt
fn c_sse(pv: f64, em: f64, r: f64) -> f64 {
    (pv / em) * r
} // RN(RN(pv/em)*r)
fn c_numr(pv: f64, em: f64, r: f64) -> f64 {
    (pv * r) / em
} // RN(RN(pv*r)/em)
fn c_afac(pv: f64, em: f64, r: f64) -> f64 {
    pv / (em / r)
} // annuity factor a=em/r first: RN(pv/RN(em/r))
fn c_rem(pv: f64, em: f64, r: f64) -> f64 {
    pv * (r / em)
} // RN(pv*RN(r/em))
fn c_x87(pv: f64, em: f64, r: f64) -> f64 {
    // quotient & product at PC64, store 53
    ext_to_f64(&ext_mul(&ext_div(&ef(pv), &ef(em), CW), &ef(r), CW), CW)
}
fn c_qext(pv: f64, em: f64, r: f64) -> f64 {
    // quotient extended, *r then store
    let q = ext_div(&ef(pv), &ef(em), CW);
    ext_to_f64(&ext_mul(&q, &ef(r), CW), CW)
}
fn c_afac_x87(pv: f64, em: f64, r: f64) -> f64 {
    // a=RN64(em/r) ext, pv/a store 53
...
fn main() {
    ...
    let combines: [(&str, fn(f64, f64, f64) -> f64); 7] = [
        ("SSE2", c_sse),
        ("num*r", c_numr),
        ("afac a=em/r", c_afac),
        ("pv*(r/em)", c_rem),
    ...
    for (cn, cf) in combines {
        ...
        let mut per_config_pin_distinct = 0u32;
        let mut tot_cfg = 0u32;
            ...
            for d in -20..=20i64 {
                ...
                let ok = idxs.iter().all(|&i| {
                    cfgs[i]
                        .rows
                        .iter()
                        .all(|(pv, want)| cf(*pv, em, cfgs[i].r).to_bits() == *want)
                });
                if ok {
                    found = true;
                    break;
        ...
        println!(
            "combine={:<7}: groups where a SINGLE em fits ALL configs: {}/{}   (total distinct-em across groups: {})",
