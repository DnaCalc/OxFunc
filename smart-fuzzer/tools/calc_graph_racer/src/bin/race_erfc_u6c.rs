//! Leftover-low 6 coverage of E0+1 / R[1]+1 / derfc0 / up(E0+1). Not an identity.
...
fn main() {
    ...
    for &lz in &SIX {
        ...
        let d0 = ulp_distance(qw(lz, f::nswc_derfc0(lz)), t).unwrap_or(99) == 0;
        let any = e || u || r1 || d0;
        if any {
            hit_any += 1;
        }
        println!(
            "  z={lz:.16} E0+1={e} upE0={u} R1={r1} derfc0={d0} any={any}"
        );
    }
    ...
    for row in rows.iter().filter(|rr| rr.z >= 4.0) {
        ...
        let d0 = ulp_distance(qw(row.z, f::nswc_derfc0(row.z)), t).unwrap_or(99) == 0;
        if e || u || r1 || d0 {
            qe_u += 1;
            if row.direct {
                qd_u += 1;
    ...
    println!("union Q={qe_u} d={qd_u} (bar 0x5005 1572/53) invented glue");
}
