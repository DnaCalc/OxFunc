...
const RC_RM: u16 = 0x0400;
const RC_RP: u16 = 0x0800;

fn spill(x: &Ext80, cw: u16) -> Ext80 {
    ef(ext_to_f64(x, cw))
} // round to double, back to ext
...
fn em(r: f64, n: f64, c: Cfg) -> f64 {
    ...
    let mut ln1p = if r.abs() < 0.292893218813452 {
        ext_fyl2xp1(&ln2, &ef(r), a)
    } else {
        ext_fyl2x(&ln2, &ext_add(&ext_one(), &ef(r), a), a)
    };
    if c.spill & 1 != 0 {
        ln1p = spill(&ln1p, a);
    }
    ...
    let mut l = if c.lndel == 0 {
        ext_fyl2x(&ln2, &u, a)
    } else {
        ext_fyl2xp1(&ln2, &ext_sub(&u, &ext_one(), a), a)
    };
    if c.spill & 32 != 0 {
        l = spill(&l, a);
    }
...
fn main() {
    ...
    for (path, po2) in [
        ("../../work/w109/G6-solvers/answers-pmt-po2n.json", true),
    ...
    let ariths = [
        ("PC53_RN", CW_PC53_RN),
        ("PC64_RN", CW_PC64_RN),
        ("PC64_RZ", CW_PC64_RN | RC_RZ),
        ("PC64_RM", CW_PC64_RN | RC_RM),
        ("PC64_RP", CW_PC64_RN | RC_RP),
                ...
                for lndel in 0..2u8 {
                    for assoc in 0..3u8 {
                        ...
                        for (r, n, ep, ispo2) in &pins {
                            if em(*r, *n, c).to_bits() == ep.to_bits() {
                                if *ispo2 { po2ok += 1 } else { genok += 1 }
                            }
                        }
                        best.push((
                            po2ok,
                            genok,
                            format!(
                                "arith={} fin={} spill={:06b} ln={} assoc={}",
                                an, fnm, spill, lndel, assoc
                            ),
                        ));
                    }
                }
    ...
    for (p, g, d) in best.iter().take(20) {
        println!("  po2 {:>3} gen {:>3} tot {:>3}  {}", p, g, p + g, d);
    }
}
