...
fn main() {
    ...
    let mut acc_ok = 0;
    let mut acc_c = 0;
    let mut acc_h: BTreeMap<i64, usize> = BTreeMap::new();
    let mut wall_ok = 0;
    let mut wall_c = 0;
    let mut by_fv: BTreeMap<i64, (usize, usize)> = BTreeMap::new();
    for row in &rows {
        if row.r == 0.0 || 1.0 + row.r <= 0.0 || row.n <= 0.0 {
            continue;
        }
    ...
    println!(
        ...
        acc_ok,
        acc_c,
        100.0 * acc_ok as f64 / acc_c as f64
    );
    println!(
        "  expm1-wall (|tau|<1):   {}/{} ({:.1}%)",
        wall_ok,
        wall_c,
        100.0 * wall_ok as f64 / wall_c as f64
    );
