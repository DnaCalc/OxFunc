//! NORMSDIST |z|>=4 glue ∪ w+4 last-store. Not an identity.
...
fn main() {
    ...
    let mut h_w = 0usize;
    let mut h_u = 0usize;
    let mut only_w = 0usize;
    println!("NORMSDIST |z|>=4 glue ∪ w+4:");
    for w in &bank.witnesses {
        ...
        let ww = poke(f::w_rn53(a), 4);
        let pw = phi_from(z, qmul(ww, f::cephes_f(a)));
        let w4 = ulp_distance(pw, t).unwrap_or(99) == 0;
        if g {
            h_g += 1;
        }
        if w4 {
    ...
    println!("n={n} glue={h_g} w+4={h_w} only_w4={only_w} union={h_u}/{n}");
}
