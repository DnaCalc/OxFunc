...
#[derive(Clone, Copy)]
enum LP { Log1pCR, LnOf1PlusR, Fyl2xp1Nat }
...
fn lp_name(lp: LP) -> &'static str { match lp { LP::Log1pCR=>"log1pCR", LP::LnOf1PlusR=>"ln(1+r)", LP::Fyl2xp1Nat=>"fyl2xp1" } }

// ---- natural expm1(t) providers ------------------------------------------
#[derive(Clone, Copy)]
enum EP { Expm1CR, Expm1Internal, ExpMinus1 }
...
fn ep_name(ep: EP) -> &'static str { match ep { EP::Expm1CR=>"expm1CR", EP::Expm1Internal=>"expm1Int", EP::ExpMinus1=>"exp-1" } }

// ---- t = -n·L staging ----------------------------------------------------
#[derive(Clone, Copy)]
enum TR { X87Mul, Sse2 }
fn t_of(tr: TR, n: f64, l: f64) -> f64 {
    match tr { TR::X87Mul => rx::x87_mul(-n, l), TR::Sse2 => (-n) * l }
}
fn tr_name(tr: TR) -> &'static str { match tr { TR::X87Mul=>"x87*", TR::Sse2=>"sse2*" } }

// ---- outer arrangement ---------------------------------------------------
#[derive(Clone, Copy)]
enum AR { NumROverDen, NumOverDenR, NumROverTfOverEm, LibreSplit, VIndependent }
fn ar_name(ar: AR) -> &'static str {
    match ar { AR::NumROverDen=>"numR/den", AR::NumOverDenR=>"num/den*r", AR::NumROverTfOverEm=>"numR/tf/em", AR::LibreSplit=>"libreSplit", AR::VIndependent=>"vIndep" }
}
...
fn load_obs(path: &str) -> Vec<(Vec<f64>, u64)> {
    ...
    for w in &ws.witnesses {
        let a: Vec<f64> = w.args.iter().filter_map(|x| match x {
...
const ARS: [AR;5] = [AR::NumROverDen, AR::NumOverDenR, AR::NumROverTfOverEm, AR::LibreSplit, AR::VIndependent];

fn score(obs: &[(Vec<f64>, u64)], lp: LP, ep: EP, tr: TR, ar: AR) -> u32 {
    obs.iter().filter(|(a,want)| model(lp,ep,tr,ar,a[0],a[1],a[2],a[3],a[4]).to_bits()==*want).count() as u32
}

fn main() {
    let argv: Vec<String> = std::env::args().collect();
    let train = argv.get(1).cloned().unwrap_or_else(|| "../../work/w109/G6-solvers/answers-pmt-r0.json".into());
    let held  = argv.get(2).cloned().unwrap_or_else(|| "../../work/w109/G6-solvers/answers-pmt-heldout.json".into());
    let tr_obs = load_obs(&train);
    let ho_obs = load_obs(&held);
    println!("train {} rows ({train})  held {} rows ({held})", tr_obs.len(), ho_obs.len());

    let mut cand: Vec<(u32,u32,LP,EP,TR,AR)> = Vec::new();
    for &lp in &LPS { for &ep in &EPS { for &tr in &TRS { for &ar in &ARS {
        cand.push((score(&ho_obs,lp,ep,tr,ar), score(&tr_obs,lp,ep,tr,ar), lp,ep,tr,ar));
    }}}}
    cand.sort_by(|a,b| b.0.cmp(&a.0).then(b.1.cmp(&a.1)));
    println!("\nTOP 16 by HELD-OUT ({}):", ho_obs.len());
    for (h,tr_s,lp,ep,tr,ar) in cand.iter().take(16) {
        println!("  held {:4}/{}  train {:3}/{}  L={:8} E={:8} t={:6} arr={}",
            h, ho_obs.len(), tr_s, tr_obs.len(), lp_name(*lp), ep_name(*ep), tr_name(*tr), ar_name(*ar));
    }

    // n=1 isolation lane: pure test of expm1(-log1p(r)). Report exact-count per provider.
    println!("\n=== n=1 ISOLATION LANE (pure em provider test) ===");
    let n1: Vec<_> = ho_obs.iter().filter(|(a,_)| a[1]==1.0).cloned().collect();
    ...
    for (s,lp,ep,tr,ar) in n1c.iter().take(8) {
        println!("  {:4}/{}  L={:8} E={:8} t={:6} arr={}", s, n1.len(), lp_name(*lp), ep_name(*ep), tr_name(*tr), ar_name(*ar));
    }

    // residual structure for the held-out champion
    let (_,_,lp,ep,tr,ar) = cand[0];
    println!("\n=== CHAMPION residual on held-out: L={} E={} t={} arr={} ===", lp_name(lp), ep_name(ep), tr_name(tr), ar_name(ar));
    ...
    for (n,(ex,off)) in &by_n { println!("  n={:5}  exact {:4}  off {:4}", n, ex, off); }
}
