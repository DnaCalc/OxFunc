...
fn exp_ext(t: &Ext80) -> Ext80 {
    let tt = ext_mul(t, &ext_l2e(), CW);
    let k = ext_rndint(&tt, CW);
    let f = ext_sub(&tt, &k, CW);
    let neg = tf(&f) < 0.0;
...
fn m1(r: f64, n: f64) -> f64 {
    let te = tau_ext(r, n);
    let ud = ext_to_f64(&exp_ext(&te), CW);
    let u = e(ud);
...
fn m4(r: f64, n: f64) -> f64 {
    ...
    let u = rx::excel_exp(t);
    let num = (u - 1.0) * t;
    tf(&ext_div(&e(num), &e(rx::excel_ln(u)), CW))
}
