//! 4 LOW: last-mul z*(1+w) RN/RU/RD vs lead+2. Not an identity.
...
fn main() {
    ...
    let graphs: [(&str, Box<dyn Fn(f64) -> f64>); 6] = [
        (
            "RN",
            Box::new(|z| ext_to_f64(&ext_mul(&ef(z), &one_w(z), CW), CW)),
        ),
        (
            "RU",
            Box::new(|z| ext_to_f64(&ext_mul(&ef(z), &one_w(z), CW_RU), CW_RU)),
        ),
        (
            "RD",
            Box::new(|z| ext_to_f64(&ext_mul(&ef(z), &one_w(z), CW_RD), CW_RD)),
        ),
        (
            "RUst",
            Box::new(|z| ext_to_f64(&ext_mul(&ef(z), &one_w(z), CW), CW_RU)),
        ),
        (
            "up1",
            Box::new(|z| {
                ext_to_f64(&ext_mul(&ef(z), &one_w(z), CW), CW).next_up()
            }),
        ),
        (
            "lead+2",
            Box::new(|z| {
                ext_to_f64(
                    &ext_mul(&ef(z.next_up().next_up()), &one_w(z), CW),
                    CW,
                )
            }),
        ),
    ];
    println!("4 LOW last-mul RN/RU/RD:");
    for &lz in &LOWS {
