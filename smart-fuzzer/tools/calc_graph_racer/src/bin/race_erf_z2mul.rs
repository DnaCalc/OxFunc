//! 4 LOW: last-mul (2z)*(1+w)/2 vs z*(1+w). Not an identity.
    ...
    let graphs: [(&str, Box<dyn Fn(f64) -> f64>); 5] = [
        ...
        (
            ...
            Box::new(|z| {
                ...
                let p = ext_mul(&ef(2.0 * z), &ow, CW);
                ext_to_f64(&ext_div(&p, &ef(2.0), CW), CW)
            }),
        ),
        (
            "(z+z)*ow/2",
            Box::new(|z| {
                ...
                let zz = ext_add(&ef(z), &ef(z), CW);
                let p = ext_mul(&zz, &ow, CW);
                ext_to_f64(&ext_div(&p, &ef(2.0), CW), CW)
            }),
        ),
        (
            "f64 2z/2",
            Box::new(|z| joint_t((2.0 * z) * 0.5, z)),
        ),
        ("lead+2", Box::new(|z| joint_t(z.next_up().next_up(), z))),
    ];
    println!("4 LOW last-mul (2z)*(1+w)/2:");
    for &lz in &LOWS {
