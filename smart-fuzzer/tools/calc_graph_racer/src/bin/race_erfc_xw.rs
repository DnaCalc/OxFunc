//! 3 stubborn: w(z±k)*F(z±k) cross last-mul. Not an identity.
...
fn main() {
    ...
    let graphs: [(&str, Box<dyn Fn(f64) -> f64>); 8] = [
        ("wF", Box::new(|z| mul(f::w_rn53(z), cephes(z)))),
        ("wdn*F", Box::new(|z| mul(f::w_rn53(z.next_down()), cephes(z)))),
        ("w*Fdn", Box::new(|z| mul(f::w_rn53(z), cephes(z.next_down())))),
        (
            "wdn*Fdn",
            Box::new(|z| mul(f::w_rn53(z.next_down()), cephes(z.next_down()))),
        ),
        (
            "wdn2*F",
            Box::new(|z| {
                mul(
                    f::w_rn53(z.next_down().next_down()),
                    cephes(z),
                )
            }),
        ),
        (
            ...
            Box::new(|z| {
                mul(
                    f::w_rn53(z),
                    cephes(z.next_down().next_down()),
                )
            }),
        ),
        ("wup*F", Box::new(|z| mul(f::w_rn53(z.next_up()), cephes(z)))),
        ("w*upF", Box::new(|z| mul(f::w_rn53(z), cephes(z).next_up()))),
    ];
    println!("3 stubborn w×F cross:");
    for &lz in &STUB {
