...
fn main() {
    ...
    for (r, em) in &rows {
        ...
        if kahan_f64(-l1).to_bits() == em.to_bits() {
            *res.entry("fyl2x(ext 1+r) stored").or_default() += 1;
        }
        ...
        let ln = rx::excel_ln(u);
        let emc = if u == 1.0 {
            tf64
        } else if tf64.abs() < 1.0 {
            tf(&rx::ext_mul(&e(u - 1.0), &tau_ext, CW)) / ln
        } else {
            u - 1.0
        };
        if emc.to_bits() == em.to_bits() {
            *res.entry("fyl2x(ext 1+r) EXT numer").or_default() += 1;
        }
        ...
        let emf = if u2 == 1.0 {
            tf(&te)
        } else if tf(&te).abs() < 1.0 {
            tf(&rx::ext_div(
                &rx::ext_mul(&rx::ext_sub(&ue, &rx::ext_one(), CW), &te, CW),
                &lnu2,
                CW,
            ))
        } else {
        ...
        if emf.to_bits() == em.to_bits() {
            *res.entry("FULL extended (fyl2xp1)").or_default() += 1;
        }
    }
