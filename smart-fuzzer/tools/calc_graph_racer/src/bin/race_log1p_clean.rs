...
fn l1p_fdlibm(x: f64) -> f64 {
    ...
    if hu == 0 {
        if f == 0.0 {
            if k == 0 {
                ...
                let c2 = c + k as f64 * LN2_LO;
                return k as f64 * LN2_HI + c2;
            }
        }
    ...
    if k == 0 {
        ...
        k as f64 * LN2_HI - ((hfsq - (s * (hfsq + r) + (k as f64 * LN2_LO + c)) as f64) - f)
    }
}
...
fn l1p_cephes(x: f64) -> f64 {
    ...
    let z0 = 1.0 + x;
    if z0 < SQRTH || z0 > SQRT2 {
        return rx::excel_ln(z0);
    }
    // polevl(x, LP, 6)
    let mut pn = LP[0];
    for i in 1..7 {
...
fn tau_for(r: f64, n: f64, l1p: fn(f64) -> f64) -> f64 {
    -(n * l1p(r))
}
...
fn main() {
    ...
    for (setname, path) in [
        ...
        for ((rb, nb), rows) in &data {
            ...
            if let Some(e) = pin_gen(rows, r, km) {
                pins.push((r, n, e, onepr_exact(r)));
            }
        }
        let n_exact = pins.iter().filter(|p| p.3).count();
        println!(
            "\n==== {} : {} configs pinned ({} with 1+r exact) ====",
            setname,
            pins.len(),
            n_exact
        );
        println!(
            "{:<10} {:>10} {:>8} {:>8} {:>10}",
            "cand", "em-repro", "groups", "conflG", "conflCfg"
        );
        for (nm, cf) in cands {
            // Metric A: em reproduction
            let repro = pins
                .iter()
                .filter(|(r, n, ep, _)| em_for(*r, *n, cf).to_bits() == ep.to_bits())
                .count();
            // Metric B: clean-function test - group by tau_cand bits
            let mut groups: BTreeMap<u64, Vec<u64>> = BTreeMap::new();
            for (r, n, ep, _) in &pins {
                groups
                    .entry(tau_for(*r, *n, cf).to_bits())
                    .or_default()
                    .push(ep.to_bits());
            }
            let ngroups = groups.len();
            let mut confl_g = 0usize;
            let mut confl_cfg = 0usize;
    ...
    // ---- DUMP collide families to CSV for partition analysis ----
    {
        ...
        for ((rb, nb), r_rows) in &data {
            ...
            let n = f64::from_bits(*nb);
            let km = rx::excel_expm1_internal(-(n * rx::excel_log1p(r)));
            if let Some(e) = pin_gen(r_rows, r, km) {
                rows.push((r, n, e));
            }
        }
        let mut out =
            String::from("r_bits,n,em_pinned,tau_cr,tau_fyl,tau_ucrt,tau_fdlibm,tau_cephes\n");
        for (r, n, em) in &rows {
            out.push_str(&format!(
                ...
                *n as u64,
                em.to_bits(),
                tau_for(*r, *n, l1p_cr).to_bits(),
                tau_for(*r, *n, l1p_fyl).to_bits(),
                tau_for(*r, *n, l1p_ucrt).to_bits(),
                tau_for(*r, *n, l1p_fdlibm).to_bits(),
    ...
    for ((rb, _), _) in &data {
        let r = f64::from_bits(*rb);
        if l1p_std(r).to_bits() != l1p_ucrt(r).to_bits() {
            diff += 1;
        }
    }
    println!(
        "\nstd_ln1p vs UCRT extern log1p differ on {}/{} collide rates",
        diff,
        data.len()
    );
}
