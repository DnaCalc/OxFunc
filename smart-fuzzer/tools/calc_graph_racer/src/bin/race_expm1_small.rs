...
fn g_extu(r: f64, n: f64) -> f64 {
    ...
    let xe = e(t);
    let tt = ext_mul(&xe, &ext_l2e(), CW);
    let k = ext_rndint(&tt, CW);
    let ff = ext_sub(&tt, &k, CW);
    let neg = tf(&ff) < 0.0;
    let w = ext_f2xm1(&ext_abs(&ff, CW), CW);
    let mut m = ext_add(&w, &ext_one(), CW);
    if neg {
        m = ext_div(&ext_one(), &m, CW);
    }
...
fn main() {
    ...
    for i in 0..7 {
        println!("  {:<14} {}/{}", names[i], score[i], tot);
    }
    println!(
        "\ndetail (offset from Excel em; 99=out-of-domain) [{}]:",
        names.join(",")
    );
    for d in &detail {
        println!("  {}", d);
