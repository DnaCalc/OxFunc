...
struct T{ v:f64, em:f64, p:f64, q:f64, pm1:f64 }
...
fn fname(f:F)->&'static str{match f{F::DiscDivRR=>"disc RN(pv*r)/em",F::DiscFull=>"disc (pv+fvv)r/(tf em)",F::FwdPq=>"fwd -(pvP+fv)/(tf q)",F::FwdPqRR=>"fwd -RN(pvP)/q",F::FwdPm1=>"fwd -(pvP+fv)r/(tf pm1)",F::FwdPm1RR=>"fwd -RN(pvP)r/pm1",F::Recip=>"recip prod-order"}}

fn combine(f:F, pv:f64,fv:f64,ty:f64,r:f64,t:&T)->f64{
    let tf = 1.0 + r*ty;
    match f{
        F::DiscDivRR => (pv*r)/t.em,
        F::DiscFull => { let num=pv+fv*t.v; (num*r)/(tf*t.em) }
        F::FwdPq   => -(pv*t.p+fv)/(tf*t.q),
        F::FwdPqRR => -(pv*t.p)/(tf*t.q),
        F::FwdPm1  => -(pv*t.p+fv)*r/(tf*t.pm1),
        F::FwdPm1RR=> -(pv*t.p)*r/(tf*t.pm1),
        F::Recip   => { let num=pv+fv*t.v; let recip=1.0/(tf*(1.0-t.v)); -num*r*recip }
    }
}
...
fn load(path:&str)->Vec<(f64,f64,f64,f64,f64,u64)>{
    ...
    for w in &ws.witnesses{
        ...
        if a.len()!=5{continue}
        if let Some(want)=parse_bits_hex(&w.expected_bits){o.push((a[0],a[1],a[2],a[3],a[4],want.to_bits()))}
    }
    o
}
...
fn main(){
    ...
    let forms=[F::DiscDivRR,F::DiscFull,F::FwdPq,F::FwdPqRR,F::FwdPm1,F::FwdPm1RR,F::Recip];
    ...
    for f in forms{
        ...
        for (pvb,(ok,off)) in &perpv{
            print!("  pv{:.3}={:3}%", f64::from_bits(*pvb), 100*ok/(ok+off));
        }
        println!();
    }
    ...
    println!("\ntotals on pv-ladder:");
    for f in forms{
        let ok=ladder.iter().filter(|(r,n,pv,fv,ty,want)|{let t=trans(*r,*n); combine(f,*pv,*fv,*ty,*r,&t).to_bits()==*want}).count();
        println!("  {:26} {}/{} ({:.1}%)", fname(f), ok, ladder.len(), 100.0*ok as f64/ladder.len() as f64);
    }
}
