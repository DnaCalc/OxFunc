...
fn main(){
    for cn in ["fvsweep","fvty","fv1sweep","heldout"]{
        ...
        for w in &ws.witnesses{
            ...
            let tf=1.0+r*ty;
            let g=[
                { let num=pv+fv*v; (num/em)/tf*r },
                { let num=fv*v+pv; (num/em)/tf*r },
                { let num=pv+fv*v; (num*r)/(tf*em) },
                { -(pv*bigp+fv)*r/(tf*(bigp-1.0)) },
                { let q=(bigp-1.0)/r; -(pv*bigp+fv)/(tf*q) },
                { let num=pv+fv*v; ((num/em)*r)/tf },
                { let num=pv+fv*vexp; (num/em)/tf*r },
                { let num=pv*bigp+fv; (num/(bigp-1.0))/tf*r*-1.0 },
            ];
            tot+=1;
            for i in 0..8{ if g[i].to_bits()==want{sc[i]+=1;} }
            let d=(sordi(g[0].to_bits())-sordi(want)).clamp(-12,12);
            *off.entry(d).or_default()+=1;
            if sample.is_empty(){
                sample=format!("  sample r={} n={} pv={} fv={} ty={}: want={:016x} v0={:016x} v3(fwd)={:016x}",
                    r,n,pv,fv,ty,want,g[0].to_bits(),g[3].to_bits());
            }
        }
        println!("=== {} (N={}) ===",cn,tot);
        for i in 0..8{ println!("  [{}] {:<34} {}/{}",i,variants[i],sc[i],tot); }
        print!("  off(v0):"); for (d,c) in &off{print!(" {:+}:{}",d,c);} println!();
        println!("{}",sample);
    }
}
