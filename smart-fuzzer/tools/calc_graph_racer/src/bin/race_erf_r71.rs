//! 71 1-ULP remaining after joint last-mul glue vs fd/m856 two-mode. Not an identity.
...
fn tm(z: f64, oy: Ext80, t: f64) -> bool {
...
fn main() {
    ...
    let hit_jtm = |z: f64, t: f64| tm(z, one_w(z), t);
    let mut n = 0usize;
    let mut h_fd = 0usize;
    let mut h_m = 0usize;
    let mut h_u = 0usize;
    println!("remaining 1-ULP vs fd/m856 two-mode ±:");
    for &(z, bits) in &rows {
        let t = f64::from_bits(bits);
        let g = jn(z);
        let ej = ulp_distance(g, t).unwrap_or(99) == 0;
        if ej || hit_ls(z, t) || hit_jtm(z, t) {
            continue;
        }
        n += 1;
        let fd = tm(z, one_y_fd(z), t);
        let mm = tm(z, one_y_m(z), t);
        if fd {
            h_fd += 1;
        }
        if mm {
        ...
        if fd || mm {
            println!("  HIT z={z:.16} fd={fd} m856={mm}");
        }
    }
    println!("n={n} fd={h_fd} m856={h_m} union={h_u}");
}
