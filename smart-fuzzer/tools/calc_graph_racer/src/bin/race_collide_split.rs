...
use rx::{Ext80, ext_add, ext_chs, ext_from_f64 as ef, ext_fyl2x, ext_fyl2xp1, ext_ln2, ext_mul,
         ext_one, ext_sub, ext_to_f64, CW_PC53_RN, CW_PC64_RN};
...
#[derive(Clone,Copy)]
struct Cfg{ lnvar:u8, deriv:u8, cw_prod:u16, cw_add:u16, fin:u16, base:u8 }
fn em(r:f64,n:f64,c:Cfg)->f64{
    ...
    let e_base = match c.base{ 0=>rx::excel_expm1_internal(tau0), _=>{
        // CR-ish double expm1 via ext exp - 1 at PC64 of tau0
        let ee=rx::excel_exp(tau0); ee-1.0 } };
    let u = match c.deriv{ 0=>rx::excel_exp(tau0), _=> 1.0+e_base };
...
fn main(){
    ...
    for lnvar in 0u8..2{ for base in 0u8..2{ for deriv in 0u8..2{
      for (pn,cw_prod) in pcs{ for (an,cw_add) in pcs{ for (fnn,fin) in fins{
        let c=Cfg{lnvar,deriv,cw_prod,cw_add,fin,base};
        let ok=pins.iter().filter(|(r,n,ep,_)| em(*r,*n,c).to_bits()==ep.to_bits()).count() as u32;
        best.push((ok,format!("lnvar={} base={} deriv={} prod={} add={} fin={}",
            lnvar, if base==0{"x87int"}else{"exp-1"}, if deriv==0{"exp"}else{"1+E"}, pn, an, fnn)));
      }}}
    }}}
    best.sort_by(|a,b| b.0.cmp(&a.0));
    println!("\nTOP split configs on collision ({} pts):", ntot);
    for (ok,d) in best.iter().take(12){ println!("  {:>3}/{}  {}", ok, ntot, d); }
    println!("configs EXCEEDING {}: {}", kbase, best.iter().filter(|x|x.0>kbase as u32).count());
}
