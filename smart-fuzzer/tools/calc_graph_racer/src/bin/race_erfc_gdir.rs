//! Signed ulp≥2 by dyad/aaa/555 on joint, 0x210, 0x5005. Not an identity.
...
fn main() {
    ...
    let names = ["dyad", "aaa", "555", "other"];
    let mut lo = [0usize; 4];
    let mut hi = [0usize; 4];
    println!("P joint ulp>=2 signed:");
    for &(z, bits) in &prows {
    ...
    for i in 0..4 {
        println!("  {} LOW={} HIGH={}", names[i], lo[i], hi[i]);
    }

    let qrows = f::load_q_rows_tagged(&dir);
    let qsign = |loz: f64, hiz: f64, ev: &dyn Fn(f64) -> f64, tag: &str| {
        let mut lo = [0usize; 4];
        let mut hi = [0usize; 4];
        println!("{tag} DIRECT ulp>=2 signed:");
        for r in &qrows {
            if !r.direct || r.z < loz || r.z >= hiz {
                continue;
            }
            let t = f64::from_bits(r.qbits);
            let g = qwf(r.z, ev(r.z));
            let d = ulp_distance(g, t).unwrap_or(99);
            if d < 2 {
                continue;
            }
    ...
    qsign(0.5, 4.0, &|z| cody(z), "0x210 mid");
    qsign(4.0, 1e300, &|z| cephes(z), "0x5005 tail");
}
