...
use rx::{CW_PC53_RN, CW_PC64_RN, ext_from_f64, ext_fyl2x, ext_fyl2xp1, ext_ln2, ext_mul, ext_to_f64};
...
fn main() {
    ...
    for line in csv.lines().skip(1) {
        ...
        let den_ext = if use_p1 { ext_fyl2xp1(&ln2, &a_ext, RN64) } else { ext_fyl2x(&ln2, &u_ext, RN64) };
        let den_dbl = ext_to_f64(&den_ext, RN53);
        if den_dbl.to_bits() != lnu.to_bits() { den_differs += 1; }
        ...
        for i in 0..5 { if cands[i].to_bits() == emb { score[i]+=1; if base_miss { score_miss[i]+=1; } } }
        if base_miss { if c1.to_bits()==emb { c1_fix+=1; } else { still_miss.push((k,n,c1.to_bits() as i64 - emb as i64)); } }
        else if c1.to_bits()!=emb { c1_break+=1; }
    }

    println!("N={} misses={} | used FYL2XP1 on {} rows | FYL2XP1-dbl != FYL2X-dbl on {} rows", tot, nmiss, used_p1, den_differs);
    ...
    println!("\nlog1p-denominator(1) FIXES {} miss rows, BREAKS {} hit rows", c1_fix, c1_break);
    println!("still-miss after c1 ({}): first 20:", still_miss.len());
    for x in still_miss.iter().take(20) { println!("  k={:3} n={:3} c1-em={:+}", x.0, x.1, x.2); }
}
