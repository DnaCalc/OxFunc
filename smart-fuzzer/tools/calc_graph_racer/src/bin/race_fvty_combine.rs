...
fn c7(r: f64, n: f64, pv: f64, fv: f64, ty: f64) -> f64 {
    ...
    let num = -(pv * p + fv); let denom = (-em * p) * tf; num * r / denom
}
...
fn main() {
    ...
    let cands: [(&str, F); 8] = [
        ("1 landed", c1), ("2 v=exp", c2), ("3 posP 1/v", c3), ("4 posP exp", c4),
        ("5 r-in-num", c5), ("6 tf-recip", c6), ("7 posPnum/emden", c7), ("8 tf-last", c8),
    ];
    print!("{:12}{:>7}", "corpus", "N");
    for (nm, _) in &cands { print!(" {:>12}", nm); }
    println!();
    for cn in corpora {
        ...
        print!("{:12}{:>7}", cn, tot);
        for i in 0..8 { print!(" {:>11.1}%", 100.0 * sc[i] as f64 / tot as f64); }
        println!();
    }
}
