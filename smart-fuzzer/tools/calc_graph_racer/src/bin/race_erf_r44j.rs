//! joint last-mul glue ∪ unpoked fused ∪ unpoked z+1. Not an identity.
...
fn z1(z: f64, a: &[f64; 21]) -> f64 {
    ext_to_f64(&ext_mul(&ef(z.next_up()), &one_w_a(z, a), CW), CW)
}
...
fn main() {
    ...
    for &(z, bits) in &rows {
        ...
        let ej = ulp_distance(a21(z, &ja), t).unwrap_or(99) == 0
            || hit_ls(z, t, &ja)
            || hit_tm(z, t, &ja);
        let ecr = ulp_distance(a21(z, &AS0), t).unwrap_or(99) == 0;
        let ez1 = ulp_distance(z1(z, &AS0), t).unwrap_or(99) == 0;
        if ej {
            n_glue += 1;
        }
        if ecr {
    ...
    println!(
        "keep joint={n_j} glue={n_glue} unpoked={n_cr} unpoked-z+1={n_z1} union={n_any}/{} miss={miss}",
        rows.len()
    );
}
