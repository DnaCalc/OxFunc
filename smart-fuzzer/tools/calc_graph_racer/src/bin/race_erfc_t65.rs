//! 0x4e05 + Q[1]+1 Q[2]+1 Q[3]-1 vs leftover DIRECT z>=4. Not an identity.
...
fn report(tail: &[&f::QRow], name: &str, q: &[f64; 8], q0: &[f64; 8]) {
    let mut both = 0usize;
    ...
    for r in tail {
        ...
        let g = mul(f::w_rn53(r.z), cephes(r.z, q));
    ...
    println!(
        "  {name} fused={nf} both={both} only_m={only_m} only_4e05={only_b} leftover-low {hit_lo}/{nlo} leftover-high {hit_hi}/{nhi} CONV={n_conv} LOSE={n_lose}"
    );
    println!(
        "    self leftover-low {slo_c}/{slo} leftover-high {shi_c}/{shi} 1-ULP {u1}/{n1}"
    );
}
...
fn main() {
    ...
    let a = q65();
    let b = q64();
    println!("DIRECT z>=4 n={n} 65/64 vs 0x4e05 leftover:");
    report(&tail, "Q[1]+1 Q[2]+1 Q[3]-1", &a, &q0);
    report(&tail, "Q[0]+1 Q[2]+1 Q[3]-1", &b, &q0);
