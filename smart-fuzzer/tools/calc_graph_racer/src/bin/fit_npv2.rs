...
fn xpow(base: f64, i: u32) -> f64 {
    rx::excel_pow_positive(base, i as f64)
}
...
fn rev_xpow_sse(rate: f64, cf: &[f64]) -> f64 {
    ...
    for (i, &c) in cf.iter().enumerate() {
        terms.push(c / xpow(base, (i + 1) as u32));
    }
    let mut t = 0.0;
...
fn rev_powf_sse(rate: f64, cf: &[f64]) -> f64 {
    ...
    let mut terms: Vec<f64> = Vec::new();
    for (i, &c) in cf.iter().enumerate() {
        terms.push(c / base.powf((i + 1) as f64));
    }
    let mut t = 0.0;
    for &x in terms.iter().rev() {
        t += x;
...
fn main() {
    ...
    for (r, cf, want, id) in &obs {
        ...
        if a != *want || b != *want {
            println!(
                "  {id:10} rate={r:.10} rhΔ={:+} xpowΔ={:+}",
                a as i64 - *want as i64,
                b as i64 - *want as i64
            );
        }
    }
}
