//! NSWC A21 converted to Chebyshev in t=x²/2−1, Clenshaw vs Schonfelder vs Horner.
//! Not an identity.
...
const A21C: [f64; 18] = [
    1.48311056409992292338e+00,
    -3.01071073372071673635e-01,
    6.89948307027011542569e-02,
    -1.39162712542080947015e-02,
...
fn main() {
    ...
    let graphs: [(&str, fn(f64) -> f64); 3] = [
        ("Schonfelder Clenshaw", |z| clenshaw(z, &SCH)),
        ("A21-as-Cheb Clenshaw", |z| clenshaw(z, &A21C)),
        ("A21 Horner 1+w", erf_a21),
    ];
    for (name, ev) in graphs {
        ...
        println!("{name:24} {ex}/{n} max={maxu} m1={m1} sum={sum}");
    }

    println!("## Schonfelder leftover signed-ULP");
    let mut prev = 0i8;
    let mut run = 0usize;
    let mut nchg = 0usize;
    ...
    for &(z, pbits) in &rows {
        let pg = clenshaw(z, &SCH);
        let po = f64::from_bits(pbits);
        let d = ulp_distance(pg, po).unwrap_or(0);
    ...
    runs.sort_unstable();
    let med = if runs.is_empty() { 0 } else { runs[runs.len() / 2] };
    let mx = runs.iter().copied().max().unwrap_or(0);
    println!(
        "nonzero {} sign_chg {} runs {} median {} maxrun {}",
        nz,
        nchg,
        runs.len(),
        med,
