//! 0x24a5-66 vs 0x4e05-65 fused overlap. Not an identity.
...
fn hit(z: f64, t: f64, p: &[f64; 9], q: &[f64; 8], mask: u32) -> bool {
    ulp_distance(mul(f::w_rn53(z), cephes(z, p, q, mask)), t).unwrap_or(99) == 0
}
...
fn main() {
    ...
    let mut nb = 0usize;
    let mut nu = 0usize;
    println!("DIRECT z>=4 n={n} 0x24a5-66 vs 0x4e05-65:");
    for r in &tail {
        let t = f64::from_bits(r.qbits);
        let ea = hit(r.z, t, &pa, &qa, 0x24a5);
        let eb = hit(r.z, t, &P0, &qb, 0x4e05);
        let eu = hit(r.z, t, &P0, &Q0, 0);
        if ea {
            na += 1;
        }
        if eb {
    ...
    println!(
        "66={na} 65={nb} unmask={nu} both={both} only_66={only_a} only_65={only_b} union={}",
        both + only_a + only_b
    );
}
