//! j3432 Q-hard 50 split by sign; named F on leftover-low hard. Not an identity.
...
fn main() {
    ...
    println!("j3432 Q-hard DIRECT ulp>=2: high n={} low n={}", hi.len(), lo.len());
    println!("leftover-low Q-hard:");
    for (r, d) in &lo {
    ...
    println!("named F on leftover-low Q-hard:");
    for (name, ev) in &graphs {
        ...
        println!("  {name:10} hit={hit}/{} ulp1={u1}", lo.len());
    }
    println!("named F on leftover-high Q-hard:");
    for (name, ev) in &graphs {
        let mut hit = 0usize;
        let mut u1 = 0usize;
        for (r, _) in &hi {
        ...
        println!("  {name:10} hit={hit}/{} ulp1={u1}", hi.len());
    }
}
