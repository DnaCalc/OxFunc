//! NORMSDIST |z|>=4 union previous glue ∪ w(z+3)*F. Not an identity.
...
fn main() {
    ...
    let mut h_z3 = 0usize;
    let mut h_u = 0usize;
    let mut only_z3 = 0usize;
    println!("NORMSDIST |z|>=4 glue ∪ z+3:");
    for w in &bank.witnesses {
        ...
        let ax = zx.abs();
        let g = hit_ff(z, a, t, f::cephes_f(a))
            || hit_ff(z, a, t, cephes_mask(a, 0))
            || hit_ff(zx, ax, t, f::cephes_f(ax))
            || hit_ff(zx, ax, t, cephes_mask(ax, 0));
        let a3 = poke(a, 3);
        let p3 = phi_from(z, qmul(f::w_rn53(a3), f::cephes_f(a)));
        let z3 = ulp_distance(p3, t).unwrap_or(99) == 0;
        if g {
            h_g += 1;
        }
        if z3 {
        ...
        if !g {
            print!("  MISS x={x:.16} bits={:016x} a={a:.16}", x.to_bits());
            for k in -8i32..=8 {
                if k == 0 {
                    continue;
                }
                let ak = poke(a, k);
                let pk = phi_from(z, qmul(f::w_rn53(ak), f::cephes_f(a)));
                let d = ulp_distance(pk, t).unwrap_or(99);
                print!(" z{k:+}={d}");
            }
            println!();
        }
    }
    println!("n={n} glue={h_g} z+3={h_z3} only_z3={only_z3} union={h_u}/{n}");
}
