...
fn has_nan(v: &[Ext80]) -> bool {
    v.iter().any(|e| {
        let f = ext_to_f64(e, RN53);
        f.is_nan()
    })
}
...
fn x87(a: &[Ext80], b: &[Ext80], op: &str) -> Vec<Ext80> {
    match op {
        "add" => map2(a, b, |x, y| ext_add(x, y, CW)),
        "sub" => map2(a, b, |x, y| ext_sub(x, y, CW)),
        "mul" => map2(a, b, |x, y| ext_mul(x, y, CW)),
        "div" => map2(a, b, |x, y| ext_div(x, y, CW)),
        _ => unreachable!(),
    }
}
fn sse(a: &[Ext80], b: &[Ext80], op: &str) -> Vec<Ext80> {
    match op {
        "add" => map2(a, b, |x, y| ext_from_f64(d(x) + d(y))),
        "sub" => map2(a, b, |x, y| ext_from_f64(d(x) - d(y))),
        "mul" => map2(a, b, |x, y| ext_from_f64(d(x) * d(y))),
        "div" => map2(a, b, |x, y| ext_from_f64(d(x) / d(y))),
        _ => unreachable!(),
    }
}
fn spill(a: &[Ext80]) -> Vec<Ext80> {
    a.iter().map(|x| ext_from_f64(d(x))).collect()
}
fn spill_rz(a: &[Ext80]) -> Vec<Ext80> {
    let rz = CW_PC53_RN | 0x0C00;
    a.iter().map(|x| ext_from_f64(ext_to_f64(x, rz))).collect()
}
fn chs(a: &[Ext80]) -> Vec<Ext80> {
    a.iter()
        .map(|x| ext_sub(&ext_from_f64(0.0), x, CW))
        .collect()
}
fn bits(v: &[Ext80]) -> Vec<u64> {
    v.iter().map(|x| d(x).to_bits()).collect()
}
...
fn eval3_row(bank: &[Node], op: u8, a: u32, b: u32, i: usize) -> Ext80 {
    ...
    match op {
        ...
        2 => ext_mul(x, y, CW),
        3 => ext_div(x, y, CW),
        4 => ext_from_f64(d(x) + d(y)),
        5 => ext_from_f64(d(x) - d(y)),
        6 => ext_from_f64(d(x) * d(y)),
        7 => ext_from_f64(d(x) / d(y)),
        8 => ext_fyl2x(x, y, CW),
        9 => ext_fyl2x(y, x, CW),
        10 => ext_scale(x, y, CW),
        11 => ext_scale(y, x, CW),
...
fn eval3(bank: &[Node], op: u8, a: u32, b: u32) -> Vec<Ext80> {
    ...
    match op {
        ...
        6 => sse(av, bv, "mul"),
        7 => sse(av, bv, "div"),
        8 => map2(av, bv, |y, x| ext_fyl2x(y, x, CW)),
        9 => map2(bv, av, |y, x| ext_fyl2x(y, x, CW)),
        10 => map2(av, bv, |x, k| ext_scale(x, k, CW)),
        11 => map2(bv, av, |x, k| ext_scale(x, k, CW)),
        _ => unreachable!(),
    }
}
...
fn round_iv(em: f64) -> (f64, f64) {
    ...
    (
        dn(dn(em - (em - prev) * 0.5)),
        up(up(em + (next - em) * 0.5)),
    )
}
...
fn main() {
    ...
    if selftest {
        ...
        println!(
            "[SELFTEST] target = spill(div(mul(a_d,tau_d), sub(lnu_d,r))) — expect HIT via join"
        );
    }
    ...
    let mut push_leaf =
        |nm, f: &dyn Fn(&(i32, u32, f64, f64, f64, u64)) -> Ext80, dbl, lv: &mut Vec<_>| {
    ...
    let te = |r: &(i32, u32, f64, f64, f64, u64)| {
        ext_mul(
            &ef(-(r.1 as f64)),
    ...
    push_leaf(
        "a_e",
        &|r| ext_sub(&exp_ext(&te(r), &l2e).0, &one, CW),
        false,
        &mut leaves,
    );
    ...
    push_leaf(
        "lnu_re",
        &|r| ext_fyl2x(&ln2, &ef(r.3), CW),
        false,
        &mut leaves,
    );

    let mut bank: Vec<Node> = leaves
        .iter()
        .map(|(nm, v, dbl)| Node {
            v: v.clone(),
            dbl: *dbl,
    ...
    for s in 1..=big_s {
            ...
            for &ia in &li {
                for &jb in &lj {
                    ...
                    if all(&|e| d(e) > 0.0, &nb.v) {
                        try_push(
                            map2(&na.v, &nb.v, |y, x| ext_fyl2x(y, x, CW)),
                            false,
                            Prov::Bin("fyl2x", ia, jb),
                            &mut seen,
                            &mut fresh,
                            &mut hit,
                        );
                    ...
                    if all(&|e| d(e).abs() < 0.2928932, &nb.v) {
                        try_push(
                            map2(&na.v, &nb.v, |y, x| ext_fyl2xp1(y, x, CW)),
                            false,
                            Prov::Bin("fyl2xp1", ia, jb),
                            &mut seen,
                            &mut fresh,
                            &mut hit,
                        );
                    ...
                    try_push(
                        map2(&na.v, &nb.v, |x, k| ext_scale(x, k, CW)),
                        false,
                        Prov::Bin("fscale", ia, jb),
                        &mut seen,
                        &mut fresh,
                        &mut hit,
                    );
        ...
        for &ia in &by_size[(s - 1) as usize].clone() {
            ...
            if na.v.iter().all(|e| d(e).abs() <= 1.0) {
                try_push(
                    na.v.iter().map(|e| ext_f2xm1(e, CW)).collect(),
                    false,
                    Prov::Un("f2xm1", ia),
                    &mut seen,
                    &mut fresh,
            ...
            try_push(
                na.v.iter().map(|e| ext_rndint(e, CW)).collect(),
                false,
                Prov::Un("frndint", ia),
                &mut seen,
                &mut fresh,
        ...
        if bank.len() > CAP {
            println!("!! bank cap {CAP} exceeded at size {s} — envelope incomplete beyond here");
            break;
        }
    }

    if hit.is_some() {
        report(&bank, bank.len() - 1, m);
        return;
    }
    ...
    if std::env::var("SEARCH3").is_ok() {
        ...
        let mut logf = std::fs::OpenOptions::new()
            .create(true)
            .append(true)
            .open(format!("{dir}/optree_search3.log"))
            .unwrap();
        macro_rules! log {
        ...
        log!(
            "=== SEARCH3 start: rows={} size<=2 bank={} ===",
            m,
            bank.len()
        );
        ...
        let light: Vec<(u8, u32, u32, bool, f64)> = if std::path::Path::new(&ckpt).exists() {
            ...
            for i in 0..n {
                ...
                v.push((
                    raw[o],
                    u32::from_le_bytes(raw[o + 1..o + 5].try_into().unwrap()),
                    u32::from_le_bytes(raw[o + 5..o + 9].try_into().unwrap()),
                    raw[o + 9] != 0,
            ...
            let raw: Vec<(u128, u8, u32, u32, bool, f64)> = pairs
                .par_iter()
                .flat_map_iter(|&(a, b)| {
                    let na = &bankr[a as usize];
                    let nb = &bankr[b as usize];
                    let both = na.dbl && nb.dbl;
                    let valid: [(u8, bool); 12] = [
                        (0, true),
                        (1, true),
                        (2, true),
                        (3, true),
                        ...
                        (6, both),
                        (7, both),
                        (8, nb.v.iter().all(|e| d(e) > 0.0)),
                        (9, na.v.iter().all(|e| d(e) > 0.0)),
                        (10, true),
                        (11, true),
                    ];
                    let mut out = Vec::new();
            ...
            if let Some((op, a, b)) = *s3hit.lock().unwrap() {
                ...
                print!("A=");
                report(&bank, a as usize, m);
                print!("B=");
                report(&bank, b as usize, m);
                return;
            }
            ...
            for (h, op, a, b, dbl, r0) in raw {
                if full_hashes.contains(&h) {
                    continue;
                }
                if lseen.insert(h) {
                    v.push((op, a, b, dbl, r0));
                }
            }
            ...
            log!(
                "size-3 distinct = {} (checkpointed {} MB)",
                v.len(),
                buf.len() / 1_048_576
            );
            v
        ...
        let mut jnodes: Vec<(f64, JN)> = bank
            .iter()
            .enumerate()
            .map(|(i, n)| (d(&n.v[0]), JN::Full(i as u32)))
            .collect();
        for (j, l) in light.iter().enumerate() {
            jnodes.push((l.4, JN::Light(j as u32)));
        }
        ...
        let row_of = |n: JN, i: usize| -> Ext80 {
            match n {
                JN::Full(k) => bankr[k as usize].v[i],
                JN::Light(j) => {
                    let (op, a, b, ..) = lightr[j as usize];
                    eval3_row(bankr, op, a, b, i)
                }
            }
        };
        ...
        let verify = |an: JN, bn: JN, op: &str, rev: bool| -> bool {
            for i in 0..m {
                let (x, y) = if rev {
                    (row_of(bn, i), row_of(an, i))
                } else {
                    (row_of(an, i), row_of(bn, i))
                };
                let r = match op {
                    "add" => ext_add(&x, &y, CW),
                    "sub" => ext_sub(&x, &y, CW),
                ...
                if ext_to_f64(&r, RN53).to_bits() != emr[i] {
                    return false;
                }
            }
            true
        ...
        let win = |t: f64| {
            let u = 64.0 * 2f64.powi(-52) * t.abs().max(2f64.powi(-60));
            (t - u, t + u)
        };

        // --- SHARDED, RESUMABLE join: partial coverage is recorded, so a kill loses one shard ---
        let nsh: usize = std::env::var("SHARDS")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or(400);
        let start: usize = std::env::var("SHARD_START")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or(0);
        let ssz = (jnodes.len() + nsh - 1) / nsh;
        log!(
            "join over {} nodes in {} shards of {} (resume with SHARD_START=k)",
            jnodes.len(),
            nsh,
            ssz
        );
        ...
        for sh in start..nsh {
                ...
                for op in ["div", "mul", "add", "sub"] {
                    ...
                    for (ti, t) in ts.iter().enumerate() {
                        ...
                        while p < jk.len() && jk[p] <= hi {
                            let (_, an) = jn[p];
                            if verify(an, bn, op, op == "sub" && ti == 1) {
                                return Some((op, ti, p, bi));
                            }
                            p += 1;
                        }
                    }
            ...
            if let Some((op, ti, p, bi)) = found {
                log!(
                    ">>> SIZE<=7 HIT: root {} (rev={}) jnode_a={} jnode_b={}",
                    op,
                    ti == 1,
                    p,
                    bi
        ...
        log!(
            "EXT6: NO size<=7 arithmetic-rooted DAG over size<=3 subtrees reproduces em. ({:?})",
            t0.elapsed()
        );
        return;
    }
    ...
    let mut idx: Vec<(f64, u32)> = bank
        .iter()
        .enumerate()
        .map(|(i, n)| (d(&n.v[0]), i as u32))
        .collect();
    ...
    let win = |t: f64| {
        let u =
            (t.abs().to_bits() as f64) * 0.0 + 64.0 * 2f64.powi(-52) * t.abs().max(2f64.powi(-60));
        (t - u, t + u)
    };
    let bankref = &bank;
        ...
        for op in ["div", "mul", "add", "sub"] {
            ...
            for (ti, t) in targets.iter().enumerate() {
                ...
                while p < keys.len() && keys[p] <= hi {
                    ...
                    let (aa, bb) = if op == "sub" && ti == 1 {
                        (&nb.v, &na.v)
                    } else {
                        (&na.v, &nb.v)
                    };
                    // x87 spill root
                    if bits(&spill(&x87(aa, bb, op))) == em_bits {
                        return Some((op, ai, bi, ti, false));
                    }
                    // sse root (needs both double)
                    let both_dbl = if op == "sub" && ti == 1 {
                        nb.dbl && na.dbl
                    } else {
                        na.dbl && nb.dbl
                    };
                    if both_dbl && bits(&sse(aa, bb, op)) == em_bits {
                        return Some((op, ai, bi, ti, true));
                    }
                    p += 1;
                }
            }
    ...
    match found {
        Some((op, ai, bi, ti, is_sse)) => {
            println!(
                "\n>>> HIT via ROOT JOIN: {}{} ( #{} {} #{} )",
                if is_sse { "sse_" } else { "x87spill_" },
                op,
                ai,
                if op == "sub" && ti == 1 { "rev" } else { "" },
                bi
            );
            print!("  A = ");
            report(&bank, ai, m);
        ...
        None => {
            ...
            println!(
                "(bank size {} = all size<=2 subtrees; root join covers size<=5 div/mul/add/sub roots)",
                bank.len()
            );
        }
    }
        ...
        for form in ["mulC", "VdivC", "CdivV", "addC", "VsubC", "CsubV"] {
            ...
            if feasible && lo <= hi {
                ...
                for cand in [
                    ...
                    hi,
                    (lo + hi) * 0.5,
                    f64::from_bits(((lo.to_bits() as i128 + hi.to_bits() as i128) / 2) as u64),
                ] {
                    if !cand.is_finite() {
                        continue;
                    }
                    ...
                    if bits(&res) == em_bits {
                        println!(
                            "\n>>> FREE-CONSTANT HIT: {} with C = {:.20e} (0x{:016x})",
                            form,
                            cand,
                            cand.to_bits()
                        );
    ...
    if synth_hits == 0 {
        println!(
            "EXT1 free-constant synthesis: no single foreign constant closes em over any size<=2 subtree + one outer op."
        );
    }
    ...
    let masks: Vec<(u64, Vec<bool>, usize)> = bank
        .iter()
        .enumerate()
        .filter_map(|(i, nv)| {
            let bt = bits(&nv.v);
        ...
        .collect();
    println!(
        "\nEXT2 branch mining: {} bank vectors match em on >=120 rows",
        masks.len()
    );
    ...
    for a in 0..masks.len() {
        for b2 in a..masks.len() {
            ...
            if cov > best_cover {
                best_cover = cov;
                best_pair = (masks[a].0, masks[b2].0);
            }
        }
    }
    println!(
        "  best 2-tree OR-cover = {}/{} rows (trees #{} , #{})",
        best_cover, m, best_pair.0, best_pair.1
    );
    if best_cover == m {
        // check if the split is a clean |tau| threshold
        let (ia, ib) = (best_pair.0 as usize, best_pair.1 as usize);
        let ma: Vec<bool> = bits(&bank[ia].v)
            .iter()
            .zip(&em_bits)
            .map(|(a, b)| a == b)
            .collect();
        ...
        let clean = only_a.iter().cloned().fold(f64::MIN, f64::max)
            < only_b.iter().cloned().fold(f64::MAX, f64::min)
            || only_b.iter().cloned().fold(f64::MIN, f64::max)
                < only_a.iter().cloned().fold(f64::MAX, f64::min);
        println!(
            "  2-tree cover EXISTS; clean |tau|-threshold split: {} (branching hypothesis {})",
            clean,
            if clean {
                "PLAUSIBLE — investigate"
            } else {
                "unlikely (masks interleave in tau)"
            }
        );
    } else {
        println!(
            "  no 2-tree cover at size<=2 -> em is not a 2-branch composition of size<=2 subtrees."
        );
    }
    ...
    let win2 = |t: f64| {
        let u = 128.0 * 2f64.powi(-52) * t.abs().max(2f64.powi(-60));
        (t - u, t + u)
    };
    ...
    let decomp = |fixed: &[Ext80], num_is_fixed: bool, label: &str| -> bool {
        ...
        let f0 = d(&fixed[0]);
        let em0 = f64::from_bits(em_bits[0]);
        let s_target0 = if num_is_fixed {
            if em0 != 0.0 { f0 / em0 } else { return false }
        } else {
            em0 * f0
        };
        ...
        let hit = (0..bank.len()).into_par_iter().find_map_any(|bi| {
            ...
            for op in ["div", "mul", "add", "sub"] {
                let a0t: Vec<f64> = match op {
                    ...
                    "add" => vec![s_target0 - b0],
                    "sub" => vec![s_target0 + b0, b0 - s_target0],
                    _ => vec![],
                };
                for (ti, t) in a0t.iter().enumerate() {
                    ...
                    while p < keys.len() && keys[p] <= hi {
                        ...
                        let full = if num_is_fixed {
                            spill(&x87(fixed, &s_vec, "div"))
                        } else {
                            spill(&x87(&s_vec, fixed, "div"))
                        };
                        if bits(&full) == em_bits {
                            return Some((op, ai, bi, ti));
                        }
                        // also try SSE searched-side op when both double
                        if na.dbl && nb.dbl {
                            let s2 = sse(aa, bb, op);
                            let full2 = if num_is_fixed {
                                spill(&x87(fixed, &s2, "div"))
                            } else {
                                spill(&x87(&s2, fixed, "div"))
                            };
                            if bits(&full2) == em_bits {
                                return Some((op, ai, bi, ti));
                            }
                        }
                        p += 1;
        ...
        match hit {
            Some((op, ai, bi, _)) => {
                println!(
                    "\n>>> DECOMP HIT ({label}): searched side = {}({}, {})",
                    op, "#", "#"
                );
                print!("  A=");
                report(&bank, ai, m);
            ...
            None => {
                println!("EXT decomp ({label}): no size<=5 searched-side tree closes em.",);
                false
            }
        }
    };
    ...
    let _ = decomp(&num_sse, true, "fix numerator -> search denominator<=5")
        || decomp(
            &lnu_dv,
            false,
            "fix denominator=lnu_d -> search numerator<=5",
        );
    ...
    let apply_all = |a: &Node, b: &Node| -> bool {
        for op in ["add", "sub", "mul", "div"] {
            if check(&spill(&x87(&a.v, &b.v, op))) || check(&x87(&a.v, &b.v, op)) {
                return true;
            }
            if a.dbl && b.dbl && check(&sse(&a.v, &b.v, op)) {
                return true;
            }
        }
        if a.v.iter().all(|e| d(e) > 0.0) && check(&map2(&a.v, &b.v, |y, x| ext_fyl2x(y, x, CW))) {
            return true;
        }
        if b.v.iter().all(|e| d(e) > 0.0) && check(&map2(&b.v, &a.v, |y, x| ext_fyl2x(y, x, CW))) {
            return true;
        }
        if check(&map2(&a.v, &b.v, |x, k| ext_scale(x, k, CW)))
            || check(&map2(&b.v, &a.v, |x, k| ext_scale(x, k, CW)))
        {
            return true;
        }
        false
    ...
    let hit3c = !hit3a
        ...
        && s2.par_iter().any(|&ai| {
            let a = &bankr[ai as usize];
            (!a.dbl && check(&spill(&a.v)))
                || (!a.dbl && check(&spill_rz(&a.v)))
                || check(
                    &a.v.iter()
                        .map(|e| ext_sub(&ext_from_f64(0.0), e, CW))
                        .collect::<Vec<_>>(),
                )
                || (a.v.iter().all(|e| d(e).abs() <= 1.0)
                    && check(&a.v.iter().map(|e| ext_f2xm1(e, CW)).collect::<Vec<_>>()))
        });
    println!(
        "\nEXT5 streaming size-3 any-root pass: em reached = {}",
        hit3a || hit3b || hit3c
    );
...
fn report(bank: &[Node], i: usize, _m: usize) {
    fn s(bank: &[Node], i: usize) -> String {
        match &bank[i].prov {
            ...
            Prov::Bin(op, a, b) => {
                format!("{}({}, {})", op, s(bank, *a as usize), s(bank, *b as usize))
            }
        }
    }
