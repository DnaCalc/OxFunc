//! 4 LOW: lead+1 last-mul RN/RU vs 0.452 555. Not an identity.
...
fn main() {
    ...
    let graphs: [(&str, Box<dyn Fn(f64) -> f64>); 6] = [
        (
            "L1RN",
            Box::new(|z| {
                ext_to_f64(&ext_mul(&ef(z.next_up()), &one_w(z), CW), CW)
            }),
        ),
        (
            "L1RU",
        ...
        (
            "L1RD",
            Box::new(|z| {
                ext_to_f64(&ext_mul(&ef(z.next_up()), &one_w(z), CW_RD), CW_RD)
            }),
        ),
        (
            "L1RUst",
            Box::new(|z| {
                ext_to_f64(&ext_mul(&ef(z.next_up()), &one_w(z), CW), CW_RU)
            }),
        ),
        (
            "L1*upow",
    ...
    println!("4 LOW lead+1 last-mul RN/RU:");
    for &lz in &LOWS {
