//! Exhaustive 6-bit Cephes Horner stores DIRECT z>=4. Not an identity.
...
fn main() {
    ...
    let s59 = score(0x4605);
    println!("DIRECT z>=4 n={n} unmask={} 0x4605={s59}", score(0));
    println!("0x4605 ∪ extra bit:");
    for b in 0..17u32 {
        if 0x4605 & (1u32 << b) != 0 {
            continue;
        }
    ...
    println!("top 12 6-bit:");
    for (s, mask) in ranked.iter().take(12) {
        println!("  fused={s} mask={mask:#x}");
    }
    let best = ranked[0].0;
    let nbest = ranked.iter().filter(|t| t.0 == best).count();
    let nge59 = ranked.iter().filter(|t| t.0 >= 59).count();
    let ngt59 = ranked.iter().filter(|t| t.0 > 59).count();
    println!(
        "best 6-bit={best} mask={:#x} unique={} nbest={nbest} ge59={nge59} gt59={ngt59} n6={}",
        ranked[0].1,
        nbest == 1,
        ranked.len()
    );
    println!("all fused>59:");
    for (s, mask) in ranked.iter().filter(|t| t.0 > 59) {
        println!("  fused={s} mask={mask:#x}");
    }
    println!("all fused==59 6-bit:");
    for (s, mask) in ranked.iter().filter(|t| t.0 == 59) {
        println!("  fused={s} mask={mask:#x}");
    }
}
