...
fn ext_abs_local(x: &rx::Ext80, cw: u16) -> rx::Ext80 {
    rx::ext_abs(x, cw)
}
...
fn em_dbl_kahan(r: f64, n: f64) -> f64 {
    let tau = -(n * log1p_cr(r));
    expm1_kahan_internal(tau)
}
fn em_dbl_cr(r: f64, n: f64) -> f64 {
    let tau = -(n * log1p_cr(r));
    expm1_cr(tau)
}
fn em_dbl_f2xm1(r: f64, n: f64) -> f64 {
    let tau = -(n * log1p_cr(r));
    expm1_f2xm1(tau)
}
...
fn main() {
    ...
    let path = if std::path::Path::new(path).exists() {
        ...
        "../../work/w109/G6-solvers/answers-pmt-denselog1p.json"
    };
    ...
    for w in &ws.witnesses {
        ...
        if (a[0].to_bits() & 0x000f_ffff_ffff_ffff) != 0 {
            continue;
        }
    ...
    let cands: [(&str, fn(f64, f64) -> f64); 5] = [
        ("dbl_kahan", em_dbl_kahan),
        ("dbl_cr", em_dbl_cr),
        ("dbl_f2xm1", em_dbl_f2xm1),
        ("ext_all", em_ext_all),
        ("dbltau_extln_kahan", em_dbltau_extln_kahan),
    ];
