//! Ablate 0x5005 bits 0,2,12,14 DIRECT z>=4. Not an identity.
...
fn main() {
    ...
    let score = |mask: u32| -> (usize, usize, usize) {
        ...
        for r in &tail {
            ...
            let u = ulp_distance(mul(f::w_rn53(r.z), cephes_mask(r.z, 0)), t).unwrap_or(99) == 0;
            let h = ulp_distance(mul(f::w_rn53(r.z), cephes_mask(r.z, mask)), t).unwrap_or(99) == 0;
            if h {
                fused += 1;
            }
            if h && !u {
    ...
    let bits = [0u32, 2, 6, 12, 14];
    println!("DIRECT z>=4 n={n} 0x5005 bits and bit6:");
    let names = [
        ("unmask", 0u32),
        ("bit0", 1),
        ("bit2", 4),
        ...
        ("5005-bit0", 0x5005 ^ 1),
        ("5005-bit2", 0x5005 ^ 4),
        ("5005-bit12", 0x5005 ^ (1 << 12)),
        ("5005-bit14", 0x5005 ^ (1 << 14)),
        ("bit0+14", 1 | (1 << 14)),
        ("bit6+14", (1 << 6) | (1 << 14)),
        ("bit0+6", 0x41),
    ];
