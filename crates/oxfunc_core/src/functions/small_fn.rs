...
pub const SMALL_META: FunctionMeta = function_spec! {
...
fn collect_values(args: &[AggregatePreparedItem]) -> Result<Vec<f64>, SmallEvalError> {
...
fn coerce_k(prepared: &CalcValue) -> Result<usize, SmallEvalError> {
...
) -> Result<CalcValue, SmallEvalError> {
    let argc = args.len();
