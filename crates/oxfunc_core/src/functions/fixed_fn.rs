...
fn coerce_number_arg(arg: &PreparedArgValue, ctx: &LocaleFormatContext) -> Result<f64, FixedEvalError> {
    match arg {
        ...
        PreparedArgValue::Eval(EvalValue::Text(text)) => ctx
            ...
            .map_err(|_| FixedEvalError::Coercion(CoercionError::NonNumericText(text.to_string_lossy()))),
        PreparedArgValue::Eval(EvalValue::Error(code)) => Err(FixedEvalError::Coercion(CoercionError::WorksheetError(*code))),
        ...
        | PreparedArgValue::Eval(EvalValue::Lambda(_)) => Err(FixedEvalError::Coercion(CoercionError::UnsupportedValueKind("fixed_arg_kind"))),
    }
}

fn coerce_boolish_arg(arg: &PreparedArgValue, ctx: &LocaleFormatContext) -> Result<bool, FixedEvalError> {
    Ok(match arg {
...
pub fn eval_fixed_adapter_prepared(args: &[PreparedArgValue], ctx: &LocaleFormatContext) -> Result<EvalValue, FixedEvalError> {
    ...
    let text = ctx
        ...
        .map_err(|_| FixedEvalError::Coercion(CoercionError::UnsupportedValueKind("fixed_format")))?;
    Ok(EvalValue::Text(text))
}

pub fn eval_fixed_surface(args: &[CallArgValue], resolver: &impl ReferenceResolver, ctx: &LocaleFormatContext) -> Result<EvalValue, FixedEvalError> {
    run_values_only_prepared(args, resolver, |prepared| eval_fixed_adapter_prepared(prepared, ctx), FixedEvalError::Coercion)
}
...
mod tests {
    ...
    fn eval_fixed_current_host_seed_rows() {
        ...
        assert_eq!(
            eval_fixed_surface(&[CallArgValue::Eval(EvalValue::Number(1234.567)), CallArgValue::Eval(EvalValue::Number(2.0))], &NoResolver, &ctx),
            Ok(EvalValue::Text(ExcelText::from_utf16_code_units("1 234.57".encode_utf16().collect())))
        );
        assert_eq!(
            eval_fixed_surface(&[CallArgValue::Eval(EvalValue::Number(1234.567)), CallArgValue::Eval(EvalValue::Number(2.0)), CallArgValue::Eval(EvalValue::Logical(true))], &NoResolver, &ctx),
            Ok(EvalValue::Text(ExcelText::from_utf16_code_units("1234.57".encode_utf16().collect())))
        );
        assert_eq!(
            eval_fixed_surface(&[CallArgValue::Eval(EvalValue::Number(-1234.567)), CallArgValue::Eval(EvalValue::Number(2.0))], &NoResolver, &ctx),
            Ok(EvalValue::Text(ExcelText::from_utf16_code_units("-1 234.57".encode_utf16().collect())))
        ...
        assert_eq!(
            eval_fixed_surface(&[CallArgValue::Eval(EvalValue::Logical(true)), CallArgValue::Eval(EvalValue::Number(2.0))], &NoResolver, &ctx),
            Ok(EvalValue::Text(ExcelText::from_utf16_code_units("1.00".encode_utf16().collect())))
        );
        assert_eq!(
            eval_fixed_surface(&[text_arg("123"), CallArgValue::Eval(EvalValue::Number(2.0))], &NoResolver, &ctx),
            Ok(EvalValue::Text(ExcelText::from_utf16_code_units("123.00".encode_utf16().collect())))
        );
        assert!(matches!(
            eval_fixed_surface(&[text_arg("x"), CallArgValue::Eval(EvalValue::Number(2.0))], &NoResolver, &ctx),
            Err(FixedEvalError::Coercion(CoercionError::NonNumericText(_)))
        ));
        assert_eq!(
            eval_fixed_surface(&[CallArgValue::EmptyCell, CallArgValue::Eval(EvalValue::Number(2.0))], &NoResolver, &ctx),
            Ok(EvalValue::Text(ExcelText::from_utf16_code_units("0.00".encode_utf16().collect())))
        );
    }
}
