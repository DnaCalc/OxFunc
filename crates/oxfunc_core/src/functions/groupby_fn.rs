// GROUPBY scaffold — W038_BLOCKED: callable invocation requires LAMBDA/LET infrastructure.
//
// This scaffold implements META, error enum, arity validation, and the non-callable
// argument parsing layer. The callable 3rd-argument invocation path depends on W038
...
pub fn eval_groupby_surface(
    ...
    // The 3rd argument is the aggregation function (callable).
...
mod tests {
    ...
    #[test]
    fn groupby_meta_deterministic() {
        assert_eq!(GROUPBY_META.determinism, DeterminismClass::Deterministic);
        assert_eq!(GROUPBY_META.volatility, VolatilityClass::NonVolatile);
    }

    #[test]
    fn groupby_meta_values_only() {
        assert_eq!(
            GROUPBY_META.arg_preparation_profile,
