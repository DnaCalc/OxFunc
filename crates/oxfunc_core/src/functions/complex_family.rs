...
use crate::functions::adapters::{
    expand_arg_values_only, prepare_arg_values_only, prepare_args_values_only,
};
use crate::resolver::ReferenceSystemProvider;
use crate::value::{CalcValue, CoreValue};
use crate::value::{ExcelText, WorksheetErrorCode};

const TEXT_META_BASE: FunctionMeta = function_spec! {
...
fn text_from_string(s: String) -> CalcValue {
...
fn prepared_scalar_to_complex(arg: &CalcValue) -> Result<ParsedComplex, ComplexFamilyEvalError> {
    match arg.core() {
        CoreValue::Number(n) => Ok(ParsedComplex::new(*n, 0.0, None)),
        CoreValue::Text(t) => parse_complex_text(&t.to_string_lossy())
            .ok_or(ComplexFamilyEvalError::Domain(WorksheetErrorCode::Num)),
        CoreValue::Logical(_) => Err(ComplexFamilyEvalError::Domain(WorksheetErrorCode::Value)),
        CoreValue::Error(code) => Err(ComplexFamilyEvalError::Coercion(
        ...
        CoreValue::Missing | CoreValue::Empty => Ok(ParsedComplex::new(0.0, 0.0, None)),
    }
}
fn prepared_scalar_to_real_for_complex(arg: &CalcValue) -> Result<f64, ComplexFamilyEvalError> {
    match arg.core() {
        CoreValue::Number(n) => Ok(*n),
        CoreValue::Text(t) => parse_real_number(&t.to_string_lossy())
...
fn prepared_scalar_to_suffix(arg: Option<&CalcValue>) -> Result<char, ComplexFamilyEvalError> {
    let Some(arg) = arg else {
        return Ok('i');
    };
    match arg.core() {
        CoreValue::Missing | CoreValue::Empty => Ok('i'),
        CoreValue::Text(t) => match t.to_string_lossy().trim() {
            "" | "i" => Ok('i'),
            "j" => Ok('j'),
...
) -> Result<Vec<CalcValue>, ComplexFamilyEvalError> {
...
) -> Result<CalcValue, ComplexFamilyEvalError> {
...
fn unary_number(
    ...
    let prepared =
        prepare_arg_values_only(&args[0], resolver).map_err(ComplexFamilyEvalError::Coercion)?;
    Ok(CalcValue::number(normalize_zero(kernel(
...
mod tests {
        ...
        CalcValue::array(
            crate::value::CalcArray::from_rows(vec![
                values
                    .iter()
                    .map(|value| {
                        crate::value::CalcValue::text(ExcelText::from_interop_assignment(value))
                    })
                    .collect(),
            ])
            .unwrap(),
    ...
    fn parsed_result(value: CalcValue) -> ParsedComplex {
        parse_complex_text(&text_result(value)).unwrap()
    }
    fn assert_close(value: CalcValue, re: f64, im: f64, suffix: char) {
