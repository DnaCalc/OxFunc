...
) -> Result<CalcValue, BaseEvalError> {
...
mod tests {
    ...
    fn eval_base_spills_array_arguments() {
        ...
        assert_eq!(
            ...
            Ok(CalcValue::array(
                CalcArray::from_rows(vec![vec![
                    CalcValue::text(ExcelText::from_interop_assignment("F")),
                    CalcValue::text(ExcelText::from_interop_assignment("10")),
                ]])
                .unwrap()
            ))
        );
