...
| G6-02 — ACCRINT | W109 (2026-07-20) **REGRADED NUM-L — the "1 ULP" row was hiding a calc_method SEMANTIC divergence.** b39 lattice (25,410 live rows, 4 bonds × bases 0-4 × 3 rates × both calc_methods): Excel `calc_method=FALSE` is the **LEGACY FLAT computation** — one fraction `days(issue→settlement, by basis)/canonical`, NO period structure (e.g. W-bond settle 43833 b2: `24.9375·(268/180)`); `TRUE` = the period-aware walk. The two are bit-identical on 7,752/12,705 pairs (pre-first + 30/360-aligned rows) and split MATERIALLY post-first on actual bases (4,227 pairs). OxFunc's FALSE (accrue from one period before first) matches NEITHER: production `17,967/25,410`, ~6,100 material FALSE rows. Remaining sub-classes: ±1-2 ULP staging both paths (~1,100 rows incl. the original witness); a material 219-row TRUE class on the quarterly act/act bond (post-first); several 3-row settlement-boundary groups. Identification in progress (agent-W); landing sequenced behind G6-03d. | NUM-L | M2 | W109_G6_BOND_SCHEDULE_IDENTIFICATION_20260720.md / b39 / b39_resid.json |
