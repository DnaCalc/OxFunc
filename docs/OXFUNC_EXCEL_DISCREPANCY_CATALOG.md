...
| ODDFPRICE, ODDFYIELD | structural `#NUM!` signed off (2026-06-20): the long (multi-quasi-coupon-period) odd first coupon now computes — the closed form generalizes with `DFC/E > 1`, so the rejecting guard was removed; the period-length basis normalization was also corrected to the bit-exact PRICE (`dc`) convention. Residual: 30/360 `~1` ULP (operation order) and actual-day bases (esp. `act/act`) not yet bit-exact | NUM-L | M2 | BUG-FUNC-032 |
