...
| G3-02 — GAMMA (+ GAMMALN substrate) | W109 re-scoping (2026-07-11): the row was under-scoped — a fresh 156-row live sweep shows the POSITIVE side is `0/79` exact (up to `1370` ULP at large x) and negatives reach `810k` ULP; the recon corpus had only probed two negative points. Ruled out: current Lanczos log-domain kernel; `x87-EXP(published GAMMALN)` composition (6/79, errors grow with |lnΓ| — Excel uses a HIGHER-precision internal lgamma than published GAMMALN). Same internal extended-lgamma substrate implicated for COMBIN (G4-04 findings). **W109 algorithm identification (2026-07-11): the internal lgamma IS Cephes `lgam`** — on the 93-row published-GAMMALN corpus the Stirling path (`x>=13`) in plain double is `82/83` exact (max 1 ULP) and the small path (`x<13`) in extended staging holds every row to `<=2` ULP incl. the 2498-ULP cancellation zone near `x=1`; NR-Lanczos is ruled out (45k ULP). Remaining work: statement-level store-mask fit of the small path (+1 Stirling row), then compose GAMMA (exp over the unstored extended lgamma) and COMBIN over the winner; reflection staging last (`excel_sin` available). | NUM-L | M2 | BUG-FUNC-027 C1 / W109 findings / smart-fuzzer/work/w109/G3-02-gamma |
