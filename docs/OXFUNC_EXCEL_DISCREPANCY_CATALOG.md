...
Last reconciled: `2026-08-18` (inverse-problem identity: live Excel 16.0
build 20228 publishes `CHIDIST(x,1)` as `ERFC.PRECISE(SQRT(x/2))` and
`GAMMA.DIST(x,0.5,2,TRUE)` as `ERF.PRECISE(SQRT(x/2))` on 154/154 nonnegative
rows. Rival `SQRT(x)/SQRT(2)` staging is refuted. The df=1 / Gamma(0.5,2) CDF
route now dispatches through the published ERF/ERFC kernels; G3-01, G3-05, and
G4-04 remain open for the ERF body and non-df=1 GRATIO tail.)
