...
| Function(s) | Discrepancy | Sev | Mat | Evidence |
|-------------|-------------|-----|-----|----------|
| G5-01 — MINVERSE | W109 (2026-07-12): algorithm = **Doolittle LU + partial pivot + division multiplier + sequential elimination + per-column unit solves, plain double** — now VALIDATED bit-exact on a fresh held-out **448/448** 4x4 element harvest (28 matrices, live build 20131), plus `150/159` on the 3x3 corpus. A Python float64 emulation of this exact staging reproduces OxFunc bit-for-bit (same `150/159`, same 9 residuals) — confirming OxFunc already implements the identified algorithm. Newly ruled out this round: adjugate/cofactor `adj(A)/det` (`51/155` 3x3, `85/448` 4x4) and Gauss-Jordan full elimination (`80/159`, `102/448`); earlier Crout/FMA/reciprocal-multiplier also out. The `9` residual `+1`-ULP misses are all on ill-conditioned 3x3 (small-determinant integer matrices e.g. [[2,-1,0],[-1,2,-1],[0,-1,2]] det 4, and [[1,2,3],[4,5,6],[7,8,10]] det -3) — a deep within-Doolittle pivot-tie/near-cancellation accumulation detail, NOT an algorithm-choice question. Final axis; needs targeted ill-conditioned probing. | NUM-S | M2 | W109 4x4 harvest / G5-01-answers-m4b.json |
