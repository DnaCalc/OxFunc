...
Last reconciled: `2026-09-25` (W111 G8-14: ROUND/ROUNDUP/ROUNDDOWN decimal rounding landed; only the max-double overflow edge stays open in G8-14. Open count 24 -> 24.)
Previous reconcile: `2026-09-25` (W111 ROUND probe: new row G8-14, open count 23 -> 24.)
...
Previous reconcile: `2026-09-25` (W111 G8-05: YEARFRAC basis 1 now follows Excel's actual/actual rule (not the ISDA split; year total on the real calendar) and negative serials give #NUM!, 3006/3006, row retired. The four functions sharing the day-count fraction were judged live for the first time: DISC, INTRATE, RECEIVED, PRICEDISC diverge on operation order, new row G8-13. Open count 26 -> 26.)
Previous reconcile: `2026-09-25` (W111 G8-04: WORKDAY/WORKDAY.INTL floor the days argument -> 2400/2400 and 4800/4800 incl. fractional days, row retired, open count 27 -> 26.)
Previous reconcile: `2026-09-25` (W111 G8 fixes: DOLLARDE/DOLLARFR fraction scale 10^ceil(log10 d) and f64 denominator -> 2412/2412 each, G8-01 retired; TBILLPRICE non-positive-price #NUM! and DATE-rollover one-year limit -> 2400/2400, G8-03 retired; TBILLEQ long-bill branch -> 2399/2400. Open count 29 -> 27.)
...
| G3-07 — GAUSS | **W109 (2026-08-21): G-F3 wrapper landed** — `NORMSDIST(x<0)=RN53(0.5*Q)`, `NORMSDIST(x≥0)=RN53(1-0.5*Q)`, `z=|x|*RN(1/√2)`, `Q=ERFC.PRECISE(z)`; GAUSS is that graph minus 0.5 except inclusive `abs(x)≤1e-15` stored-z G-A400 tiny-direct. `GAUSS(1)` moved from 2 ULP (`0.5*libm::erf`) to 0 ULP. Remaining GAUSS/NORMSDIST misses inherit the open ERFC body (`NORMSDIST(-1)` / `GAUSS(-1)` 1 ULP at the `CHIDIST(1,1)` z). PHI remains resolved out of this row (W109 2026-07-11: `RN53(RN64(x·x))` -> x87 EXP -> `RN53(RN64(e·RN(1/sqrt(2π))))` with a live-pinned subnormal publication flush; `764/764` answered rows). | NUM-S | M1 | recon G3-07 / `W109_NORMSDIST_GF3_WRAPPER_20260821.md` / `W109_ERF_SWARM_RESULTS_20260821.md` |
...
| G8-13 — DISC, INTRATE, RECEIVED, PRICEDISC | **2026-09-25, first live judging:** DISC ~35% of rows miss (max 10 ULP), INTRATE ~35% (max 2), PRICEDISC ~33% (max 111), RECEIVED ~7% (max 53); flat across bases 0-4, so the shared day-count fraction is right and the misses are each function's operation order. The July map had all four as bit_exact_observed. Fix bead `oxf-mwue.26`. | NUM | M1 tested | Live Excel 16.0 build 20430 CV2, two fresh corpora (seeds 20260924, 20260925), `docs/function-lane/evidence/w111-g8-yearfrac-20260925/misses.json`. |
| G8-08 — COUPDAYS, COUPDAYSNC, COUPNUM | **2026-09-25:** coupon dates are now computed from maturity (a 28th-30th maturity day is kept, only a month-end maturity gives month-end coupons): COUPPCD, COUPNCD, COUPDAYBS 4084/4084. Residuals: (a) COUPDAYS basis 1 (50/4084): Excel's period length is neither NCD-PCD nor PCD+period with DATE rollover (588/650) or with clamping; e.g. PCD 2024-08-31 quarterly -> Excel 92 (NCD-PCD 91), PCD 1997-10-31 semiannual -> Excel 181 (= NCD-PCD); (b) COUPDAYSNC basis 0 (10/4084): COUPDAYS-COUPDAYBS matches 628/637 basis-0 rows, 632/637 if COUPDAYBS counts from the unclamped (year, month, anchor day) triple with settle==PCD -> 0; misses have a February-end PCD or NCD; (c) settlement serial 0: COUPNUM #NUM! in Excel (OxFunc 3), COUPDAYSNC 33 in Excel (OxFunc 180). | GRS | M1 tested | Live Excel 16.0 build 20430 CV2, two fresh corpora + the July b37 battery, `docs/function-lane/evidence/w111-g8-coup-20260925/misses.json`. |
...
| G8-14 — ROUND, ROUNDUP, ROUNDDOWN (FIXED, DOLLAR to re-judge) | **2026-09-25:** decimal rounding landed (15 significant digits with per-function tie rule, then decimal rounding; ROUNDUP = ROUNDDOWN + one unit in binary): ROUND 4019/4035, ROUNDUP 3808/3824, ROUNDDOWN 3810/3824, MROUND 2000/2000 on two fresh corpora plus half-way and tie probes. Residual: only the input 1.7976931348623157e308, whose 15-digit value overflows (Excel returns a non-finite number for ROUND/ROUNDDOWN and #NUM! for ROUNDUP; OxFunc returns +inf). FIXED and DOLLAR use the same rounding and need re-judging in the locale lane (the judge has no host format engine). | STR | M1 tested | Live Excel 16.0 build 20430 CV2, `docs/function-lane/evidence/w111-g8-round-20260925/misses.json`. |
