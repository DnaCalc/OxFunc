...
| G3-05 — CHISQ.TEST, CHITEST | **W109 sweep (2026-07-12): decomposition unblock proven** — `CHISQ.TEST(o,e) == CHIDIST(S, df)` BIT-EXACTLY for a specific stored double S (tail cancels in the comparison), so the internal statistic is directly measurable without the gamma substrate: S sits 1 ULP above plain row-major `Σ(o-e)²/e` on the first witness. Statistic staging race is unblocked; the CHIDIST tail half remains on the G3-01 substrate. Same method applies to F.TEST via FDIST. | NUM-L | M1 | recon G3-05 / W109 sweep / G3-05-answers-*.json |
...
| Function(s) | Discrepancy | Sev | Mat | Evidence |
|-------------|-------------|-----|-----|----------|
| G4-02 — ATANH | **W109 sweep (2026-07-12): decisively scoped.** Fresh 107-row grid: for `\|x\| < ~0.5` Excel's error vs true EQUIOSCILLATES (`+6.3..-3.4` ULP, damping toward 0.5) — a **custom Microsoft odd rational** (same custom-core class as GAMMALN's `(0,11)`); above ~0.5 a ~CR log form; tiny `x` passthrough. Ruled out bit-exactly: UCRT atanh (77/107), Cephes atanh P/Q (78/107), fdlibm log1p form, x87 fyl2xp1 pairs (ext + stored), x87 fFLN ratio forms. Closure = coefficient recovery (Phase-5b machinery, shared with GAMMALN/G3-02). | NUM-S | M2 | BUG-FUNC-027 C4 / W109 sweep / check_atanh.rs |
...
| Function(s) | Discrepancy | Sev | Mat | Evidence |
|-------------|-------------|-----|-----|----------|
| G5-01 — MINVERSE | **W109 sweep (2026-07-12): UNBLOCKED and nearly identified.** Element-level oracle probing landed via the existing `INDEX(...)` wrapper + `result_index` pass-through (driver patched). 51-element race over 2x2/3x3 (integer, fractional, near-singular, cancellation-heavy): adjugate formula RULED OUT even for 2x2; Gauss-Jordan ruled out; **LU decomposition (Doolittle, partial pivot, division form) + per-column unit solves scores `47/51`**, residual 4 rows all `+1` ULP in 3x3 factorization staging (Crout vs Doolittle / x87 detail next). | NUM-S | M2 | BUG-FUNC-025 / W109 sweep / G5-01-answers-minverse.json |
