...
| ACOTH | catastrophic large-`\|x\|` band fixed (2026-06-21): odd-symmetric `0.5*ln1p(2/(\|x\|-1))` replaces the direct `0.5*ln((x+1)/(x-1))` ratio (`~1.2E14` ULP → bit-exact across the probed range *except* `ACOTH(5)`/`ACOTH(10)`; ATANH shared the same odd-symmetry defect, now bit-exact, row removed). **Open residual (tracked, not accepted):** `ACOTH(5)` is 1 ULP — *6* double-precision forms all round 1 ULP below Excel (so Excel's `ln` is its own/extended-precision impl, not reachable in IEEE double here); `ACOTH(10)` is 1 ULP under ln1p but **bit-exact under `atanh(1/x)`** (which regresses `ACOTH(1.001)` by 39 ULP), so no single double-precision form matches every point. Full closure needs Excel's exact `ln` substrate. | NUM-S | M1 | BUG-FUNC-027 C5 |
