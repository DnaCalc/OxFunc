...
| Function(s) | Discrepancy | Sev | Mat | Evidence |
|-------------|-------------|-----|-----|----------|
| G5-01 — MINVERSE | **W109 sweep (2026-07-12): UNBLOCKED and nearly identified.** Element-level oracle probing landed via the existing `INDEX(...)` wrapper + `result_index` pass-through (driver patched). 51-element race over 2x2/3x3 (integer, fractional, near-singular, cancellation-heavy): adjugate formula RULED OUT even for 2x2; Gauss-Jordan ruled out; **Doolittle LU + partial pivot + division-form multiplier + sequential subtract-then-divide column solves = `150/159`** on a combined corpus (51 + 108 fresh 3x3). Crout DECISIVELY ruled out (`36/159`), FMA inner-update ruled out (`111`), reciprocal-multiply ruled out (`130`). The 9 residuals are all `<=1` ULP, concentrated on ill-conditioned matrices — a factorization-inner-loop accumulation-order detail (the last staging axis). | NUM-S | M2 | BUG-FUNC-025 / W109 sweep / G5-01-answers-minverse.json |
