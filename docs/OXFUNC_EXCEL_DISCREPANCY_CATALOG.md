...
Last reconciled: `2026-08-09` (the COMBIN sublane of mixed G4-04 is
...
“not black-box-closable”, “needs provenance”, and “boundary proven” retained in
the historical row narrative below are retracted by
...
lane. G6-01 is therefore an actionable open discrepancy, not a stopping-rule
boundary.

| Function(s) | Discrepancy | Sev | Mat | Evidence |
|-------------|-------------|-----|-----|----------|
| G6-01 — PMT, PPMT (IPMT, CUMPRINC/CUMIPMT adjacent) | Annuity publication exactness drift. W108 Phase E supersedes the earlier forward-form conclusion: Excel uses the discount arrangement already present in OxFunc, `em=expm1(-n*log1p(r)); v=1+em; pmt=(pv+fv*v)*r/em`. The best tested historical Kahan/x87 helper candidate scores `2285/4040` exact (`56.6%`) and `92.9%` within `1` ULP on the adversarial PMT corpus; the `553`-row `n=1` isolation lane localizes its remaining error to the final rounding of `em`. The fresh mortgage PMT control is exact while first-period PPMT is `1` ULP, sharpening the adjacent recurrence/publication lane. W109 replay (2026-07-11): the full `5,154`-row Phase-E live corpus through current OxFunc scores `3117` exact, `1318` at `1` ULP, `542` at `2-16`, `177` above — PMT lanes are dominated by `<=4` ULP publication drift while IPMT/PPMT/CUM schedule rows reach catastrophic drift, including sign-of-zero/branch rows (`IPMT-139` publishes `-0` vs a finite Excel value; `ISO-EM-T0-109/122` publish tiny nonzero vs Excel exact `0`). Rollup: `smart-fuzzer/runs/w109-phase-e-replay/rollup.json`. **W109 (2026-07-21) metamorphic-sibling advance:** FV var0 `149/149` and PV var0 `48/48` are bit-exact as the NAIVE FORWARD binexp kernel (plain SSE2 double: `P=binexp(1+r,n)`, `q=(P-1)/r`), but harvesting Excel's OWN `P` and `tf·q` from those closed FV siblings and feeding them into `-(pv·P+fv)/(tf·q)` is `0/109` on small rate — **proving PMT uses a DIFFERENT, cancellation-stable DISCOUNT algorithm than FV/PV** (they do not share the annuity factor). Residual localized by elimination to Excel's exact annuity `log1p`/`expm1` primitive: held-out ceilings are `482/875` (SSE2 body) and `460/875` (whole-body x87-DR, store-mask INVARIANT) — so form (discount, confirmed), body precision, and spill-staging are NOT the gap; the ±1-3 ULP sawtooth sign-flips with rate and matches none of {portable-CR, internal Kahan-F2XM1, raw base-2 hardware} `expm1`. Next: em-interval isolation on fv=0/ty=0 + micro-stage search (Lane D of the G6 schedule note). **W109 (2026-07-23) — "non-CR log1p" RETRACTED; residual is an expm1 double-rounding op-graph wall:** the earlier "matches none of ... expm1" and the presumed "non-CR log1p imprecision" are superseded. Excel's PMT `log1p` is CORRECTLY ROUNDED — live `LN(1+r)` at 148 exactly-representable `1+r` points is `0` non-CR, and live `EXP(tau)` == the x87 `excel_exp` emulation `234/234`. The residual localizes to the **`expm1` `|tau|<1` branch**: on a model-free em oracle (`r=2⁻ᵏ` ·r-exact trick × `n∈{1..64}`, `answers-pmt-po2n`, 258 pinned `(r,n)`), the all-double Kahan `(u−1)·t/ln(u)` ceilings at `163/234` (`70%`) while the `|tau|≥1` `u−1` branch is `100%` (`3840/3840`). With `tau,u,ln u` all proven-identical doubles to Excel's, NO double-op sequence closes it: 12 op-graphs raced (associations, additive `b+b(t−lnu)/lnu`, `t·b/lnu`, denominators `fyl2x`/`fyl2xp1`/`log1pCR`) cap at 163; extended + base-2 `F2XM1` worse. The "expm1 SOLVED 87/87" was an overfit (didn't stress `|tau|<1`). Large-`|fv|` misses (fvsweep `0/1024`) are the SAME wall amplified (`v=1+em` cancels for large `n`; `v=exp` doesn't save it). Discount `(num/em)/tf·r` confirmed as the fv=0 combine (combsweep `2304/2304`); forward-exp `−(pv·P+fv)/((P−1)/r)/tf` worse on every corpus. **W109 (2026-07-23, 12-agent workflow): PMT em is a DISTINCT financial-body routine, NOT Excel's standard expm1.** `EXPON.DIST(x,1,TRUE)` reads Excel's expm1 directly and it == all-double Kahan `232/234`, but PMT's em matches Kahan only `165/234` — at identical exact tau they DIFFER, so PMT does not call the standard expm1. The residual is **REAL not a po2-sampling artifact** (generic rates `r=m·2⁻ᵏ`: Kahan `61/90`, same **toward-zero bias**, Excel underestimates `|expm1|`), and `delta(tau)` is a ±1-2 ULP spread (last-bit op-graph, not a fittable coefficient error). Exhaustively refuted: extended-x87 (<163), polynomial (fdlibm 129/Cephes 84/CR 133), directed rounding (112), binexp. Firm `163/234` ceiling → a hand-coded inline expm1 (toward-zero-biased) = genuine ≤1 ULP Excel imprecision, op-graph outside every tested family. Provenance: LibreOffice/Gnumeric confirm the expression but +tau forward + library expm1 (refuted); Excel matches NO open source. **Inheritance: PMT → {IPMT,PPMT} → {CUMIPMT,CUMPRINC}**; naive `IPMT==RN(FV(per−1)·r)` refuted live (0/9), needs own recurrence ID; `fvsweep` is a DEGENERATE corpus (constant across 256 pv). Row STAYS OPEN (reclassified: bespoke non-CR inline expm1, toward-zero-biased ±1 ULP op-graph wall). **W109 (2026-07-23) — LANDED a major fix; log1p CONFIRMED CR (not the bug):** a multi-agent workflow's confound-free `|tau|≥1` oracle (where `em=u−1` is exact, so only log1p matters) proved Excel's PMT log1p IS correctly-rounded (= x87 FYL2XP1, bit-identical to CR on `0/1350` rows) — my interim "faithful-non-CR log1p" / "extended argument" claims are BOTH retracted (the collision "variation" was the expm1's own ±1 ULP rounding noise; std ln_1p's +30 heldout was expm1-wall compensation, `−2` in the clean region). **The real bug was the COMBINE + SUBSTRATE:** production `pmt()` used a refuted reciprocal-multiply combine + portable `exp/expm1`; corrected to the pinned quotient-first SSE2 combine `RN(RN(RN(num/em)/tf)·r)` + x87 `excel_exp`/`excel_expm1_internal` (log1p kept CR). **Measured (live-Excel): heldout 46%→56%, combsweep 62%→100%, po2 61%→87%, genrate 47%→73%, fvty 36%→43%** — LANDED commit `1eb9011`, all 1513 lib tests pass. Also LANDED (`5082589`): IPMT epsilon-band bug (tiny/negative rates were collapsed to 0.0) → exact `r==0`. Remaining residual = the expm1 `|tau|<1` non-CR Kahan double-rounding (`≤165/234` ceiling; toward-zero ~0.5 ULP, denominator-`lnu`-attributable, minimax-error fingerprint outside the `{tau,u,lnu}` family — must be fit as `g(tau)`). IPMT/PPMT/CUMIPMT/CUMPRINC inherit it. Row STAYS OPEN (residual = the expm1 wall only). **W109 (2026-07-24) — deep re-attack (Fable + 4-lane workflow), wall EXHAUSTIVELY characterized + one CORRECTION:** the "+2 via RN64 numerator" was unreproducible; the real `163→165` is the **x87 SPILL-LOOP** discipline (each Kahan op = `fl53(fl64(·))`, a genuine per-op double-rounding, same as the proven XNPV body — register-resident PC64 scores 145, *worse*). Confirmed `u,tau,ln u` in the em oracle are Excel's EXACT live captures (`answers-em-exp/ln.json`), so the 69 residual misses use Excel's OWN intermediates yet no op-graph reproduces them. **Triple-confirmed verdict** (manual racing + Fable + workflow poly/library/structure lanes): a GENUINE faithful-rounding boundary, ceiling `165/234`, NOT black-box-closable. Ceiling proof: CR `expm1` matches only `128/234` — Excel's em is NOT a smooth-fn approximation (every polynomial bounded by 128: fdlibm 129, Cephes 84, Boost 103); it is the compensated Kahan riding on the x87 exp's own rounding, which is why `165` beats CR. Fable's D-histogram: the exact real Kahan quotient rounds correctly to OUR value (66/69, ~0.5 ULP from the midpoint), Excel returns the neighbor — an upstream sub-ULP difference not exposed by any worksheet capture. Structure-lane separating predicate: a row misses IFF `tau=−n·log1p(r)` is EXACTLY representable (68/71 misses). REFUTED afresh on real hardware: binexp pow (96), extended-arg (156), fFEXPM1 `−w·m` assemble (115), w-numerator (7), **real-`ext_f2xm1` F2XM1-direct (133** = un-rounded `u−1` too-accurate path**)**, classic positive-power PMT (42–51%), x87-spill COMBINE (no change → combine genuinely SSE2, pinning valid), all Kahan groupings/precision schedules. The other corpora walls are the same class: on `|tau|≥1` (em exact) NO combine arrangement beats landed; the general-rate deficit is NOT log1p (x87 FYL2XP1 == portable CR) but the composed x87 chain's ~0.502-ULP faithful rounding amplified by `fv`. Closing the last bit needs PROVENANCE (public clean-room source of the legacy annuity `(1+r)^n` routine), not more op-graph racing. Optional `+2` spill-Kahan landing is marginal (po2n +0.4%) and touches the shared `excel_expm1_internal` (EXPON.DIST 99.96% SSE2) — hold unless made PMT-local. Story note `docs/notes/PMT_EXPM1_IDENTIFICATION_STORY.md`; 16 new racers under `calc_graph_racer/src/bin`. **W109 (2026-07-24 cont.) — inverse interval solve + "different-ln-implementation" class, wall PROVEN from a 3rd angle:** Fable-designed inverse solve (back-out the denominator/numerator each pinned row must have used, zero oracle calls) + a fresh SLP campaign (8 racers). Every mechanism refuted on real hardware: extended-denominator linkage (123), single-site chop (112–120), fully-extended-from-`(r,n)` (133), mixed-provenance (`ln(u)` is catastrophically u-low-bit sensitive at `u≈1` → up to ±131072 ULP → `u` MUST spill before log, locking `u_dbl`/`lnu_dbl`), binexp discount factor (refuted), and the "different ln" class: constant-chain `log10·LN10` (right direction, overshoots — 143 rows shifted not 57), fdlibm `log1p`/`log` & Cody-Waite ALOG (all faithful≈CR, symmetric ±1 scatter — a faithful minimax log CANNOT produce the required one-signed 57-row shift), additive-correction split (145), double-divides (120–129). Inverse solve: 57/71 misses toward-zero wanting denominator `+1` ULP uniformly, 14 away, 2 `|off|>1` (the last two formally refute the entire faithful-rounding class). **Stopping rule invoked (Fable's):** every mechanism class expressible via real hardware curves or published period implementations is crossed; the remainder is a bespoke MS-internal approximation unrecoverable without disassembly (prohibited); fitting a free faithful curve to 71 flips is interpolation not identification. Boundary now PROVEN, not presumed. Row STAYS OPEN (characterized boundary). | NUM-L→NUM-S | M2 | BUG-FUNC-015 / KED-FIN-001 / W108 / recon G6-01 / W109 replay / W109_G6...20260720.md Lane D / PMT_EXPM1_IDENTIFICATION_STORY.md |
...
| G6-03d — PRICE (Actual/360 & Actual/365) | W109 (2026-07-20) **IDENTIFIED + LANDED + HELD-OUT GATED.** Root cause: Excel's PRICE derives the settlement discount fraction with the UNIVERSAL rule **`dsc = E − A`** — it never counts days settlement→next-coupon directly. On bases 0/1/4 the derivation coincides with the direct span (except settlement-on-31st 30/360 rows, which `E − A` ALSO fixes); on 2/3, `E` is the fiction (`360/f`/`365/f`) while `A` is real days, so they split by whole cents (witness: off `= (180−81)/180 = 0.55` → `114.588731` = Excel's `114.5887`). **Excel's own COUPDAYSNC publishes ACTUAL days at bases 2/3 on the same args (458/458 rows each) — PRICE's internal DSC deliberately diverges from the published COUP\* functions.** Second lever: the fractional discount pow is the x87 CRT chain `exp(RN53(RN64(y·ln x)))` = `excel_pow_chain` — the SAME pow identified in the G3 lane-1 distribution substrate (cross-lane confirmation); body is plain single-rounded double, coupons-ascending + separate redemption (x87-DR body / extended-accumulator / 8 orders refuted, `agentV_results.md`). Landed in `pcomp_disc` (universal) + `excel_bond_pow` fractional branch; pins `price_dsc_e_minus_a_and_x87_pow_chain_pins`; 1510 green, zero pin movement. Scores: b37 ident `7458/7472` (b0/b1/b4 48/48, b2 3656/3664, b3 3658/3664); **fresh b38 held-out `945/945` ALL bases**. OPEN residual: 14 extreme-yield rows (±1 ULP; Excel's published value is up to `+3` ULP above the correctly-rounded true sum — an accumulation op-graph fact), banked `agentV_residual14.json` with next-probe design (adjacent-double yld bracket + truncated-ladder partials; battery b41). Blast radius: DURATION now inherits `E−A` dirty on 2/3 via shared `pcomp` (its own residual = G6-03c, still open); YIELD pins byte-stable. | NUM-S | M3 | W109_G6_BOND_SCHEDULE_IDENTIFICATION_20260720.md / agentV_results.md / b37+b38 |
| G6-03c — DURATION, MDURATION | W109 (2026-07-20) **IDENTIFIED + LANDED + GATED (agent-X).** Identity: weighting exponent shares PRICE's schedule (`off = (E−A)/E`, the docs-era direct settlement→next span was the bases-2/3 material bug); discount = `excel_bond_pow` (binexp on-coupon / x87 `excel_pow_chain` off-coupon — the PRICE substrate); Macaulay body = plain-f64 SSE2, redemption a SEPARATE term, weight grouped `(diff·cash)/disc` left-assoc, publication `num/den/f` (own-sum denominator, not pcomp's dirty). Accrued span A = **CoupDaysBS `diff360_us(prev,settle,false)` (ModifyStartDate order), NOT plain `us_30_360`** — they diverge only on 31st/month-end settlements by adjustment ORDER (Feb-end prev + 31st settle: 31 vs 30), a 1-integer A error worth ~2.5e13 ULP; caught by the b45 pre-registered gate, fixed, re-gated. Both catalog witnesses exact. Scores (coordinator-verified): b44 `6217/6360`, b45 `456+84≤3`, **fresh month-end-majority b46 `641/720` exact, ALL misses ±2, zero material; MDURATION `644/720` mirrors** (division staging validated). Suite 1,609 green (incl. 8 new pins: catalog pair, b2/b3, quarterly, leap, Feb-end + 31st month-end guards). OPEN residual: ±1-2 ULP off-coupon class (b44 143 rows banked `agentX_b44_residual143.json`) = the SHARED fractional x87 pow-chain wall (same class as PRICE's 14 rows) — cross-lane, closes with the pow-wall probe (b41/b43 family). | NUM-S | M3 | W109_G6_BOND_SCHEDULE_IDENTIFICATION_20260720.md / agentX_results.md / b44+b45+b46 |
