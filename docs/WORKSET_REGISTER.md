...
2. `crates/oxfunc_core/src/function_call.rs`
...
(each Excel quirk a closed-enum axis, the landed `real_result_policy` being the prototype),
collapsing the parallel dispatch representations (calc-dispatch chain, generated by-index
table, XLL scalar table) onto one generic executor with generated specializations, and
piloting the migration on the unary-numeric family where the drift is live.

Canonical surfaces:
1. `docs/worksets/W105_DECLARATIVE_FUNCTION_SPEC_AND_GENERIC_EXECUTOR.md`
2. `docs/decisions/ODR-FN-004-declarative-function-spec-and-generic-executor.md`
3. `crates/oxfunc_core/src/function.rs` (`FunctionMeta` → `FunctionSpec` growth surface)
4. `crates/oxfunc_core/src/functions/excel_numeric.rs` (`ExcelRealPolicy` / `ArgDomainGuard` prototype)
5. `crates/oxfunc_core/src/functions/surface_dispatch.rs` and
   `crates/oxfunc_core/src/functions/surface_dispatch_by_index_generated.rs` (the paths to collapse)
6. `.beads/` W105 epic and task lanes

Entry conditions:
1. `ODR-FN-004` is accepted and indexed.
...
5. ATANH's exact current-reference graph is identified and landed in
   ...
   closed signed off, and G4-02 is retired. Other BUG-FUNC-027 subclasses and
   the wider W109 campaign remain open; no FEC/F3E handoff is needed.
...
7. ACOTH's exact current-reference graph landed in `7f7eac9`: below
   `0x400d92b14ec204f3`, native binary64 ratio add/sub feed
   a stored-x87 divide and worksheet LN; at/above it, a direct inverse
   odd-power series stores every reciprocal/multiply/divide/add through x87
   PC64, and a subnormal reciprocal publishes +0 for both signs. Discovery is
   `202217/202217`, the candidate-frozen prior-disjoint held-out is
   `66552/66552`, and actual production replay is `268769/268769`. Focused,
   full-core, seven-tool release-check, and 492-job Lean gates pass.
