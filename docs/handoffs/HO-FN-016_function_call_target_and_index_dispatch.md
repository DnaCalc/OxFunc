# HO-FN-016 - Function Call Target and Full-Catalog Index Dispatch
...
OxFml should be able to resolve a function/operator call target once, retain a
...
1. `oxfunc_core::function_call::FunctionCallTarget`
2. `oxfunc_core::function_call::FunctionExecutionContextBundle`
3. `oxfunc_core::function_call::FunctionCallScratch`
4. `oxfunc_core::function_call::ExpressionHoistPolicy`
...
11. hoistability checks under explicit expression hoist policy.
...
`FunctionExecutionContextBundle` bundles the FEC providers used by OxFunc function
semantics:
...
`FunctionCallScratch` is an optional reusable argument buffer for repeated
compiled function-target invocation. It is not a semantic cache. OxFml may reuse it
...
1. replace repeated broad dispatcher calls in compiled/hot paths with
   `FunctionCallTarget` handles where formula binding has already resolved the
   function/operator identity,
2. keep value-only versus full trace execution as an OxFml trace-mode decision,
3. use `FunctionCallTarget` metadata for generic purity/hoisting gates rather than
...
3. `cargo test --manifest-path crates\oxfunc_core\Cargo.toml --lib function_call`
...
3. Initial consumption exists: the first compiled/hot-path slice already
   consumes `FunctionCallTarget`, `FunctionExecutionContextBundle`, and `FunctionCallScratch`.
