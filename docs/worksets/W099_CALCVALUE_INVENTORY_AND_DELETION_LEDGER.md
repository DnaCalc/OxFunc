...
open_lanes:
1. Generated by-index dispatch still calls kernels through the temporary `legacy_kernel_args_from_calc_values` projection; W099-012 owns kernel signature migration.
2. Extended-call publication still uses `calc_values_from_legacy_call_args` where legacy extended carriers remain in play; W099-010/W099-012/W099-015 own final rich/extended carrier deletion.
...
4. Dispatch conversion helpers were renamed to `legacy_kernel_args_from_calc_values` and `calc_values_from_legacy_call_args`, making their deletion owners explicit.
...
open_lanes:
1. The generated by-index dispatch bridge still projects through `legacy_kernel_args_from_calc_values(...)` for unmigrated kernel families and remains W099-015 residue.
...
4. Issue checked: `calc_values_from_legacy_call_args(...)` is still used by other legacy/prepared bridge paths and was intentionally not removed in this bead.
