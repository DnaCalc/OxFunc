...
Shape OxFunc's function-surface and dispatch architecture for long-term full-model optimization, where large Excel workbooks can be recalculated through resolved function handles, graph-level planning, concurrency, cacheable function execution contexts, and future compiled backends without moving function semantics out of OxFunc.
...
4. Scratch/state reuse exists only in localized paths and is not yet a first-class function-call contract.
...
1. Resolve function id or surface name once into a stable function-call target handle.
2. Execute through a full-catalog dispatch table keyed by a stable dispatch key or catalog ordinal.
3. Use one erased function execution context ABI for all surface handlers.
...
1. `FunctionCallTarget` as the resolved immutable function-handle surface.
2. `FunctionExecutionContextBundle` as the concrete FEC provider bundle for resolver, locale, time, random, host info, callable invocation, RTD, and registered external providers.
...
7. Scratch/state contracts for repeated function-target invocation and array-lift/arg-preparation reuse.
...
W096 should make these facts available at or near the resolved function call target:
...
17. whether a function is pure after argument preparation under a fixed execution context.

## 8. Initial Epic Lanes

1. Resolved function-call target API consolidation.
2. Erased FEC provider ABI.
...
8. OxFml receives a handoff describing the resolved function-call target API, FEC provider bundle, trace/value-mode implication, and remaining integration obligations.
...
4. OxFunc may expose metadata and stable target ids needed for trace templates, but should not make tracing part of ordinary value invocation.
...
3. W094 owns locale-profile facts consumed by function execution contexts.
4. W095 owns callable batching and lambda-helper hot-loop allocation work.
5. W096 generalizes the W095 resolved-target direction from lambda hot loops to whole-model optimized recalculation.

## 12. Tracking

1. Primary code surfaces:
   `crates/oxfunc_core/src/function_call.rs`,
...
2. `cargo test --manifest-path crates\oxfunc_core\Cargo.toml --lib function_call`
...
5. OxFml reports initial consumption of `FunctionCallTarget`,
   `FunctionExecutionContextBundle`, and `FunctionCallScratch` in its first compiled/hot-path
