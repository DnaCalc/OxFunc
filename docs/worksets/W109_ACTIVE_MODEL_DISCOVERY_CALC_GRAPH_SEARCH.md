...
| Piece | Where | What |
|---|---|---|
| Oracle cache | `smart-fuzzer/tools/OracleCache.psm1` (+ `Test-OracleCacheHelpers.ps1`, 28 checks) | Persistent JSONL cache keyed by `(function, arg bits, build, cpu)` wrapping `CellRefBatch.psm1`; **no Excel question is ever asked twice**. Shards under `smart-fuzzer/cache/oracle/build-<n>/`. |
