...
4. current value-required lanes materialize same-sheet rect-style `MultiArea` through OxFunc-owned resolver-driven row-vector combination in member order,
5. `OP_TRIM_REF_*` currently admit structural reference-target normalization only,
6. `OP_SPILL_REF` remains the explicit spill-anchor operator slice already owned in the function lane and is reconciled into `W45` as part of the reference family.

## 3. Out Of Slice
1. `@` / implicit intersection,
2. broader multi-area materialization beyond same-sheet rect-style parts and the named current value-required consumers,
...
9. current local seam direction is that union result identity is `ReferenceKind::MultiArea`, the old non-`MultiArea` parenthesized carrier is no longer admitted on the named local consumers, and same-sheet value-required materialization now happens in OxFunc rather than OxFml-local helper code
