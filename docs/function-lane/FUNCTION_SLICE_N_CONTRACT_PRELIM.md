# Function Slice Contract (Prelim) - N()

## 1. Slice Identity
1. `function_id`: `FUNC.N`
2. `display_name`: `N`
3. `owner_lane`: `OxFunc`
...
1. arity:
   ...
   - maximum: `1`
2. admission policy:
   - admitted for `N(value)` over scalar, array, error, and dereferenced blank-cell lanes in the current slice.
...
2. logicals become `1` / `0`.
3. text and blank cells become numeric zero.
4. worksheet errors propagate unchanged.
5. arrays map elementwise with the same policy.

## 6. Post-call Adaptation Policy
1. successful scalar evaluation returns a numeric or error `EvalValue`.
...
4. current status rationale:
   - `N()` is `function-phase-complete` for the current reference baseline,
   - the important seam is now explicit: dereferenced blank single-cell input normalizes to prepared `empty_cell`, after which `N()` returns numeric zero,
   - array mapping and error propagation are pinned in Rust, Lean, and workbook replay.

## 9. Artifact Bindings
1. Rust: `crates/oxfunc_core/src/functions/n_fn.rs`
2. Lean: `formal/lean/OxFunc/Functions/N.lean`
