# Function Slice Contract (Prelim) - DOLLAR()
...
1. arity:
   ...
   - maximum: `2`
2. admission policy:
   - admitted in this slice as `DOLLAR(number, [decimals])` through the local locale-format render seam.
...
3. logicals coerce numerically (`TRUE -> 1`, `FALSE -> 0`) in the current slice.
4. blank cells coerce to zero in the current slice.

## 5. Core Outcome Model
1. successful evaluation delegates to the locale-profile currency renderer.
2. current-host seed rows pin:
   - positive currency output
   - negative currency output
   - text-numeric admission
   - logical admission
   - blank-cell zeroing
3. current-host renderer currently uses currency symbol `R`, space grouping, and `2` default decimals.
