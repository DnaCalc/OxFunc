...
7. `RANDARRAY` is treated as the inventory-stated owned-file target name, not as a shared integration claim about `RANDARRAY`. The current kernel returns a scalar or rectangular in-memory array, defaults to `1 x 1`, defaults the numeric range to `[0, 1)`, and supports a bounded whole-number mode.
...
8. `RANDARRAY()` with a deterministic test provider yields a `1 x 1` array
9. `RANDARRAY(2,2,10,12,TRUE)` over deterministic provider samples yields a bounded `2 x 2` whole-number matrix
...
2. `EUROCONVERT` and `RANDARRAY` were replayed natively on `2026-03-18` and returned `#NAME?` on the current host baseline.
...
3. `RANDARRAY` dynamic-array/version reconciliation now belongs to `W025`.
