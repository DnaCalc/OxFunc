# W24 Batch 05 Runtime Requirements - Confidence/Test Helper Family
...
## 1. Packet Purpose
1. replay the admitted current-baseline packet for `CONFIDENCE.T` and `Z.TEST`,
2. pin the `Z.TEST` survivor policy under native Excel,
3. bind the family to a deterministic worksheet artifact in `.tmp`.

## 2. Inputs
1. scenario manifest: `docs/function-lane/W24_BATCH05_CONFIDENCE_TEST_SCENARIO_MANIFEST_SEED.csv`
2. worksheet host: native Excel via COM
3. output artifact: `.tmp/w24-batch05-confidence-test-results.csv`
