# Function Slice Contract (Prelim) - ASIN()

## 1. Slice Identity
1. `function_id`: `FUNC.ASIN`
2. `display_name`: `ASIN`
3. `owner_lane`: `OxFunc`
...
1. arity:
   ...
   - maximum: `1`
2. admission policy:
   - admitted for `ASIN(number)` under the ordinary worksheet numeric-coercion lane.
...
1. admitted scalar numeric inputs in `[-1, 1]` return the ordinary inverse-sine numeric result.
2. inputs outside `[-1, 1]` return worksheet-visible `#NUM!`.
3. admitted array inputs return a shape-preserving array of per-element results.
...
4. current status rationale:
   - `ASIN()` is `function-phase-complete` for the current reference baseline,
   - numeric-text admission, explicit `#NUM!` domain failure, and elementwise array-lift behavior are now aligned across Rust, Lean, and workbook replay,
   - broader locale/version sweeps remain orthogonal validation-phase work.

## 9. Artifact Bindings
1. Rust: `crates/oxfunc_core/src/functions/asin.rs`
2. Lean: `formal/lean/OxFunc/Functions/Asin.lean`
