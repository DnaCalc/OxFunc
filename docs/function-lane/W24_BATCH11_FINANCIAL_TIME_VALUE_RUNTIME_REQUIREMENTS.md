# W24 Batch 11 Runtime Requirements - Financial Time-Value Family
...
## 1. Packet Purpose
1. replay the admitted current-baseline packet for the financial time-value family,
2. pin the corrected `ISPMT` period-index behavior,
3. bind the family to a deterministic worksheet artifact in `.tmp`.

## 2. Inputs
1. scenario manifest: `docs/function-lane/W24_BATCH11_FINANCIAL_TIME_VALUE_SCENARIO_MANIFEST_SEED.csv`
2. worksheet host: native Excel via COM
3. output artifact: `.tmp/w24-batch11-financial-time-value-results.csv`
