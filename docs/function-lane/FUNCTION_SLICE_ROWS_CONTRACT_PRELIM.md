# Function Slice Contract (Prelim) - ROWS()

## 1. Slice Identity
1. `function_id`: `FUNC.ROWS`
2. `display_name`: `ROWS`
3. `owner_lane`: `OxFunc`
...
1. arity:
   ...
   - maximum: `1`
2. admission policy:
   - admitted for `ROWS(reference_or_array)`.
...
1. `ROWS(single_cell_reference)` returns `1`.
2. `ROWS(area_reference)` returns the number of rows in the referenced area as a scalar number.
3. `ROWS(array_constant)` returns the row count of the array.
...
2. Rust obligations:
   - correct row-count extraction from reference and array arguments.
   - arity rejection for zero-argument and multi-argument calls.
   - unit tests mapped in correlation ledger.

## 7. Artifact Bindings
1. Rust: `crates/oxfunc_core/src/functions/rows_fn.rs`
2. Lean: `formal/lean/OxFunc/Functions/Rows.lean`
...
3. current status rationale:
   - `ROWS()` is `function-phase-complete` for the current reference baseline,
   - scalar row-count extraction from references and arrays is aligned across Rust and Lean,
