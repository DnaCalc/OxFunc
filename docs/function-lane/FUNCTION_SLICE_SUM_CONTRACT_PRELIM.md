...
1. arity:
   ...
   - maximum: `255`
2. admission policy:
   - admitted as a variadic aggregate fold with provenance-sensitive argument preparation.
...
1. OxFunc prepares `SUM` arguments as provenance-carrying aggregate items rather than plain scalar values.
...
4. the current OxFunc provenance classes for `SUM` are:
   - `direct_scalar`
   - `direct_array_literal`
   - `reference_derived`
   - `opaque_array_value`
5. when only an evaluated array reaches the OxFunc surface without stronger upstream provenance, the explicit OxFunc fallback is `opaque_array_value`.
...
4. `SUM` currently applies the same scan policy to all array-like origins, but the provenance classes remain explicit because the seam must be able to represent them distinctly.
...
4. current status rationale:
   ...
   - the OxFunc seam now models the required aggregate provenance classes explicitly and defines the fallback for provenance-erased arrays,
   - remaining OxFml work is upstream propagation of the richer provenance vocabulary, not a remaining OxFunc function-semantic gap.
