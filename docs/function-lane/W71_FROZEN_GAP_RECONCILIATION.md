...
2. `517` rows already witness-covered in the completed V2 surface,
3. `0` rows still requiring W71 witness population.
...
W71 uses the frozen W069 ledgers as its control surface.

That means:
1. the W71 gap inventory must reconcile exactly to the frozen W069 tranche
   counts,
...
4. the eventual W71 final reconciliation must show no leftover supported-row
   witness gap in the parked baseline.

## 4. Current Status
1. W71 witness population now covers all supported rows.
2. The frozen gap inventory and tranche register remain consistent.
3. This reconciliation is the first W71 row-level control artifact.
