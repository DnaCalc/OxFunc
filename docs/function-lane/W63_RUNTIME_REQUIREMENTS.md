# W63 Runtime Requirements

1. Native Excel baseline runner:
   - `powershell -ExecutionPolicy Bypass -File tools/w63-probe/run-w63-date-time-business-day-baseline.ps1`
2. Targeted Rust coverage:
   - `cargo test --manifest-path crates/oxfunc_core/Cargo.toml date_parts_family -- --nocapture`
   - `cargo test --manifest-path crates/oxfunc_core/Cargo.toml date_week_family -- --nocapture`
   - `cargo test --manifest-path crates/oxfunc_core/Cargo.toml workday_networkdays_family -- --nocapture`
