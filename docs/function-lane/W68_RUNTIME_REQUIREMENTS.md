# W68 Runtime Requirements

1. Native Excel baseline runner:
   - `powershell -ExecutionPolicy Bypass -File tools/w68-probe/run-w68-lookup-logical-baseline.ps1`
2. Targeted Rust coverage:
   - `cargo test --manifest-path crates/oxfunc_core/Cargo.toml --lib choose_ifs_family -- --nocapture`
   - `cargo test --manifest-path crates/oxfunc_core/Cargo.toml --lib vhlookup_family -- --nocapture`
