...
- Current state: active W099 execution lane through W099-015. The initial W099-001 inventory mapped legacy `EvalValue`, `CallArgValue`, `PreparedArgValue`, `EvalArray`, `ArrayCellValue`, `ExtendedValue`, `LambdaValue`, old reference-provider, and `.target` surfaces to deletion batches; W099-002 through W099-014 landed the typed `ReferenceLike`, `ReferenceSystemProvider`, CalcValue, callable, cleanup, and OxFml/OxCalc W060 provider foundations; and W099-015 removed the public legacy carrier names plus the public `ReferenceLike.kind`/`ReferenceLike.target` mirrors from active OxFunc Rust code while updating OxFml consumers to the new function-facing names. The next gate is W099-016 final cross-repo validation.
