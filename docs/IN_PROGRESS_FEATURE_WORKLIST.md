...
- Current state: active landed-ref promotion follow-up from live Excel replay. `XMATCH`, `MATCH`, `VLOOKUP`, and `HLOOKUP` array-valued `lookup_value` corrections are landed on `5d54d7f4ab2cdde6458272292d15ae1b317a0fef`; fresh 2026-04-29 replay pinned adjacent `XLOOKUP` shape-preserving multi-needle behavior, and the local `XLOOKUP` correction is validated in the working tree pending landed-ref promotion.
...
- Current state: active local review seed. `LEFT` / `LEFTB`, `RIGHT` / `RIGHTB`, and `MID` / `MIDB` are landed on `5d54d7f4ab2cdde6458272292d15ae1b317a0fef`, and the first bounded follow-on `W066` batch for `CHAR`, `CODE`, `LOWER`, `UPPER`, `TRIM`, `REPT`, `TEXTAFTER`, and `TEXTBEFORE` is landed on `2e818f03a71ba393690275a7fb437ddd9a6bf760`. The second bounded follow-on batch found and locally corrected `FIND` / `FINDB`, `SEARCH` / `SEARCHB`, `REPLACE` / `REPLACEB`, `PROPER`, and `SUBSTITUTE` single-array spill lanes under `BUG-FUNC-016`; broader systematic review remains open.
